...
c declare (i.e. book) the histograms
      ...
      call HwU_book(3,'total QCD       ', 5,0.5d0,5.5d0)
      call HwU_book(4,'total EW        ', 5,0.5d0,5.5d0)
      return
      end
...
c only fill the total rate for the Born when ibody=3
      ...
      if (orders_tag_plot.eq.602) call HwU_fill(3,var,wgts)
      if (orders_tag_plot.eq.800) call HwU_fill(4,var,wgts)
      return
      end
