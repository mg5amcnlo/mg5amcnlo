...
c are filled from the PDG codes (iPDG array) in this function.
     ...
     $     ,is_a_ph(nexternal)
      ...
      integer n_isoph, n_dphiph
      double precision pt_hardestph
      logical is_isoph, found_hardestph, found_hardestdphiph
      double precision p_taggamma(0:3,nexternal)
      integer iex
      integer nph_required
      double precision delta_phi, r2
...
c find the photons
      ...
      if (ptgmin.ne.0d0) then
         ...
         alliso=.true.
         n_isoph = 0
         ! these two variables are in case asymmetric cuts are imposed
         pt_hardestph = 40d0
         ! at least three photons
         nph_required = 3
         found_hardestph = .false.
...
c End of loop over photons
          !!write(*,*) 'IS ISO', is_isoph
          if (is_isoph) then
           n_isoph = n_isoph + 1
           p_taggamma(0:3, n_isoph) = pgamma(0:3, j)
           endif
         enddo
         !!write(*,*) 'N isoph', n_isoph,found_hardestph 
         ! at least three photons, and one satisfyin the asymmetric cut
         if(n_isoph.lt.nph_required.or..not.found_hardestph )then
            passcuts_user=.false.
            return
         endif
 444     continue
c End photon isolation
      ...
      ! now, p_taggamma contains only momenta satisfying
      ! isolation/pt/eta cuts, of which one with 

C photon-photon separation
      n_dphiph = 0
      found_hardestdphiph = .false.
      ! case 1) there are three isolated photons
      if (n_isoph.eq.nph_required) then
        do i = 1, n_isoph
          if (pt(p_taggamma(0, i)).gt.pt_hardestph) found_hardestdphiph = .true.
          do j = i+1, n_isoph
            if (dsqrt(R2(p_taggamma(0, i), p_taggamma(0, j))).lt.0.4d0) then
               passcuts_user = .false.
               return
            endif
          enddo
        enddo
        ! check that one photon satisfies the pt_hardestph condition
        if (.not.found_hardestdphiph) then
           passcuts_user = .false.
           return
        endif
      else
          ! now it is a mess, we have to re-do the above loop
          ! but excluding one photon at a time
          ! if a suitable triplet is found then we are happy
        do iex = 1, n_isoph
          n_dphiph = 0
          passcuts_user = .true.
          found_hardestdphiph = .false.
          do i = 1, n_isoph
            ...
            do j = i+1, n_isoph
              if (j.eq.iex) cycle
              if (delta_phi(p_taggamma(0, i), p_taggamma(0, j)).lt.0.4) then
                passcuts_user = .false.
              endif
            enddo
          enddo
          ! if passcuts_user is still true here, then we have found a
          ! triplet satisfying the phi separation.
          ! If there is also a photon hardest than the asymm cut
          ! then we can exit
          if (passcuts_user .and. found_hardestdphiph) goto 24
        enddo
        passcuts_user =.false.
        return
      endif
