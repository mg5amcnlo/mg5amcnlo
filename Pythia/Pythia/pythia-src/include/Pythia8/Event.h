...
public:
  ...
  void rotbst(const Vec4& p1, const Vec4& p2) {pSave.rotbst(p1, p2);
    if (hasVertexSave) vProdSave.rotbst(p1, p2);}
...
public:
  ...
  void rotbst(const Vec4& p1, const Vec4& p2)
    {for (int i = 0; i < size(); ++i) entry[i].rotbst(p1,p2);}
