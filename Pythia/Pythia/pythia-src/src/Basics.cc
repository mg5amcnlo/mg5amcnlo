...
// Catani-Seymour type boost.

void Vec4::rotbst(const Vec4& p1, const Vec4& p2) {

  Vec4 kTilde(p1);
  Vec4 k(p2);
  Vec4 kSum(kTilde + k);

  Vec4 pIn = Vec4(xx,yy,zz,tt);
  double kSum2    = kSum.m2Calc();
  double k2       = k.m2Calc();
  double kTildeXp = kTilde*pIn;
  double kSumXp   = kSum*pIn;
  Vec4 res = pIn - kSum * 2.0*( kSumXp / kSum2 ) + k * 2.0 *( kTildeXp/k2);  
...
ostream& operator<<(ostream& os, const Vec4& v) {
  os << fixed << setprecision(9) << " " << setw(15) << v.xx << " "
     << setw(15) << v.yy << " " << setw(15) << v.zz << " " << setw(15)
     << v.tt << " (" << setw(15) << v.mCalc() << ")\n";
  return os;
}
