...
bool PartonLevel::init( Info* infoPtrIn, Settings& settings,
  ...
  SpaceShower* spacePtrIn, BeamRemnants* remnantsPtrIn,
  ...
  timesDecPtr->init( beamAPtr, beamBPtr);
  //if (doISR) spacePtr->init( beamAPtr, beamBPtr);
  spacePtr->init( beamAPtr, beamBPtr);
  ...
  if (!remnantsPtr->init( infoPtr, settings, rndmPtr, beamAPtr, beamBPtr,
...
bool PartonLevel::next( Event& process, Event& event) {
  ...
  for (int iTry = 0; iTry < NTRY; ++ iTry) {
    ...
    if (!doTrial && physical && doRemnants
      ...
      && !remnantsPtr->add( event, iFirst, doDiffCR)) physical = false;
...
void PartonLevel::setupResolvedDiff( Event& process) {
  ...
  remnantsPtr->reassignBeamPtrs(  beamAPtr, beamBPtr, iDS);
...
void PartonLevel::leaveResolvedDiff( int iHardLoop, Event& process,
  ...
  remnantsPtr->reassignBeamPtrs(  beamAPtr, beamBPtr, 0);
  colourReconnection.reassignBeamPtrs(  beamAPtr, beamBPtr);
...
void PartonLevel::setupHardDiff( Event& process) {
  ...
  remnantsPtr->reassignBeamPtrs(  beamAPtr, beamBPtr, (isHardDiffB) ? 2 : 1);
  colourReconnection.reassignBeamPtrs(  beamAPtr, beamBPtr);
...
bool PartonLevel::setupResolvedLeptonGamma( Event& process) {
  ...
  remnantsPtr->reassignBeamPtrs(  beamAPtr, beamBPtr, beamOffset);
  colourReconnection.reassignBeamPtrs(  beamAPtr, beamBPtr);
...
bool PartonLevel::resonanceShowers( Event& process, Event& event,
  ...
  while (nHardDone < process.size()) {
    ...
    Vec4 pHardMother( hardMother.p() );
    Vec4 pAftMother( aftMother.p() );
    ...
    for (int i = iBegin; i < process.size(); ++i) {
      ...
      // new boost, according to CS
      now.rotbst( pHardMother, pAftMother);
