...
Pythia::~Pythia() {
  ...
  // Delete beam remnants created with new.
  if (useNewRemnants) delete remnantsPtr;
...
void Pythia::initPtrs() {
  ...
  // Initial values for pointer to beam remnants.
  useNewRemnants  = false;
  remnantsPtr     = 0;
...
bool Pythia::init() {
  ...
  // Set up beam remnant object.
  if (remnantsPtr == 0) {
    remnantsPtr    = new BeamRemnants();
    useNewRemnants = true;
  }
  ...
  if ( doPartonLevel && doProcessLevel && !partonLevel.init( &info, settings,
    ...
    spacePtr, remnantsPtr, &rHadrons, userHooksPtr, mergingHooksPtr, false) ) {
    info.errorMsg("Abort from Pythia::init: "
  ...
  if ( !doProcessLevel || !doPartonLevel) partonLevel.init( &info, settings,
    ...
    timesDecPtr, 0, 0, 0, &rHadrons, 0, 0, false);
  ...
  if ( doMerging && !trialPartonLevel.init( &info, settings, &particleData,
      ...
      spacePtr, remnantsPtr, &rHadrons, userHooksPtr, mergingHooksPtr, true) ) {
    info.errorMsg("Abort from Pythia::init: "
...
bool Pythia::initKinematics() {
  ...
  if (boostType == 2) {
    // Find boost+rotation needed to move from/to CM frame.
    MfromCM.reset();
    MfromCM.bst( Vec4( 0., 0., pzAcm, eA), pAinit );
    MtoCM = MfromCM;
    MtoCM.invert();
  }
...
void Pythia::boostAndVertex( bool toLab, bool setVertex) {
  ...
  if (toLab) {
    ...
    if      (boostType == 2) process.rotbst(MfromCM);
    ...
    if (event.size() > 0) {
      ...
      if      (boostType == 2) event.rotbst(MfromCM);
...
PDF* Pythia::getPDFPtr(int idIn, int sequence, string beam, bool resolved) {
  ...
  // Also photon inside lepton PDFs.
    ...
    //else tempPDFPtr = new LeptonPoint(idIn);
      ...
      // Allow shower PDFs for e+e- -> jets
