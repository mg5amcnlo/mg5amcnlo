...
{
  model = new Parameters_sm_ckm(); 
  if (card_path != string())
    initModelFromSLHACard(card_path); 
  releaseModelOnExit = true; 
  load_processes(); 
}
PY8MEs::PY8MEs(Parameters_sm_ckm * model_input) : model(model_input)
{
...
// model.
Parameters_sm_ckm * PY8MEs::instantiateModel(string param_card_path) 
{
  Parameters_sm_ckm * new_model = new Parameters_sm_ckm(); 
