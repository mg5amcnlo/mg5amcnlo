...
string replaceOld[4] = { "Copyright (C) 2021", "GNU GPL version 2",
  "\"true\"", "\"false\"" }; 
string replaceNew[4] = { "Copyright (C) 2021", "GNU GPL v2 or later",
  "\"on\"",   "\"off\"" };
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int i = 0; i < files.size(); ++i) {
    ...
    // Skip FJcore and MixMax files; they give plenty of warnings 
    // not to be addressed.
    if (fileName.find("FJcore") != string::npos) isCode = false;
    if (fileName.find("MixMax") != string::npos) isCode = false;
