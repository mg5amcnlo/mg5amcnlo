// test090.cc is a part of the PYTHIA event generator.
// Copyright (C) 2021 Torbjorn Sjostrand.
// PYTHIA is licenced under the GNU GPL v2 or later, see COPYING for details.
// Please respect the MCnet Guidelines, see GUIDELINES for details.

// Test PYTHIA run with LHEF input and sample-by-sample generation of
// CKKW-L merged events with trial enhancements.
...
int main(){
  ...
  // Input parameters:
  pythia.readFile("test090.cmnd",0);
  ...
  Pythia8ToHepMC toHepMC("test090.dat");
  ...
  if(!doMatchMerge) {
    ...
    for (int iMerge = 0; iMerge < nMerge; ++iMerge) {
      ...
      pythia.readFile("test090.cmnd", iMerge);
      // Initialise.
      pythia.init();
