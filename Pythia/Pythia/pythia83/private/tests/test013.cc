// test013.cc is a part of the PYTHIA event generator.
...
// just like in test012.cc, but here with the difference that information is
// stored in test013.cmnd and read out using the subruns possibility.
...
int main() {
  ...
  // Read in subrun-independent data from test013.cmnd.
  pythia.readFile( "test013.cmnd", 0);
  ...
  for (int iSubrun = 1; iSubrun <= nSubrun; ++iSubrun) {
    ...
    // Read in subrun-specific data from test013.cmnd.
    pythia.readFile( "test013.cmnd", iSubrun);
