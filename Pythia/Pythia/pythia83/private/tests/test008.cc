// test008.cc is a part of the PYTHIA event generator.
...
int main() {
  ...
  // Number of bins to use. In mode 2 read from test008.cmnd file.
  int nBin = 5;
  if (mode == 2) {
    pythia.readFile("test008.cmnd");
  ...
  for (int iBin = 0; iBin < nBin; ++iBin) {
    ...
    else if (mode == 2) pythia.readFile("test008.cmnd", iBin);
