// test200.cc is a part of the PYTHIA event generator.
// Copyright (C) 2021 Torbjorn Sjostrand.
// PYTHIA is licenced under the GNU GPL v2 or later, see COPYING for details.
// Please respect the MCnet Guidelines, see GUIDELINES for details.

// Tests VINCIA shower model, on Z decays at LEP I, with some basic
// event shapes, spectra, and multiplicity counts.  Also useful as a
// basic test of the respective final-state showers.
...
int main() {
  ...
  ofstream ofs("test200.dat");
  write(ofs, histNCharged);
  write(ofs, histT);
  write(ofs, histC);
  write(ofs, histD);
  write(ofs, histX1);
  write(ofs, histX1Gamma);
  write(ofs, histX2Gluon);
  write(ofs, histX2Quark);
  write(ofs, histXStrange);
  write(ofs, histXCharm);
  write(ofs, histXBottom);
  write(ofs, histMSS);
  write(ofs, histMCC);
  write(ofs, histMBB);
  ...
  ofs<<endl;
  ofs<<fixed;
  ofs<< " <nGluonSplit> = "<<num2str(nQsum * normFac-1.0)<<endl;
  ofs<< " <nG->SS>      = "<<num2str(nStrangeSum * normFac / 2.)<<endl;
  ofs<< " <nG->CC>      = "<<num2str(nCharmSum * normFac / 2.)<<endl;
  ofs<< " <nG->BB>      = "<<num2str(nBottomSum * normFac / 2.)<<endl;
  ofs<< " <nPartons>    = "<<num2str(nPsum * normFac)<<endl;
  ofs<< " <nPhotons>    = "<<num2str(nGammaSum * normFac)<<endl;
  ofs<< " <nCharged>    = "<<num2str(nCsum * normFac)<<endl;
  ofs<< " <nBaryons>    = "<<num2str(nBaryonSum * normFac)<<endl;
  ofs<<endl;
