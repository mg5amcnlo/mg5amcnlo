// test003.cc is a part of the PYTHIA event generator.
...
// All input is specified in the test003.cmnd file.
...
int main() {
  ...
  dndeta   *=  5. / nEvent;
  dndpT    *= 10. / nEvent;
  ofstream cmp("test003.dat");
  pThard.table(cmp, true);
  nFinal.table(cmp, true);
  nCharged.table(cmp, true);
  dndy.table(cmp, true);
  dndeta.table(cmp, true);
  dndpT.table(cmp, true);
  epCons.table(cmp, true);
  ...
  // After the program has run, type "python test003plot.py" (without the " ")
