...
// This tests the string shoving model supplied in the Rope
// Hadronization framework. It produces four histograms shoving the
// emergence of a "ridge" in two-particle correlations at high event
// multiplicity.
...
int main() {
  ...
  pythia.stat();
  ofstream cmp("test101.dat");
  deltaPhi1.table(cmp, true);
  deltaPhi2.table(cmp, true);
  deltaPhi3.table(cmp, true);
  deltaPhi4.table(cmp, true);
  return 0;
}
