// test010.cc is a part of the PYTHIA event generator.
...
public:
  ...
  ~MyUserHooks() {
    delete slowJet;
    ofstream cmp("test010.dat");
    pTtrial->table(cmp); delete pTtrial;
    pTselect->table(cmp); delete pTselect;
    pTaccept->table(cmp); delete pTaccept;
    nPartonsB->table(cmp); delete nPartonsB;
    nJets->table(cmp); delete nJets;
    nPartonsA->table(cmp); delete nPartonsA;
    nFSRatISR->table(cmp); delete nFSRatISR;
  }
...
int main() {
  ...
  pythia.setUserHooksPtr( make_shared<MyUserHooks>());
