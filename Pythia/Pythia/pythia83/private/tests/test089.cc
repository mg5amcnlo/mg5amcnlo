...
int main(){
  ...
  // Input parameters:
  pythia.readFile("test089.cmnd",0);
  ...
  Pythia8ToHepMC toHepMC("test089.dat");
  ...
  if(!doMatchMerge) {
    ...
    for (int iMerge = 0; iMerge < nMerge; ++iMerge) {
      ...
      pythia.readFile("test089.cmnd", iMerge);
      // Initialise.
      pythia.init();
