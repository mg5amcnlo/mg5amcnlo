...
# Execute as "./private/doxygen.sh".

################################################################################
# FUNCTION: Run Doxygen via Docker.
################################################################################
function generate() {
    docker run -i -t -v "$PWD:$PWD" -w $PWD -u `id -u` --rm\
	   pythia8/dev:doxygen $@
}
...
# Check Docker and SED exist.
if ! type "docker" &> /dev/null; then
    echo "Error: Docker not found."; exit; fi
...
$SED "s|\(PROJECT_NUMBER *= *\)[^ ]*|\1$VER|g" private/doxygen.cfg

# Create the documentation and clean up.
generate private/doxygen.cfg
