...
void MyHardProcess::initOnProcess( string process, ParticleData* particleData) {
...
void MyHardProcess::translateProcessString( string process){
...
bool MyHardProcess::allowCandidates(int iPos, vector<int> Pos1,
...
void MyHardProcess::storeCandidates( const Event& event, string process){
...
bool MyHardProcess::matchesAnyOutgoing(int iPos, const Event& event){
...
bool MyHardProcess::findOtherCandidates(int iPos, const Event& event,
    bool doReplace){
...
bool MyHardProcess::exchangeCandidates( vector<int> candidates1,
...
// Initialise MyMergingHooks class
...
bool MyMergingHooks::doVetoStep( const Event& process, const Event& event,
  bool doResonance ) {
...
int MyMergingHooks::getNumberOfClusteringSteps(const Event& event,
  bool resetJetMax ){
...
bool MyMergingHooks::checkAgainstCut( const Particle& particle){
...
double MyMergingHooks::scalems( const Event& event){
  ...
  for(int i=0; i < int(FinalPartPos.size()); ++i){
    ...
    if ( event[in1].colType() != 0 ) {
      for(int j=0; j < int(FinalPartPos.size()); ++j) {
        ...
        if ( i != j ){
          // Check with first initial as recoiler
          double temp = rhoPythia( event, FinalPartPos[i],FinalPartPos[j],in1, 0);
          pt12 = min(pt12, temp);
        }
      }
    }
    ...
    if ( event[in2].colType() != 0) {
      for(int j=0; j < int(FinalPartPos.size()); ++j) {
        ...
        if ( i != j ){
          ...
          double temp = rhoPythia( event, FinalPartPos[i],FinalPartPos[j],in2, 0);
          pt12 = min(pt12, temp);
        }
      }
    }
...
double MyMergingHooks::rhoPythia(const Event& event, int rad, int emt, int rec,
              int ShowerType){
  ...
  if ( useShowerPlugin() ) {
    ...
    if (isFSR) {
      vector<string> name = showers->timesPtr->getSplittingName(event, rad, emt, rec);
      for (int i=0; i < int(name.size()); ++i) {
        stateVars = showers->timesPtr->getStateVariables(event, rad, emt, rec, name[i]);
        double pttemp = ptret;
        if (stateVars.size() > 0 && stateVars.find("t") != stateVars.end()) pttemp = sqrt(stateVars["t"]);
        ptret = min(ptret,pttemp);
      }
    } else {
      vector<string> name = showers->spacePtr->getSplittingName(event, rad, emt, rec);
      for (int i=0; i < int(name.size()); ++i) {
        stateVars = showers->spacePtr->getStateVariables(event, rad, emt, rec, name[i]);
        double pttemp = ptret;
        if (stateVars.size() > 0 && stateVars.find("t") != stateVars.end()) pttemp = sqrt(stateVars["t"]);
        ptret = min(ptret,pttemp);
      }
...
int MyMergingHooks::findColour(int col, int iExclude1, int iExclude2,
