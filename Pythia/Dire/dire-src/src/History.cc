...
void listFlavs(const Event& event) {

cout << " (";
for (int i=0; i < event.size(); ++i) {
if (event[i].status() == -21) cout << " " << event[i].id();
}
cout << " ) -->> (";
for (int i=0; i < event.size(); ++i) {
if (event[i].status() ==  23) cout << " " << event[i].id();
if (event[i].status() ==  22) cout << " " << event[i].id();
}
cout << " ) ";
...
// emissions
const int MyHistory::NTRIAL = 1;

const double MyHistory::MCWINDOW = 0.1;
const double MyHistory::MBWINDOW = 0.1;
...
MyHistory::MyHistory( int depth,
         double scalein,
         Event statein,
         MyClustering c,
         //MyMergingHooks* mergingHooksPtrIn,
         ...
         DireTimes* fsrIn,
         DireSpace* isrIn,
         WeightContainer* psweightsIn,
         CoupSM* coupSMPtrIn,
         bool isOrdered = true,
         bool isAllowed = true,
         bool isAllowedOrdering = true,
  ...
  if (nFinalHeavy == 0 && nFinalLight == 2) depth = 0;
  ...
  if (mother && mother->MECnum > MECnum && clusterings.empty() ) { 
    fsr->debugPtr->eatCout(); 
    cout << scientific << setprecision(8);
    mother->state.list();
    state.list();
    cout << "Large difference in ME values!" << endl;
    cout << "  real emission state ME=" << mother->MECnum
         << " for "; listFlavs(mother->state); cout << endl;
    cout << "  born state          ME=" << MECnum
         << " for "; listFlavs(state);
    cout << " pT2=" << pow2(clusterIn.pT()) << endl;
    fsr->debugPtr->freeCout();
  } else if ( mother) {
    ;
  } else {
    fsr->debugPtr->eatCout(); 
    cout << "Sensible ME values for highest multiplicity ";
    listFlavs(state);
    cout << " ME=" << MECnum
         << " at pT2=" << pow2(clusterIn.pT()) << endl;
    fsr->debugPtr->freeCout();
  }
  ...
  multimap<double, MyClustering *> sorted;
  ...
  it != sorted.end(); ++it ) {
    ...
    // history node.
    children.push_back(new MyHistory(depth - 1,it->first, newState,
...
bool MyHistory::projectOntoDesiredHistories() {
  ...
  if (paths.size() > 0) {
    ...
    for ( map<double, MyHistory*>::iterator it = paths.begin();
      it != paths.end(); ++it )
      if (it->second->mother) {
        it->second->mother->setProbabilities();
        // Only need to set probabilities once!
        break;
      }
    ...
    for ( map<double, MyHistory*>::iterator it = paths.begin();
      it != paths.end(); ++it )
      if (it->second->mother) {
        it->second->mother->setEffectiveScales();
        // Only need to set effective scales once!
        break;
      }
  }
...
double MyHistory::weightMOPS(PartonLevel* trial, AlphaStrong * asFSR,
  ...
  if (mergingHooksPtr->settingsPtr->flag("PartonLevel:MPI")) mpiwt
    = selected->weightTreeEmissions( trial, -1, 0, njetsMaxMPI, maxScale );
...
double MyHistory::weightTREE(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN) {

  if ( mergingHooksPtr->canCutOnRecState() && !foundAllowedPath ) {
    string message="Warning in MyHistory::weightTREE: No allowed history";
    message+=" found. Using disallowed history.";
    infoPtr->errorMsg(message);
  }

  if ( mergingHooksPtr->orderHistories() && !foundOrderedPath ) {
    string message="Warning in MyHistory::weightTREE: No ordered history";
  ...
  if ( mergingHooksPtr->canCutOnRecState()
    ...
    string message="Warning in MyHistory::weightTREE: No allowed or ordered";
...
double MyHistory::weightLOOP(PartonLevel* trial, double RN ) {

  if ( mergingHooksPtr->canCutOnRecState() && !foundAllowedPath ) {
    string message="Warning in MyHistory::weightLOOP: No allowed history";
    message+=" found. Using disallowed history.";
    infoPtr->errorMsg(message);
  }
...
double MyHistory::weightFIRST(PartonLevel* trial, AlphaStrong* asFSR,
  AlphaStrong* asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN,
  Rndm* rndmPtr ) {
...
double MyHistory::weight_UMEPS_TREE(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN) {
...
double MyHistory::weight_UMEPS_SUBT(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN ) {
...
double MyHistory::weight_UNLOPS_TREE(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN,
  int depth) {
...
double MyHistory::weight_UNLOPS_LOOP(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN,
  int depth) {
...
double MyHistory::weight_UNLOPS_SUBT(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN,
  int depth) {
...
double MyHistory::weight_UNLOPS_SUBTNLO(PartonLevel* trial, AlphaStrong * asFSR,
  AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN,
  int depth) {
...
double MyHistory::weight_UNLOPS_CORRECTION( int order, PartonLevel* trial,
...
void MyHistory::printMyHistory( const double RN ) {
  MyHistory *  selected = select(RN);
...
void MyHistory::printStates() {
  if ( !mother ) {
    cout << scientific << setprecision(6) << "Probability="
         << prodOfProbs << endl;
  ...
  cout << scientific << setprecision(6) << "Probabilities:"
       << "\n\t Product =              " << prodOfProbs
...
bool MyHistory::getClusteredEvent( const double RN, int nSteps,
                Event& outState) {
...
bool MyHistory::getFirstClusteredEventAboveTMS( const double RN, int nDesired,
  ...
  // Set scales in the states to the scales pythia would have set.
  select(RN)->setScalesInMyHistory();
...
double MyHistory::getPDFratio( int side, bool forSudakov, bool useHardPDFs,
...
void MyHistory::setScalesInMyHistory() {
...
void MyHistory::scaleCopies(int iPart, const Event& refEvent, double rho) {
...
Event MyHistory::clusteredState(int nSteps) {
...
MyHistory * MyHistory::select(double rnd) {
  ...
  double sum = 0.;
  map<double, MyHistory*> selectFrom;
...
bool MyHistory::trimHistories() {
  ...
  for ( map<double, MyHistory*>::iterator it = paths.begin();
    ...
    if ( it->second->keep() && !it->second->keepMyHistory() )
      it->second->remove();
  }
...
bool MyHistory::isOrderedPath( double maxscale ) {
...
bool MyHistory::isAllowedOrderingPath( double maxscale ) {
  //int nSteps = mergingHooksPtr->getNumberOfClusteringSteps(state);
  double newscale = clusterIn.pT();
  if ( !mother ) return true;
  bool ordered = mother->isAllowedOrderingPath(newscale);
  if ( !ordered || maxscale < newscale) return false;
  return ordered;
}
...
bool MyHistory::allIntermediateAboveRhoMS( double rhoms, bool good ) {
...
bool MyHistory::foundAnyOrderedPaths() {
...
double MyHistory::weightTree(PartonLevel* trial, double as0, double aem0,
...
//    if (mergingHooksPtr->settingsPtr->flag("Dire:doMOPS"))
//      aemScale = mother->scaleEffective;
...
double MyHistory::weightFirst(PartonLevel* trial, double as0, double muR,
...
double MyHistory::weightFirstALPHAS( double as0, double muR,
...
double MyHistory::weightFirstPDFs( double as0, double maxscale, double pdfScale,
  Rndm* rndmPtr ) {
...
double MyHistory::weightFirstEmissions(PartonLevel* trial, double as0,
...
double MyHistory::hardFacScale(const Event& event) {
...
double MyHistory::hardRenScale(const Event& event) {
  // Declare output scale.
  double hardscale = 0.;
  // If scale should not be reset, done.
...
double MyHistory::doTrialShower( PartonLevel* trial, int type,
...
bool MyHistory::updateind(vector<int> & ind, int i, int N) {
  if ( i < 0 ) return false;
...
vector<double>
MyHistory::countEmissions(PartonLevel* trial, double maxscale,
...
double MyHistory::monteCarloPDFratios(int flav, double x, double maxScale,
...
bool MyHistory::onlyOrderedPaths() {
...
bool MyHistory::onlyAllowedPaths() {
...
bool MyHistory::registerPath(MyHistory & l, bool isOrdered,
...
vector<MyClustering> MyHistory::getAllClusterings( const Event& event) {

  vector<MyClustering> ret;
  vector<MyClustering> systems;
...
void MyHistory::attachClusterings (vector<MyClustering>& clus, int iEmt, int iRad,
  ...
  if ( !mergingHooksPtr->doWeakClustering() ) {
    ...
    clus.push_back( MyClustering(iEmt, iRad, iRec, iPartner, pT, &event[iRad],
      &event[iEmt], &event[iRec], name, 0, 0, 0, 0));
    ...
    clus.push_back( MyClustering(iEmt, iRad, iRec, iPartner, pT, &event[iRad],
      &event[iEmt], &event[iRec], name, radBeforeFlav, 0, 0, 0));
...
vector<MyClustering> MyHistory::getClusterings (int emt, int rad,
                      const Event& event ) {

  vector<MyClustering> clus;
    ...
    for ( int iName=0; iName < int(names.size()); ++iName) {
      ...
      for ( int i = 0; i < int(recsNow.size()); ++i ) {
        if ( allowedClustering( rad, emt, recsNow[i], recsNow[i], names[iName], event) ) {
          ...
          //if (names[iName].compare("fsr_qed_1->22&1_CS") == 0)
          //  pT = pTLund(event, emt, rad, recsNow[i], names[iName]);
          //if (names[iName].compare("fsr_qed_11->22&11_CS") == 0)
          //  pT = pTLund(event, emt, rad, recsNow[i], names[iName]);
...
pair<double,double> MyHistory::getProb(const MyClustering & SystemIn) {
  ...
  //bool isFSR = showers->timesPtr->isTimelike(state, rad, emt, rec, "");
  ...
  if (isFSR) {
    ...
    if (name.find("qed") != string::npos)
      coupling = pow2(state[rad].charge())
                * mergingHooksPtr->AlphaEM_FSR()->alphaEM(mu2Ren); 
    if (name.find("ew") != string::npos) {
      int flav = abs(SystemIn.flavRadBef);
      coupling = 4.*M_PI
  ...
  if (isISR) {
    ...
    if (name.find("qed") != string::npos)
      coupling = pow2(state[rad].charge())
               * mergingHooksPtr->AlphaEM_ISR()->alphaEM(mu2Ren); 
    if (name.find("ew") != string::npos) {
      int flav = abs(SystemIn.flavRadBef);
      coupling = 4.*M_PI
           * (pow2(coupSMPtr->rf( flav )) + pow2(coupSMPtr->lf( flav )))
...
double MyHistory::pdfForSudakov() {
...
double MyHistory::hardProcessME( const Event& event ) {
  ...
  if (isEW2to1(event)) {
    ...
    // qqbar -> Z. No interference with gamma included.
      ...
      string message="Warning in MyHistory::hardProcessME: Only Z/W are";
...
Event MyHistory::cluster( MyClustering & inSystem ) {
...
int MyHistory::getRadBeforeFlav(const int RadAfter, const int EmtAfter,
      const Event& event) {
...
int MyHistory::getRadBeforeSpin(const int radAfter, const int emtAfter,
...
bool MyHistory::connectRadiator( Particle& Radiator, const int RadType,
...
int MyHistory::FindCol(int col, int iExclude1, int iExclude2,
...
int MyHistory::FindParticle( const Particle& particle, const Event& event,
  bool checkStatus ) {
...
int MyHistory::getRadBeforeCol(const int rad, const int emt,
      const Event& event) {
...
int MyHistory::getRadBeforeAcol(const int rad, const int emt,
      const Event& event) {
...
int MyHistory::getColPartner(const int in, const Event& event) {
...
int MyHistory::getAcolPartner(const int in, const Event& event) {
...
vector<int> MyHistory::getReclusteredPartners(const int rad, const int emt,
  const Event& event) {
...
bool MyHistory::getColSinglet( const int flavType, const int iParton,
...
bool MyHistory::isColSinglet( const Event& event,
  vector<int> system ) {
...
bool MyHistory::isFlavSinglet( const Event& event,
...
bool MyHistory::allowedClustering( int rad, int emt, int rec, int partner,
...
bool MyHistory::hasConnections( int, int nIncIDs[], int nOutIDs[]) {
...
bool MyHistory::canConnectFlavs( map<int,int> nIncIDs, map<int,int> nOutIDs) {
...
bool MyHistory::isSinglett( int rad, int emt, int rec, const Event& event ) {
...
bool MyHistory::validEvent( const Event& event ) {
...
bool MyHistory::equalClustering( MyClustering clus1 , MyClustering clus2 ) {
...
double MyHistory::choseHardScale( const Event& event ) const {
...
int MyHistory::getCurrentFlav(const int side) const {
  int in = (side == 1) ? 3 : 4;
  return state[in].id();
}
...
double MyHistory::getCurrentX(const int side) const {
  int in = (side == 1) ? 3 : 4;
...
double MyHistory::getCurrentZ(const int rad,
...
double MyHistory::pTLund(const Event& event, int rad, int emt, int rec, 
  string name) {
...
int MyHistory::posChangedIncoming(const Event& event, bool before) {
...
double MyHistory::pdfFactor( const Event& event, const int type,
...
double MyHistory::integrand(int flav, double x, double scaleInt, double z) {
...
vector<int> MyHistory::posFlavCKM(int flav) {
...
bool MyHistory::checkFlavour(vector<int>& flavCounts, int flavRad,
...
bool MyHistory::isQCD2to2(const Event& event) {
...
// (Only enabled if W reclustering is used).
bool MyHistory::isEW2to1(const Event& event) {
...
bool MyHistory::isMassless2to2(const Event& event) {
...
bool MyHistory::isDIS2to2(const Event& event) {
...
void MyHistory::setSelectedChild() {
...
void MyHistory::setGoodChildren() {
...
void MyHistory::setProbabilities() {
  ...
  for (int i = 0;i < int(goodSisters.size());++i) {
    ...
    for (int j = 0;j < int(sisterNow->goodChildren.size());++j) {
      ...
      //// Do not include paths where first emission is above the shower starting
      //// scale.
      //if ( int(childNow->goodChildren.size()) == 0
      //  && hardStartScale(childNow->state) < childNow->clusterIn.pT())
      //  { cout << "    child " << j << " continue as first emission is above starting scale " << endl; continue; }

      //// Do not update denominator if the step childNow --> sisterNow was
      //// unordered.
      //if (childNow->clusterIn.pT() < sisterNow->clusterIn.pT())
      //  { cout << "    child " << j << " continue as emission was unordered " << endl; continue; }
      //
      //foundOrdered=true;
      ...
      if (hasPartonLevel) {
        isFSR = showers->timesPtr->isTimelike(sisterNow->state, rad, emt, rec, "");
        if (isFSR) stateVars = showers->timesPtr->getStateVariables(sisterNow->state,rad,emt,rec,"");
        else       stateVars = showers->spacePtr->getStateVariables(sisterNow->state,rad,emt,rec,"");
      } else if (hasShowers) {
        isFSR = fsr->isTimelike(sisterNow->state, rad, emt, rec, "");
        if (isFSR) stateVars = fsr->getStateVariables(sisterNow->state,rad,emt,rec,"");
...
class asFunction : public Function {

public:

  asFunction(AlphaStrong* asIn) : as(asIn) {};

  double f(double x, vector<double> = vector<double>())
    { return as->alphaS(x); } 
  double solveForZ(double tMin, double tMax, double c) {
   if (abs(tMax-tMin)/tMax < 1e-4) return tMax;
   return findRoot1D(tMin, tMax, c, vector<double>(), 100);
  }
...
void MyHistory::setEffectiveScales() {
  ...
  for (int i = 0;i < int(goodSisters.size());++i) {
    ...
    for (int j = 0;j < int(sisterNow->goodChildren.size());++j) {
      ...
      // Scale with correct coupling factor.
      double tnow(pow2(childNow->clusterIn.pT()));
      tmax = max(tmax,tnow);
      tmin = min(tmin,tnow);

      double coupling(1.);
      if (childNow->clusterIn.name().find("qcd") != string::npos)
        coupling = mergingHooksPtr->AlphaS_FSR()->alphaS(tnow); 
      if (childNow->clusterIn.name().find("qed") != string::npos)
        coupling = pow2(childNow->clusterIn.rad()->charge())
                 * mergingHooksPtr->AlphaEM_FSR()->alphaEM(tnow); 
      if (childNow->clusterIn.name().find("ew") != string::npos) {
      ...
      if (childNow->generation > 0 && tEffPrev > 0.0) {
        tmin = min(tmin,tEffPrev);
        tmax = max(tmax,tEffPrev);
        alphasOftEffPrev = mergingHooksPtr->AlphaS_FSR()->alphaS(tEffPrev);
        // Include correct power for effective alphaS.
        alphasOftEffPrev = pow(alphasOftEffPrev, childNow->generation);
      }
    ...
    asFunction as(mergingHooksPtr->AlphaS_FSR());
    // Note: For negative probabilities, it might be necessary to increase the
    // scale range for root finding.
    double as_tmin = as.f(tmin,vector<double>());
    double as_tmax = as.f(tmax,vector<double>());
    double headroom = 1.;
    bool failed = false;
    ...
    while ( tmin/headroom < tmax*headroom
      ...
      headroom *= 1.01;
      as_tmin = as.f(tmin/headroom,vector<double>());
      as_tmax = as.f(tmax*headroom,vector<double>());
    }
...
double MyHistory::getShowerPluginScale(const Event& event, int rad, int emt,
  int rec, string name, string key, double scalePythia) {
