...
public:
  ...
  // Copy constructor
  MyClustering( const MyClustering& inSystem ){
  ...
  MyClustering( int emtPosIn, int radPosIn, int recPosIn, int partnerIn,
...
public:
  ...
  double weightMOPS(PartonLevel* trial, AlphaStrong * asFSR,
    AlphaStrong * asISR, AlphaEM * aemFSR, AlphaEM * aemISR, double RN);
  ...
  void printMyHistory( const double RN );
...
private:
  ...
  void printScales() {
    int type = (!mother) ? 0
             ...
             : (!clusterIn.rad()->isFinal() &&  clusterIn.rec()->isFinal()) ? -1 : -2;
    cout << scientific << setprecision(6);
    cout << " size " << state.size() << " scale " << scale
    << " clusterIn " << clusterIn.pT() << " state.scale() " << state.scale()
    << " scaleEffective " << scaleEffective;
    if (type==-2) cout << "\t\t" << " II splitting emt=" << clusterIn.emt()->id() << endl;
    if (type==-1) cout << "\t\t" << " IF splitting emt=" << clusterIn.emt()->id() << endl;
    if (type== 1) cout << "\t\t" << " FI splitting emt=" << clusterIn.emt()->id() << endl;
    if (type== 2) cout << "\t\t" << " FF splitting emt=" << clusterIn.emt()->id() << endl;
  ...
  bool isAllowedOrderingPath( double maxscale);
  ...
  bool registerPath(MyHistory & l, bool isOrdered,
  ...
  vector<MyClustering> getAllClusterings( const Event& event);
  vector<MyClustering> getClusterings( int emt, int rad, const Event& event);
  // Function to attach (spin-dependent duplicates of) a clustering.
  void attachClusterings (vector<MyClustering>& clus, int iEmt, int iRad,
  ...
  pair <double,double> getProb(const MyClustering & SystemIn);
  ...
  // OUT clustered state
  Event cluster( MyClustering & inSystem);
  ...
  bool equalClustering( MyClustering clus1 , MyClustering clus2 );
...
private:
  ...
  // state.
  vector<MyHistory *> children;
  vector<MyHistory *> goodSisters;
...
public:
  ...
  // the (incremental) corresponding probability. This map is empty
  // unless this is the initial step (mother == 0).
  map<double,MyHistory *> goodBranches, badBranches;
...
private:
  ...
  double clusterProb, prodOfProbs, prodOfProbsFull;

  // The partons and scale of the last clustering.
  MyClustering clusterIn;
  ...
  MyHistory(const MyHistory &) {}

  // The assignment operator is private.
  MyHistory & operator=(const MyHistory &) {
    return *this;
  }
