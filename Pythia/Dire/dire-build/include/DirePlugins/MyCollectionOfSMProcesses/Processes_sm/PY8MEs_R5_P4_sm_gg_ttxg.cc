...
#include "PY8MEs_R5_P4_sm_gg_ttxg.h"
...
// Process: g g > t t~ g WEIGHTED<=3 @5

// Exception class
class PY8MEs_R5_P4_sm_gg_ttxgException : public exception
{
  virtual const char * what() const throw()
  {
    return "Exception in class 'PY8MEs_R5_P4_sm_gg_ttxg'."; 
  }
}
PY8MEs_R5_P4_sm_gg_ttxg_exception; 

// Required s-channel initialization
//int PY8MEs_R5_P4_sm_gg_ttxg::req_s_channels[nreq_s_channels] = {}; 

int PY8MEs_R5_P4_sm_gg_ttxg::helicities[ncomb][nexternal] = {{-1, -1, -1, -1,
...
void PY8MEs_R5_P4_sm_gg_ttxg::initColorConfigs() 
{
  color_configs = vector < vec_vec_int > (); 

  // Color flows of process Process: g g > t t~ g WEIGHTED<=3 @5
  color_configs.push_back(vec_vec_int()); 
  // JAMP #0
  int jamp00[2 * nexternal] = {1, 2, 2, 3, 1, 0, 0, 4, 4, 3}; 
  color_configs[0].push_back(vec_int(jamp00, jamp00 + (2 * nexternal))); 
  // JAMP #1
  int jamp01[2 * nexternal] = {1, 2, 4, 3, 1, 0, 0, 3, 4, 2}; 
  color_configs[0].push_back(vec_int(jamp01, jamp01 + (2 * nexternal))); 
  // JAMP #2
  int jamp02[2 * nexternal] = {3, 2, 1, 3, 1, 0, 0, 4, 4, 2}; 
  color_configs[0].push_back(vec_int(jamp02, jamp02 + (2 * nexternal))); 
  // JAMP #3
  int jamp03[2 * nexternal] = {4, 2, 1, 3, 1, 0, 0, 2, 4, 3}; 
  color_configs[0].push_back(vec_int(jamp03, jamp03 + (2 * nexternal))); 
  // JAMP #4
  int jamp04[2 * nexternal] = {4, 2, 2, 3, 1, 0, 0, 3, 4, 1}; 
  color_configs[0].push_back(vec_int(jamp04, jamp04 + (2 * nexternal))); 
  // JAMP #5
  int jamp05[2 * nexternal] = {3, 2, 4, 3, 1, 0, 0, 2, 4, 1}; 
  color_configs[0].push_back(vec_int(jamp05, jamp05 + (2 * nexternal))); 
}

//--------------------------------------------------------------------------
// Destructor.
PY8MEs_R5_P4_sm_gg_ttxg::~PY8MEs_R5_P4_sm_gg_ttxg() 
{
...
vector < vec_int > PY8MEs_R5_P4_sm_gg_ttxg::getHelicityConfigs(vector<int>
    permutation)
{
...
vector < vec_int > PY8MEs_R5_P4_sm_gg_ttxg::getColorConfigs(int
...
vector<int> PY8MEs_R5_P4_sm_gg_ttxg::getHelicityConfigForID(int hel_ID,
    vector<int> permutation)
{
  if (hel_ID < 0 || hel_ID >= ncomb)
  {
    cout <<  "Error in function 'getHelicityConfigForID' of class" << 
    " 'PY8MEs_R5_P4_sm_gg_ttxg': Specified helicity ID '" << 
    hel_ID <<  "' cannot be found." << endl; 
    throw PY8MEs_R5_P4_sm_gg_ttxg_exception; 
  }
  vector<int> chosenPerm; 
  if (permutation.size() == 0)
  {
...
int PY8MEs_R5_P4_sm_gg_ttxg::getHelicityIDForConfig(vector<int> hel_config,
    vector<int> permutation)
{
  vector<int> chosenPerm; 
  if (permutation.size() == 0)
  ...
  if (hel_config.size() > 0)
  {
    ...
    if (user_ihel == -1)
    {
      ...
      " 'PY8MEs_R5_P4_sm_gg_ttxg': Specified helicity" << 
...
vector<int> PY8MEs_R5_P4_sm_gg_ttxg::getColorConfigForID(int color_ID, int
    specify_proc_ID, vector<int> permutation)
{
  int chosenProcID = -1; 
  if (specify_proc_ID == -1)
  ...
  if (color_ID < 0 || color_ID >= int(color_configs[chosenProcID].size()))
  {
    ...
    " 'PY8MEs_R5_P4_sm_gg_ttxg': Specified color ID '" << 
...
int PY8MEs_R5_P4_sm_gg_ttxg::getColorIDForConfig(vector<int> color_config, int
    specify_proc_ID, vector<int> permutation)
{
  int chosenProcID = -1; 
  if (specify_proc_ID == -1)
  ...
  if (color_config.size() > 0)
  {
      ...
      for (int j = 0; j < (nexternal * 2); j++ )
      {
        ...
        else
        {
          ...
          if (color_linked_pos == -1)
          {
            ...
            " 'PY8MEs_R5_P4_sm_gg_ttxg': A color line could " << 
            " not be closed." << endl;
            return -2; 
            //throw PY8MEs_R5_P4_sm_gg_ttxg_exception; 
          }
    ...
    {
      cout <<  "Error in function 'getColorIDForConfig' of class" << 
      " 'PY8MEs_R5_P4_sm_gg_ttxg': Specified color" << 
      " configuration cannot be found." << endl; 
      return -2;
      //throw PY8MEs_R5_P4_sm_gg_ttxg_exception; 
    }
...
vector < vec_double > PY8MEs_R5_P4_sm_gg_ttxg::getAllResults(int
    specify_proc_ID)
{
  int chosenProcID = -1; 
  if (specify_proc_ID == -1)
...
double PY8MEs_R5_P4_sm_gg_ttxg::getResult(int helicity_ID, int color_ID, int
    specify_proc_ID)
{
...
pair < vector<int> , int > PY8MEs_R5_P4_sm_gg_ttxg::static_getPY8ME(vector<int>
    initial_pdgs, vector<int> final_pdgs, set<int> schannels)
{
  ...
  const int out_pdgs[nprocs][nexternal - ninitial] = {{6, -6, 21}}; 
...
//--------------------------------------------------------------------------
// Set momenta
void PY8MEs_R5_P4_sm_gg_ttxg::setMomenta(vector < vec_double > momenta_picked)
{
  if (momenta_picked.size() != nexternal)
  {
    cout <<  "Error in function 'setMomenta' of class" << 
    " 'PY8MEs_R5_P4_sm_gg_ttxg': Incorrect number of" << 
  ...
  for(int i = 0; i < nexternal; i++ )
  {
    ...
    {
      cout <<  "Error in function 'setMomenta' of class" << 
      " 'PY8MEs_R5_P4_sm_gg_ttxg': A momentum was specified" << 
...
void PY8MEs_R5_P4_sm_gg_ttxg::setColors(vector<int> colors_picked)
{
  ...
  if (colors_picked.size() != (2 * nexternal))
  {
    ...
    " 'PY8MEs_R5_P4_sm_gg_ttxg': Incorrect number" << 
...
void PY8MEs_R5_P4_sm_gg_ttxg::setHelicities(vector<int> helicities_picked) 
{
...
void PY8MEs_R5_P4_sm_gg_ttxg::setPermutation(vector<int> perm_picked) 
{
...
void PY8MEs_R5_P4_sm_gg_ttxg::setProcID(int procID_picked) 
{
...
void PY8MEs_R5_P4_sm_gg_ttxg::initProc() 
{
  // Initialize vectors.
...
double PY8MEs_R5_P4_sm_gg_ttxg::sigmaKin() 
{
...
void PY8MEs_R5_P4_sm_gg_ttxg::calculate_wavefunctions(const int hel[])
{
...
double PY8MEs_R5_P4_sm_gg_ttxg::matrix_5_gg_ttxg() 
{
  int i, j; 
  // Local variables
  ...
  // The color matrix;
  double denom[ncolor] = {9, 9, 9, 9, 9, 9}; 
  double cf[ncolor][ncolor] = {{64, -8, -8, 1, 1, 10}, {-8, 64, 1, 10, -8, 1},
      {-8, 1, 64, -8, 10, 1}, {1, 10, -8, 64, 1, -8}, {1, -8, 10, 1, 64, -8},
      {10, 1, 1, -8, -8, 64}};
