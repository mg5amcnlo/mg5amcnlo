...
c When this routine is called we are always singular:
      logical nonsingular
      parameter (nonsingular=.false.)
...
c               xzoomfact = 1d0
               if (nonsingular)then
                  fx = dsig(p,wgt,1d0) !Evaluate function
                  wgt = fx * wgt
               else
                  fx = dsig(p,wgt,1d0)
                  wgt = fx      ! Already multiplied by the wgt in dsig()
               endif
c               if (xzoomfact .gt. 0d0) then
c                  wgt = wgt*fx*xzoomfact
c               else
c                  wgt = -xzoomfact
c               endif
...
cRF         if (wgt .gt. 0d0) kevent=kevent+1    
...
c
      ...
      enddo
      tmean = tmean/tsigma
cRF      tsigma= tmean/sqrt(tsigma)
      tsigma= abs(tmean)/sqrt(tsigma)
c      nun = n_unwgted()
...
cRF         if (wgt .gt. 0.) then
cRF            if (wgt*iter*events .gt. wmax) then
cRF               wmax=wgt*iter*events
         if (wgt .ne. 0.) then
            if (abs(wgt)*iter*events .gt. wmax) then
               wmax=abs(wgt)*iter*events
            endif
            non_zero = non_zero + 1
            mean = mean + wgt
            if (.true. ) then
...
c                        if (j .eq. 1) then
     ...
     &                          real(non_zero)/real(inon_zero(i,j)),non_zero,inon_zero(i,j),grid(1,i,j)
c                        endif
                     endif
                     inon_zero(i,j) = 0
                  endif
...
c$$$                  do i=1,ng
c$$$                     if(j.eq.5)write (*,*) i,j,grid(2,i,j),grid(1,i,j),inon_zero(i,j)
c$$$                     inon_zero(i,j)=0
c$$$                  enddo
...
c
            if (tsigma .gt. 0d0 .and. cur_it .gt. 5 .and. accur .gt. 0d0) then
               ...
               xmean = tmean/tsigma
               xchi2 = max((chi2/xmean/xmean-tsigma)/dble(cur_it-2),1d0)
...
cRF               tsigma = tmean / sqrt(tsigma)
 ...
 80            format(/1X,79(1H-)/1X,23HAccumulated results:   ,
     .              10HIntegral =,e15.7/24X,10HStd dev  =,e15.7/
