...
data/numbers.txt                   the numeric report (g g only -- it is
                                   REGENERATED by plot_zz_loopinduced.py, so
                                   nothing hand-written survives in it)
qq_coefficients.py                 the q qbar half of the two-mechanism spin
                                   table: p p > e+ e- mu+ mu- [QCD] as truth and
                                   p p > z z [QCD] + MadSpin in four modes, read
                                   through THIS directory's observables.py so the
                                   two halves share one frame and one eta_l
data/numbers_qq.txt                its report, and the file spin_definitions.tex
data/numbers_qq.json               takes the q qbar rows from
...
That is a statement about **LO 2 -> 2 production**, not about MadSpin, and it
does not carry over to the `q qbar` side of `qq_coefficients.py`. There the
production is MC@NLO, and the modes that redraw the virtuality *do* reach below
`2 m_Z`: `madspin` down to 136.8 GeV with 0.15 % of its weight there, `PA` to
145.6 GeV with 0.22 %, while `onshell`, which changes no mass, still starts at
`2 m_Z` exactly. `SPIN_COEFFICIENTS.md` section 6b has the measurement.
