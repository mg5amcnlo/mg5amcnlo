...
events per sample except where a count is quoted, all moments \emph{unbinned},
i.e.\ weighted means over events with the ordinary error of the mean, and no
binning or within-bin reconstruction anywhere.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccccc}
    ...
    \quad at LO        & $0.1742\pm0.0012$ & $0.0618\pm0.0027$ & $\phantom{-}0.0315\pm0.0027$ & $0.7135\pm0.0034$ & $-0.628\pm0.035$ \\
  ...
  \caption{Spin coefficients of the two $ZZ$ production mechanisms, unbinned
    moments over $200\,000$ events per sample --- the LO reference row
    excepted, which is $800\,000$ --- on the common selection of the text.
    The ``at LO'' row is $pp\to e^+e^-\mu^+\mu^-$ with the same photon veto
    and the same selection as the NLO truth above it, one order down: a
    reference and not a third mechanism, which is why it carries no pull row.
    It was generated with no cut of any kind at the generator level and
    filtered to the common selection afterwards, which is the same selection
    because both cuts act on the reconstructed pairs; the only generator-level
    threshold it carries, a $40$~GeV floor on the same-flavour pairs, sits
    below the window's $54.567$~GeV edge and is inert ($13.4\,\%$ of the
    surviving events have $m(e^+\mu^-)<40$~GeV, against $12.9\,\%$ of the NLO
    truth's, so nothing was removed there either). $f_0$ is the per-event
    average of the two $Z$; on a single side the
...
Third, and this is what the LO reference row of Table~\ref{tab:spincoeff} is
for: the same comparison one order down --- $pp\to ZZ$ at LO through the same
\ms{} chain, against the LO four-lepton reference, on the same selection ---
agrees at $+0.6\sigma$, $f_0=0.1725\pm0.0024$ against $0.1742\pm0.0012$, so
whatever the $q\bar q$ $f_0$ tension is, it is not something the chain does
that survives at LO.
