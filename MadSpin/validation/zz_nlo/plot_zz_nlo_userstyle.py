#!/usr/bin/env python3
"""The NLO ``p p > z z`` MadSpin comparison in the user's personal style.

A second, independent rendering of the figures ``plot_zz_nlo.py`` draws in the
MG7 paper style.  The physics, the data and the numbers are identical; this
module shares the data loading, the ratio statistics, the structural-emptiness
test and the choice of reference with that one, and differs only in the drawing.

The conventions are the ones the sibling studies under ``MadSpin/validation/``
follow: stock rcParams (no usetex, sans serif), figsize (6, 6), a [3, 1]
gridspec, a plain step for the reference and ``errorbar(fmt='o', ms=4)`` plus a
faint companion step for everything else, a ratio panel clipped to a fixed
window rather than an autoscaled ladder, arrows for measured points that leave
the window, and open markers for exact structural zeros.

Usage::

    python3 plot_zz_nlo_userstyle.py [--data DIR] [--out DIR]
"""
...
from plot_zz_nlo import (                                        # noqa: E402
    Data, MODES, CURVES_PLAIN, reference, structural,
)
_LI = os.path.abspath(os.path.join(_HERE, '..', 'zz_loopinduced'))
if _LI not in sys.path:
    sys.path.insert(0, _LI)
from plot_zz_loopinduced import ratio, LOGY, RATIO_CLIP          # noqa: E402

OBS = OZ.leptonic()

# plot_zz_nlo imports the MG7-style module, which sets the paper rcParams
# (serif / usetex) at import time; they are reset here because the user's style
# sets no rcParams at all.
mpl.rcParams.update(mpl.rcParamsDefault)
matplotlib.use('Agg')

# The reset also turns ``text.usetex`` back OFF, which means these figures never
# go through matplotlib's usetex Type1 subsetting path and cannot be bitten by
# the minus-eating bug ``plot_zz_loopinduced._fix_type1_subset_minus`` works
# around.  Do NOT "check" these PDFs with ``check_minus``: a non-usetex PDF
# carries ``/minus`` whether or not the workaround ran, so the check would pass
# either way, which is worse than no check.  The discriminating one is
# ``plot_zz_nlo.py --check-minus`` on the MG7-style PDFs.
        ...
        for xi, yi in zip(x, np.where(gone, np.nan, r)):
            ...
            if yi > RATIO_CLIP[1] or yi < RATIO_CLIP[0]:
                ...
                rx.annotate('', xy=(xi, edge),
                            xytext=(xi, edge + off * (RATIO_CLIP[1]
                                                      - RATIO_CLIP[0])),
