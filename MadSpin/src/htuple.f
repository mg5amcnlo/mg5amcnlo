      subroutine ntuple(x,a,b,ii,mode)
C--------------------------------------------------------------
C    P.A. Go back to the htuple version, only one config. is necessary
C    for madspin. Introduce the variable 'mode':
c           mode=0: generate a random number
c           mode=1: write down k, accum
c           mode=2: read k, accum
C
...
c      include "genps.inc"
      ...
      integer lmaxconfigs, maxdim
      parameter (lmaxconfigs=1, maxdim=25)
...
c
      if (mode.eq.1) then
        open (unit=50,file='seeds.dat',status='unknown')
        write(50,*) accum
        write(50,*) k
        close(50)
...
c      jj = jconfig
      jj=1      !use jj=jconfig for multiconfiguraion mode
