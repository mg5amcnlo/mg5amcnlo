...
C     
C     Process: g u~ > e+ e- d u~ d~ WEIGHTED<=7 @1
C     Process: g u~ > e+ e- s u~ s~ WEIGHTED<=7 @1
C     Process: g u~ > mu+ mu- d u~ d~ WEIGHTED<=7 @1
C     Process: g u~ > mu+ mu- s u~ s~ WEIGHTED<=7 @1
C     Process: g c~ > e+ e- d c~ d~ WEIGHTED<=7 @1
C     Process: g c~ > e+ e- s c~ s~ WEIGHTED<=7 @1
C     Process: g c~ > mu+ mu- d c~ d~ WEIGHTED<=7 @1
C     Process: g c~ > mu+ mu- s c~ s~ WEIGHTED<=7 @1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C      where in initialization mode as well for the helicity.
     ...
     $  THEN
        DO I=1,NCOMB
          IF (GOODHEL(I,10) .OR. NTRY(10)
     $     .LE.MAXTRIES.OR.(ISUM_HEL.NE.0)) THEN
            T=MATRIX10(P ,NHEL(1,I),JC(1),I,AMP2, JAMP2, IVEC)
     ...
     $       .EQ.0.AND.ALLOW_HELICITY_GRID_ENTRIES) THEN
          ...
          ENDIF
        ENDDO
        IF(NTRY(10).EQ.(MAXTRIES+1).AND.DS_GET_DIM_STATUS('Helicity')
     $   .NE.-1) THEN
...
C            update.
     ...
     $              'IMIRROR', IMIRROR
                ENDIF
              ELSE IF (.NOT.GOODHEL(I,10) .AND. (DABS(TS(I)).GT.ANS
     $         *LIMHEL/NCOMB)) THEN
                GOODHEL(I,10)=.TRUE.
                NGOOD = NGOOD +1
                PRINT *,'Added good helicity ',I, 'for process 10'
     $           ,TS(I)*NCOMB/ANS,' in event ',NTRY(10)
              ENDIF
            ENDDO
          ENDIF
          IF(NTRY(10).EQ.MAXTRIES)THEN
...
C           Set right sign for ANS, based on sign of chosen helicity
 ...
 10     CONTINUE
     ...
     $       //' zero but not the total matrix-element'
      ...
      CALL SELECT_COLOR(RCOL, JAMP2, ICONFIG,10,  ICOL)

      END


      REAL*8 FUNCTION MATRIX10(P,NHEL,IC, IHEL,AMP2, JAMP2, IVEC)
C     
C     Generated by MadGraph5_aMC@NLO v. 3.7.1, 2026-04-10
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Amplitude(s) for diagram number 8
      ...
      CALL FFV1_1(W(1,2),W(1,1),GC_11(IVEC),ZERO, FK_ZERO,W(1,14))
      CALL FFV1P0_3(W(1,6),W(1,14),GC_11(IVEC),ZERO, FK_ZERO,W(1,15))
C     Amplitude(s) for diagram number 9
      CALL FFV1_0(W(1,7),W(1,11),W(1,15),GC_11(IVEC),AMP(9))
C     Amplitude(s) for diagram number 10
      CALL FFV1_0(W(1,12),W(1,5),W(1,15),GC_11(IVEC),AMP(10))
C     Amplitude(s) for diagram number 11
      CALL FFV1_0(W(1,7),W(1,4),W(1,15),GC_11(IVEC),AMP(11))
C     Amplitude(s) for diagram number 12
      CALL FFV1_0(W(1,3),W(1,5),W(1,15),GC_11(IVEC),AMP(12))
      CALL FFV1_1(W(1,14),W(1,9),GC_2,ZERO, FK_ZERO,W(1,15))
C     Amplitude(s) for diagram number 13
      CALL FFV1_0(W(1,6),W(1,15),W(1,10),GC_11(IVEC),AMP(13))
      CALL FFV1_1(W(1,14),W(1,10),GC_11(IVEC),ZERO, FK_ZERO,W(1,15))
C     Amplitude(s) for diagram number 14
      CALL FFV1_0(W(1,6),W(1,15),W(1,9),GC_2,AMP(14))
      CALL FFV2_5_1(W(1,14),W(1,13),-GC_50,GC_58,ZERO, FK_ZERO,W(1,8))
C     Amplitude(s) for diagram number 15
      CALL FFV1_0(W(1,6),W(1,8),W(1,10),GC_11(IVEC),AMP(15))
C     Amplitude(s) for diagram number 16
      CALL FFV2_5_0(W(1,6),W(1,15),W(1,13),-GC_50,GC_58,AMP(16))
      CALL FFV1_1(W(1,5),W(1,1),GC_11(IVEC),ZERO, FK_ZERO,W(1,15))
...
C     Amplitude(s) for diagram number 20
      ...
      CALL FFV1_2(W(1,6),W(1,9),GC_2,ZERO, FK_ZERO,W(1,15))
C     Amplitude(s) for diagram number 21
      CALL FFV1_0(W(1,15),W(1,2),W(1,14),GC_11(IVEC),AMP(21))
      CALL FFV1_1(W(1,2),W(1,9),GC_2,ZERO, FK_ZERO,W(1,16))
C     Amplitude(s) for diagram number 22
      CALL FFV1_0(W(1,6),W(1,16),W(1,14),GC_11(IVEC),AMP(22))
      CALL FFV2_5_2(W(1,6),W(1,13),-GC_50,GC_58,ZERO, FK_ZERO,W(1,17))
