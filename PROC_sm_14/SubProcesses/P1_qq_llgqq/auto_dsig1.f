...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u > e+ e- g u u
      PD(IPROC)=U1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u u > mu+ mu- g u u
      PD(IPROC)=U1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c > e+ e- g c c
      PD(IPROC)=C1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c > mu+ mu- g c c
      PD(IPROC)=C1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
...
C           LP=SIGN(1,LPP(IB(2)))
     ...
     $       ,DSQRT(ALL_Q2FACT(2, IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=U1(IVEC)*U2(IVEC)
      ...
      IPROC=IPROC+1  ! c c > e+ e- g c c
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=C1(IVEC)*C2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
