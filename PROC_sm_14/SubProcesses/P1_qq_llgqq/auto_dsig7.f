...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX7(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,7,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG7=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG7*WGT,7,SELECTED_HEL(1), SELECTED_COL(1), 1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG7_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT,
...
C     
      ...
      DOUBLE PRECISION C2(VECSIZE_MEMMAX)
      DOUBLE PRECISION XPQ(-7:7),PD(0:MAXPROC)
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG7(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(7,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(7) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(7)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX7_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C          1.0)
          DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,7, IVEC)
...
C           Call UNWGT to unweight and store events
            ...
            CALL UNWGT(ALL_PP(0,1,IVEC), ALL_OUT(IVEC)*ALL_WGT(IVEC),7
     $       , SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
      ...
      SUBROUTINE SMATRIX7_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX7(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
