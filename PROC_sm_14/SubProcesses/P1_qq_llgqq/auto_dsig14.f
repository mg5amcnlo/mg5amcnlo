...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d u~ > e+ e- g d u~
      PD(IPROC)=D1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d u~ > mu+ mu- g d u~
      PD(IPROC)=D1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d c~ > e+ e- g d c~
      PD(IPROC)=D1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d c~ > mu+ mu- g d c~
      PD(IPROC)=D1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s u~ > e+ e- g s u~
      PD(IPROC)=S1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s u~ > mu+ mu- g s u~
      PD(IPROC)=S1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s c~ > e+ e- g s c~
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s c~ > mu+ mu- g s c~
      PD(IPROC)=S1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG14 = PD(0)
...
C           Call UNWGT to unweight and store events
     ...
     $       ,14, SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
          ENDIF
        ENDDO
      ENDDO
