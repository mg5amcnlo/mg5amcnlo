...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ d~ > e+ e- g u~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ d~ > mu+ mu- g u~ d~
      PD(IPROC)=UX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ s~ > e+ e- g u~ s~
      PD(IPROC)=UX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u~ s~ > mu+ mu- g u~ s~
      PD(IPROC)=UX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ d~ > e+ e- g c~ d~
      PD(IPROC)=CX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ d~ > mu+ mu- g c~ d~
      PD(IPROC)=CX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > e+ e- g c~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ s~ > mu+ mu- g c~ s~
      PD(IPROC)=CX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG19 = PD(0)
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX19(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,19,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG19=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG19*WGT,19,SELECTED_HEL(1), SELECTED_COL(1),
     $    1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG19_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG19(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C           LP=SIGN(1,LPP(IB(2)))
     ...
     $       ,DSQRT(ALL_Q2FACT(2, IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=UX1(IVEC)*DX2(IVEC)
      ...
      IPROC=IPROC+1  ! u~ s~ > e+ e- g u~ s~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=UX1(IVEC)*SX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! c~ d~ > e+ e- g c~ d~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=CX1(IVEC)*DX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
      ...
      IPROC=IPROC+1  ! c~ s~ > e+ e- g c~ s~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=CX1(IVEC)*SX2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(19,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(19) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(19)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX19_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C          1.0)
          DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,19, IVEC)
...
C           Call UNWGT to unweight and store events
            ...
            CALL UNWGT(ALL_PP(0,1,IVEC), ALL_OUT(IVEC)*ALL_WGT(IVEC)
     $       ,19, SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
          ENDIF
        ENDDO
      ENDDO
      ...
      SUBROUTINE SMATRIX19_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX19(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
...
!$OMP END PARALLEL
      ...
      INTEGER FUNCTION GET_NHEL19(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL19 = NHEL(IPART, IABS(HEL))
      RETURN
      END
