def selection_23():
    ...
    pad.hist(x=xData, bins=xBinning, weights=y24_M_0_weights,\
    ...
    plt.xlabel(r"$M$ $[ l+_{1} l-_{1} p_{1} p_{2} p_{3} ]$ $(GeV/c^{2})$ ",\
               fontsize=16,color="black")
    plt.ylabel(r"$\mathrm{Events}$ $(\mathcal{L}_{\mathrm{int}} = 10\ \mathrm{fb}^{-1})$ ",\
               fontsize=16,color="black")

    # Boundary of y-axis
    ymax=(y24_M_0_weights).max()*1.1
    #ymin=0 # linear scale
    ymin=min([x for x in (y24_M_0_weights) if x])/100. # log scale
