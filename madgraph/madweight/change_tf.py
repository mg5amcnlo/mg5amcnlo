...
class TF_in_SubProcesses:
    ...
    def text_get_central_point(self):
        ...
        for i in range(0,len(self.blockname_list)):
            ...
            if blockname != -1: #particle is visible
                text+=' c_point(perm, {},1,1)=theta(pexp_init(0,2+perm_id({})))\n'.format(i+1,i-1)
                text+=' c_point(perm, {},2,1)=phi(pexp_init(0,2+perm_id({})))\n'.format(i+1,i-1)
            ...
            for j in range(1,4):
                if blockname == -1: #particle is invisible 
                    text+=' c_point(perm,{},{},2)=-1d0\n'.format(i+1,j)    
                elif blockname == 0: # in delta mode
                    text+=' c_point(perm,{},{},2)=0d0\n'.format(i+1,j)
                else: #visible particles with TF define
                    text+=' c_point(perm,{},{},2)=width_{}_{}(pexp_init(0,2+perm_id({})),tag_lhco({}))\n'.format(i+1,j,variable[j-1],blockname,i-1,i+1)
    ...
    def text_tf_E_for_part(self):
        ...
        for i in range(0,len(self.blockname_list)):
            ...
            if not isinstance(blockname, str):
                ...
                text+=' n_lhco=tag_lhco(%s)\n'% (i+1)
                text+=' call tf_E_{}(pexp(0,{}),momenta(0,{}),n_lhco,tf_E_for_part)\n'.format(blockname,i+1,i+1)
                text+='\n return\n endif\n'        
    ...
    def text_tf_E_for_one_part(self,i):
        ...
        if not isinstance(blockname, str):        
            ...
            text+=' tf_E_for_%s=1d0\n' %(i+1)
            text+=' n_lhco=tag_lhco(%s)\n'% (i+1)
            text+=' call tf_E_{}(pexp(0,{}),momenta(0,{}),n_lhco,tf_E_for_{})\n'.format(blockname,i+1,i+1,i+1)       
