...
class Cmd(CheckCmd, HelpCmd, CompleteCmd, BasicCmd):
    ...
    def ask(self, question, default, choices=[], path_msg=None, 
        ...
        if choices + path_msg:
            question += ' ['
            question += f"\033[{text_format}m{default}\033[0m, "    
    ...
    def write_configuration(self, filepath, basefile, basedir, to_keep):
        ...
        for line in open(basefile):
            ...
            text += f'{key} = {value} # {comment} \n'
        for key in to_write:
            if key in to_keep:
                text += f'{key} = {to_keep[key]} \n'
...
class ControlSwitch(SmartQuestion):
    ...
    def __init__(self, to_control, motherinstance, *args, **opts):
        ...
        for key in self.switch:
            allowed_args += [f'{key}={s};' for s in self.get_allowed(key)]
        # adding special mode
    ...
    def default(self, line, raise_error=False):
        ...
        if '=' in line:
            ...
            raise NotValidInput(f'unknow command: {line}. Did you mean \"{line[4:]}\"')
        elif raise_error:
        ...
        if hasattr(self, 'ans_%s' % base):
            ...
            raise NotValidInput(f'Not valid command: {line}. Did you mean \"{line[4:]}\"')
        elif raise_error:
    ...
    def check_consistency(self, key, value):
        ...
        if hasattr(self, 'consistency_%s' % key):
            ...
            for key2,value2 in self.switch.items():
                if hasattr(self, f'consistency_{key}_{key2}'):
                    rules[key2] = getattr(self, f'consistency_{key}_{key2}')(value, value2)
    ...
    def create_question(self, help_text=True):
        ...
        for key, descrip in self.to_control:
            ...
            if key in self.inconsistent_keys:
                to_display = f'{self.switch[key]} < {self.inconsistent_keys[key]}'
            else:
