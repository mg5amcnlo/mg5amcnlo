...
#                    ['%d: %s'%(i+1,ex) for i, ex in enumerate(example)]},
    ...
    def find_restrict_card(self, model_name, base_dir='./', no_restrict=True,
        ...
        if online and not local_model and model_name in self._online_model:
            output += ['{}-{}'.format(model_name, tag) for tag in self._online_model[model_name]]
            return output  
        ...
        for name in os.listdir(pjoin(base_dir, model_name)):
            if name.startswith('restrict_') and not name.endswith('default.dat') \
                ...
                while model_name.endswith(os.path.sep):
                    model_name = model_name[:-1]
                output.append('{}-{}'.format(model_name, tag))
            ...
            if args[1]=='pythia8':
                ...
                for prefix in ['--with', '--veto']:
                    for prog in ['fastjet', 'delphes', 'delphesMA5tune']:
                        options.append('{}_{}'.format(prefix, prog))
  ...
  > Loop corrections cannot be considered."""
    ...
    def add_model(self, args):
        ...
        if output_dir:
            ...
            output_dir = pjoin(MG5DIR, 'models', '{}__{}'.format(name,
        ...
        if 'modelname' in self.history.get('full_model_line'):
            ...
            opts='' 
        self.exec_cmd('import model {} {}'.format(new_model_name, opts), errorhandling=False, 
                              printcmd=False, precmd=True, postcmd=True)         
    ...
    def do_display(self, line, output=sys.stdout):
        ...
        if args[0] == 'particles' and len(args) == 1:
            ...
            for i, inter in enumerate(self._curr_model['interactions']):
                ...
                text += '\n'
                print("{} {} {}".format(name,  inter['orders'], inter['type'] if 'type' in inter else 'tree'))
            if text =='':
                ...
                for value in item:
                    if hasattr(value, 'expr'):
                        if value.value is not None:
                            text+= '        {} = {} = {}\n'.format(value.name, value.expr ,value.value)
                        else:
                            text+= '        {} = {}\n'.format(value.name, value.expr)
                    else:
                        if value.value is not None:
        ...
        while order_re:
            ...
            if name.endswith('^2'):
                ...
                if basename not in list(model_orders) + ['WEIGHTED']:
                    ...
                    raise self.InvalidCmd("model order {} not valid for this model (valid one are: {}). Please correct".format(name, ', '.join(valid))) 
            ...
            line = '{} {}'.format(order_re.group('before'),order_re.group('after')) 
            order_re = order_pattern.match(line)   
...
#                misc.sprint(part_name)
                ...
                while True:
                    try:
                        ...
                        if no_dup_name in self._multiparticles:
                            spins = {self._curr_model.get_particle(p).get('spin') for p in self._multiparticles[no_dup_name]}
                            mass = {self._curr_model.get_particle(p).get('mass') for p in self._multiparticles[no_dup_name]}
        ...
        if ',' in procline:
            ...
            for one_decay in all_decays:
                ...
                if nb_level:
                    ...
                    final = '{},{}'.format(tmp_decay, one_decay)
                    final = final.strip()
    ...
    def extract_decay_chain_process(self, line, level_down=False, proc_number=0):
        ...
        while index_comma > -1:
            ...
            if line.lstrip()[0] == '(' and index_par !=-1 and \
                ...
                par_start = line.find('(')
                line = '{} {}'.format(line[par_start+1:index_par], line[index_par+1:]) 
                index_par = line.find(')')
            if line.lstrip()[0] == '(':
    ...
    def add_default_multiparticles(self):
        ...
        for part in self._curr_model.get('particles'):
            line.append('{} {}'.format(part.get('name'), part.get('antiname')))
        line = 'all =' + ' '.join(line)
        self.do_define(line)
...
#            logger.info('{:^80}'.format("-"*70), '$MG:BOLD')
            logger.info("   You are installing '{}', please cite ref(s): \033[92m{}\033[0m. ".format(args[0], ', '.join(advertisements[args[0]])), '$MG:BOLD')
...
################################################################################
        ...
        if args[0] not in path:
            if not source:
                ...
                return self.do_install('{} --source={}'.format(' '.join(args), othersource), 
            ...
            if self.options['lhapdf']:
                ...
                if self.options['lhapdf'] != 'lhapdf-config':
                    if misc.which('lhapdf-config') != os.path.realpath(self.options['lhapdf']):
                        os.environ['PATH'] = '{}:{}'.format(os.path.realpath(self.options['lhapdf']),os.environ['PATH']) 
            else:
        ...
        if name in plugin:
            ...
            try:
                ...
                print(error)
                if six.PY2:
                    raise Exception('Plugin {} fail to be loaded. Please contact the author of the PLUGIN\n Error {}'.format(name, error))
                else:
            ...
            logger.info('Plugin {} correctly interfaced. Latest official validition for MG5aMC version {}.'.format(name, '.'.join(repr(i) for i in latest_validated_version)))
            if new_interface:
...
'''.format(name,'' if pyvers == 2 else pyvers)                     
    ...
    def install_update(self, args, wget):
        ...
        if web_version == data['version_nb']:
            ...
            logger_stderr.info('impossible to update: local {} web {}'.format(data['version_nb'], web_version))
            fsock = open(os.path.join(MG5DIR,'input','.autoupdate'),'w')
            fsock.write("version_nb   %s\n" % data['version_nb'])
            fsock.write("last_check   %s\n" % int(time.time()))
            fsock.write("last_message   %s\n" % data['last_message'])
            ...
            if key in ['pythia8_path', 'hwpp_path', 'thepeg_path', 'hepmc_path',
                ...
                if isinstance(self.options[key],str) and self.options[key].lower() == 'auto':
                    ...
                    if program != None:
                        ...
                        logger.info('Using {} library in {}'.format(key,fpath))
...
in the MG5aMC option 'samurai' (instead of leaving it to its default 'auto').""")
                ...
                try:
                    ...
                    if key in self.options_madgraph:
                        self.history.append('set {} {}'.format(key, self.options[key]))
    ...
    def do_launch(self, line):
        ...
        current_options = {name: self.options[name] for name in self.options_madgraph}
        start_cwd = os.getcwd()
        ...
        if args[0] == 'ignore_six_quark_processes':
            ...
            if able_to_mod:
                ...
                if 'modelname' in self.history.get('full_model_line'):
                    ...
                    opts=''
                MadGraphCmd.do_import(self,'model {} {}'.format(model_name, opts), force=True)
            elif log:
            ...
            if args[1] in ['None',"''",'""']:
                ...
                if program!=None:
                    res = 0
                    logger.info('set {} to {}'.format(args[0],args[1]))
                    self.options[args[0]] = args[1]
                else:
                    res = 1
    
                if res != 0 :
                    logger.warning('{} does not seem to correspond to a valid {} lib '.format(args[1],args[0]) + \
...
#        line_options = dict(arg[2:].split('=') for arg in args if arg.startswith('--') and '=' not in arg)
        ...
        if self.options['group_subprocesses'] in [True, False]:
            ...
            if self._curr_amps[0].get_ninitial() == 1 and \
                ...
                if len({proc.get('id') for proc in processes})!=len(processes):
...
""")
    ...
    def do_help(self, line):
        ...
        if len(self.history) == 0:
            ...
            last_action = args[0]
            if len(args)>1:
                last_action_2 = '{} {}'.format(last_action, args[1])
            else:
                last_action_2 = 'none'
    ...
    only valid in narrow-width approximation and at tree-level."""
        ...
        self.do_decay_diagram('{} {}'.format(' '.join([repr(id) for id in particles]),
            ...
            with misc.MuteLogger(['madgraph','ALOHA','cmdprint','madevent'], [40,40,40,40]):
                ...
                for name, val in self.options.items():
                    if name in me_cmd.options and me_cmd.options[name] != val:
                        try:
                            me_cmd.exec_cmd('set {} {} --no_save'.format(name, val)) 
