...
#            raise self.InvalidCmd, 'The UFO model does not include partial widths information. Impossible to compute widths automatically'
        ...
        particles_name = {p.get('name'): p.get('pdg_code')
                                               for p in model.get('particles')}
        particles_name.update({p.get('antiname'): p.get('pdg_code')
                                               for p in model.get('particles')})        
...
class CommonRunCmd(HelpToCmd, CheckValidForCmd, cmd.Cmd):
                    ...
                    if pid == 'unknown' or misc.pid_exists(pid):
                        ...
                        if crashifpresent:
                            if isinstance(crashifpresent, Exception):
                                ...
                                message = '''Another instance of the program is currently running (pid = {}).
                (for this exact same directory). Please wait that this is instance is 
                closed. If no instance is running, you can delete the file
                {} and try again.'''.format(pid, pjoin(me_dir, 'RunWeb'))
                                raise AlreadyRunning(message)
                        elif warnifpresent:
            ...
            if amcatnlo and run_card['pdlabel']=='lhapdf':
                ...
                for lhaid in run_card['lhaid']:
                    if lhaid in pdfsets:
                        ...
                        raise MadGraph5Error("lhaid {} is not a valid PDF identification number. This can be due to the use of an outdated version of LHAPDF, or {} is not a LHAGlue number corresponding to a central PDF set (but rather one of the error sets).".format(lhaid,lhaid))
    ...
    def create_plot(self, mode='parton', event_path=None, output=None, tag=None):
        ...
        if mode not in ['parton', 'reweight']:
            plot_dir = pjoin(self.me_dir, 'HTML', self.run_name,'plots_{}_{}'.format(mode.lower(),tag))
        elif mode == 'parton':
...
#        logger.info("       244800@X : runs over the Xth set of the associated error set")
    ...
    def do_systematics(self, line):
        ...
        if nb_submit in [0,1]:
            ...
            for i in range(nb_submit):
                ...
                output_files = ['./tmp_{}_{}'.format(i, os.path.basename(output)),
                                './log_sys_%s.txt' % (i)]
                argument = []
                if not __debug__:
                    argument.append('-O')
            ...
            subprocess.call(['cat']+\
                            ['./tmp_{}_{}'.format(i, os.path.basename(output)) for i in range(nb_submit)],
        ...
        if not self.force_run:
            ...
            if self.options['nb_core']==1 or self.run_card['nevents'] < 101 or not check_multicore(self):
                ...
                for i in range(nb_file):
                    ...
                    if to_zip:
                        new_command.append('{}_{}.lhe'.format(new_args[0],i))
                        all_lhe.append('{}_{}.lhe'.format(new_args[0],i))
                    else:
                        new_command.append('{}_{}.lhe'.format(new_args[0][:-3],i))
                        all_lhe.append('{}_{}.lhe'.format(new_args[0][:-3],i))
                ...
                if any(os.path.exists('{}_{}_debug.log'.format(f, self.run_tag)) for f in all_lhe):
                    for f in all_lhe:
                        if os.path.exists('{}_{}_debug.log'.format(f, self.run_tag)):
                            raise Exception("Some of the run failed: Please read {}_{}_debug.log".format(f, self.run_tag)) 
                ...
                for key in cross_sections:
                    if key == 'orig' or key.isdigit():
                        continue
                    logger.info('{} : {} pb'.format(key, cross_sections[key]))
                return
    ...
    def do_pgs(self, line):
        ...
        banner_path = pjoin(self.me_dir, 'Events', self.run_name, '{}_{}_banner.txt'.format(self.run_name, self.run_tag))
        ...
        if eradir and misc.is_executable(pjoin(eradir, 'ExRootLHCOlympicsConverter')):
            ...
            try:
                misc.call([eradir+'/ExRootLHCOlympicsConverter',
                             'pgs_events.lhco',pjoin('{}/{}_pgs_events.root'.format(self.run_name, tag))],
                            cwd=pjoin(self.me_dir, 'Events'))
            except Exception:
...
#                logger.info('No rivet_card detected, so not running Rivet')
        ...
        for analysis in analysis_list:
            if analysis.startswith("MC_"):
                analysis =  "{}:ENERGY={}".format(analysis, rivet_config["rivet_sqrts"])
            run_analysis = f"{run_analysis},{analysis}"
        ...
        if "$CONTUR_" in run_analysis:
            ...
            p = subprocess.Popen('source {} &>/dev/null; echo $CONTUR_USER_DIR'.format(pjoin(self.options['contur_path'], "conturenv.sh"))
            ...
            set_env = set_env + "{}=`echo {} | sed 's/,ATLAS_2016_I1469071//'`\n".format(run_analysis.replace("$",""), run_analysis)
        ...
        wrapper.write(f"#!{misc.which(shell)}\n{set_env}\n")
        wrapper.write(sys.executable + " {} &> {}\n".format(run_rivet, pjoin(self.me_dir, 'Events', self.run_name, "rivet.log")))
        if rivet_config['draw_rivet_plots']:
            draw_rivet = "{} {} -o {}".format(pjoin(rivet_path, "bin", "rivet-mkhtml"), pjoin(self.me_dir, 'Events', self.run_name, "rivet_result.yoda"), pjoin(self.me_dir, 'Events', self.run_name, 'rivet-plots'))
            wrapper.write(sys.executable + " " + draw_rivet + " &> {}".format(pjoin(self.me_dir, 'Events', self.run_name, "rivet-plots.log")))
            logger.info("Rivet plots will be stored in {}".format(pjoin(self.me_dir, 'Events', self.run_name, 'rivet-plots')))
#            rivet_config["run_rivet_later"] =True
        ...
        os.system("chmod +x {}".format(pjoin(self.me_dir, 'Events', self.run_name, "run_rivet.sh")))
        ...
        if postprocess: #inside postprocessing functions, no need to run, just need to return rivet configurations
            ...
            if postprocess_RIVET:
                ...
                logger.info(f"Running Rivet with {hepmc_file}")
...
#       Here's how to print the MA5 commands generated by MG5aMC
    ...
    def do_delphes(self, line):
        ...
        if os.path.exists(pjoin(self.me_dir, 'Source', 'banner_header.txt')):
            ...
            self.banner.write(pjoin(self.me_dir, 'Events', self.run_name, '{}_{}_banner.txt'.format(self.run_name, tag)))
        ...
        # True/False formatting
            ...
            if args[1] in ['None','True','False']:
                ...
                if os.path.exists(args[1]):
                    ...
                    raise self.InvalidCmd('Not a valid path: keep previous value: \'{}\' for {} instead of {}'.format(self.options[args[0]], args[0], args[1]) )
            else:
                self.options[args[0]] = args[1]
    ...
    def static_compute_widths(line, interface, path=None):
        ...
        if hasattr(interface, 'do_compute_width'):
            interface.do_compute_widths('{} --path={}'.format(line, path))
            ...
            line = 'compute_widths {} --path={}'.format(line, path)
    ...
    def add_error_log_in_html(self, errortype=None):
        ...
        if hasattr(self, 'results') and hasattr(self.results, 'current') and\
            ...
            self.debug_output = pjoin(self.me_dir, '{}_{}_debug.log'.format(name,tag))
            if errortype:
    ...
    def set_configuration(self, config_path=None, final=True, initdir=None, amcatnlo=False):
        ...
        for key in self.options:
            ...
            if key.endswith('path') and not key.startswith("cluster"):
                ...
                try:
                    self.do_set("{} {} --no_save".format(key, self.options[key]), log=False)
                except self.InvalidCmd:
    ...
    def update_make_opts_full(path, def_variables, keep_old=True):
        ...
        content_variables = '\n'.join('{}={}'.format(k,v) for k, v in variables.items() if v is not None)
        content_variables += '\n%s' % tag
        ...
        if lhapdf_version.startswith('5.'):
            ...
            pdfsets_lines = \
                    [l for l in open(indexfile).read().split('\n') if l.strip() and \
                        not '90cl' in l]
            lhapdf_pdfsets = { int(l.split()[0]): {'lhaid': int(l.split()[0]),
...
#     try:                
        ...
        ### PARAM_CARD WITH BLOCK NAME -----------------------------------------
            ...
            if key in self.param_card[args[start]].param_dict:
                ...
                logger.warning('invalid information: {} not defined in the param_card'.format(key))
        # PARAM_CARD NO BLOCK NAME ---------------------------------------------
            ...
            for bname, lhaid in all_var:
                new_line = 'param_card {} {} {}'.format(bname,
...
#         print error
    ...
    def do_set(self, line):
        ...
        if args[0] in self.special_shortcut:
            ...
            if len(args) != len(targettypes) +1:
                logger.warning('shortcut {} requires {} argument'.format(args[0], len(targettypes)))
                if len(args) < len(targettypes) +1:
                    return
                else:
        ...
        if args[0].lower() == 'showerkt':
            ...
            if os.path.exists(pythia_path):
                ...
                p_card, n = re.subn(r'''^\s*SHOWERKT\s*=\s*[default\de\+\-\.]*\s*$''',
                                    ...
                                    p_card, flags=(re.M+re.I))
                if n==0:
                    p_card = '{} \n SHOWERKT= {}'.format(p_card, args[1].upper())
                with open(pythia_path, 'w') as fsock:
                    fsock.write(p_card)
                return
        ...
        # PARAM_CARD NO BLOCK NAME ---------------------------------------------
            ...
            if len(all_var) > 1:
                ...
                for bname, lhaid in all_var:
                    logger.warning('   {} {}'.format(bname, ' '.join([str(i) for i in lhaid])))
    ...
    def setM(self, block, name, value):
        ...
        if block not in self.mw_card:
            ...
            logger.info('name {} was not present in the block {} for the current MadWeight card. We are adding it'.format(name,block),'$MG:BOLD')
        if value == 'default':
            import madgraph.madweight.Cards as mwcards
    ...
    def setR(self, name, value):
        ...
        logger.info('modify parameter {} of the run_card.dat to {}'.format(name, new_value),'$MG:BOLD')        
    ...
    def setML(self, name, value, default=False):
        ...
        try:
            ...
            return
        logger.info('modify parameter {} of the MadLoopParam.dat to {}'.format(name, value),'$MG:BOLD')
    ...
    def setPY8(self, name, value, default=False):
        try:
            ...
            logger.warning("Fail to change parameter. Please Retry. Reason: %s." % error)
            return
        logger.info('modify parameter {} of the pythia8_card.dat to {}'.format(name, value), '$MG:BOLD')
    ...
    def setRivet(self, name, value, default):
        try:
            ...
            logger.warning("Fail to change parameter. Please Retry. Reason: %s." % error)
            return
        logger.info('modify parameter {} of the rivet_card.dat to {}'.format(name, value), '$MG:BOLD')
        ...
        def check_block(self, blockname):
            ...
            for key in block.keys():
                if key not in input_in_block:
                    param = block.get(key)
                    if blockname != 'decay':
                        text.append('\t{}\t{} # {}\n'.format(' \t'.join([repr(i) for i in param.lhacode]), param.value, param.comment))
                    else: 
                        text.append('DECAY \t{}\t{} # {}\n'.format(' \t'.join([repr(i) for i in param.lhacode]), param.value, param.comment))
                    add_entry += 1
            if add_entry:
    ...
    def do_add(self, line):
        ...
        if len(args) == 3 and args[0] in ['pythia8_card', 'pythia8_card.dat'] and self.has_PY8:
            ...
            self.PY8Card.write(pjoin(self.me_dir,'Cards','pythia8_card.dat'),
                          pjoin(self.me_dir,'Cards','pythia8_card_default.dat'),
                          print_only_visible=True)
            logger.info("add in the pythia8_card the parameter \"{}\" with value \"{}\"".format(name, value), '$MG:BOLD')
        elif len(args) > 0:
            ...
            if args[1] == '--clean':
                ...
                for posline,l in enumerate(split):
                    ...
                    if new_line.startswith(('--before_line=','--after_line')):
                        return self.do_add('{} {}'.format(args[0], new_line))   
    ...
    def copy_file(self, path, pathname=None):
        ...
        if card_name != 'banner':
            logger.info('copy {} as {}'.format(pathname, card_name))
