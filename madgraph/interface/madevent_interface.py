        ...
        if postprocess_CONTUR:
            ...
            if nb_rivet == 1:
                ...
                if rivet_config["weight_name"] == "None":
                    ...
                    contur_cmd = 'contur --nomultip -g scan --wn "{}" >> contur.log 2>&1\n'.format(rivet_config["weight_name"] + contur_add)
   ...
   Please check/correct your param_card and/or your run_card.'''
    ...
    def do_multi_run(self, line):
        ...
        for i in range(nb_run):
            self.nb_refine = 0
            self.exec_cmd('generate_events {}_{} -f'.format(main_name, i), postcmd=False)
...
#           helicity filters.
    ...
    def do_refine(self, line):
        ...
        if not self.history or 'refine' not in self.history[-1]:
            ...
            if  cross == 0:
                return
            logger.info("- Current estimate of cross-section: {} +- {}".format(cross, error))
    ...
    def do_combine_events(self, line):
        ...
        self.banner.write(pjoin(self.me_dir, 'Events', self.run_name, 
                                '{}_{}_banner.txt'.format(self.run_name, tag)))
...
"""
        ...
        if int(self.run_card['ickkw']):
            ...
            if 'MGGenerationInfo' in self.banner:
                ...
                self.banner['MGGenerationInfo'] = '#  Matched Integrated weight (pb)  :  %s\n' % self.results.current['cross_pythia']
        banner_path = pjoin(self.me_dir, 'Events', self.run_name, '{}_{}_banner.txt'.format(self.run_name, tag))
    ...
    def do_remove(self, line):
        ...
        if 'banner' in mode:
            ...
            if tag:
                # remove banner
                try:
                    os.remove(pjoin(self.me_dir, 'Events',run,'{}_{}_banner.txt'.format(run,tag)))
                except Exception:
    ...
    def launch_job(self,exe, cwd=None, stdout=None, argument = [], remaining=0, 
        ...
        if mode == 0:
            ...
            logger.info('{} run in {:f} s'.format(exe, time.time() -start))
            if status:
            ...
            if 'ajob' in exename: 
                ...
                if suffix == '_0':
                    suffix = ''
                output_files = ['G{}{}'.format(i, suffix) for i in argument[1:]]
                for G in output_files:
                    required_output.append('%s/results.dat' % G)
    ...
    def run_syscalc(self, mode='parton', event_path=None, output=None):
        ...
        if not event_path:
            if mode == 'parton':
                ...
                stdout = open(pjoin(event_dir, self.run_name, '{}_{}_syscalc.log'.format(tag,mode)),'w')
