...
class ShowerCard(dict):
    ...
    def set_param(self, key, value, write_to = ''):
        ...
        if key in self.logical_vars:
            ...
            raise ShowerCardError('Unknown entry: {} = {}'.format(key, value))
        self.keylist.append(key)

        #then update self.text and write the new card
        if write_to:
            logger.info('modify parameter {} of the shower_card.dat to {}'.format(key, value))
            ...
            for line in self.text.split('\n'):
                ...
                if key_match and not ( str(key).upper().startswith('DM') ):
                    ...
                    if key not in self.logical_vars:
                        newlines.append('{} = {} #{}'.format(key, value, comment))
                    else:
            ...
            if str(key).upper().startswith('DM') and not value.lower() in ['','none','default']:
                newlines.append('{} = {}'.format(str(key).upper(), value[0:len(value)]))
    ...
    def write_card(self, shower, card_path):
        ...
        for key in self.keylist:
            ...
            if key in self.logical_vars:
                ...
                if key == 'analyse':
                    if value is None or not value:
                        try:
                            ...
                            pass
                    try:
                        line = '{}="{}"'.format(self.names_dict[key][self.shower].upper(), value)
                        lines.append(line)
                        continue
                    except KeyError:
                        continue
                ...
                raise ShowerCardError('Unknown key: {} = {}'.format(key, value))
            try:
                line = '{}={}'.format(self.names_dict[key][self.shower].upper(), value.upper())
                lines.append(line)
            except KeyError:
                pass
