...
class Particle:
    ...
    def __repr__(self):
        return 'Particle("{}", event={})'.format(str(self), self.event)
...
#                    "unweighting is %s yielding %s events." % (max_wgt,nb_keep))
        ...
        for i, event in enumerate(self):
            if (not (partition is None) and i==sum(partition[:nb_file+1])) or \
                ...
                if not partition is None and (nb_file+1>len(partition)):
                    return nb_file
                if zip:
                    current = EventFile(pjoin(cwd,'{}_{}.lhe.gz'.format(self.path, nb_file)),'w')
                else:
                    current = open(pjoin(cwd,'{}_{}.lhe'.format(self.path, nb_file)),'w')                    
                ...
                for h in hwu:
                    if isinstance(value, tuple):
                        ...
                        if self.keep_wgt:
                            event.parse_reweight()
                            if self.keep_wgt is True:
                                data = {'{}_{}'.format(name, key):event.reweight_data[key]
                                                    for key in event.reweight_data}
    ...
    def create_syscalc_data(self, out_path, pythia_input=None):
        ...
        for i, orig_event in enumerate(self):
            ...
            if smin:
                new_event.syscalc_data['matchscale'] = "{} {} {}".format(smin, scomp, smax)
...
<totfact> 0.10344054E+04</totfact>
    ...
    def add_decay_to_particle(self, position, decay_event):
        ...
        assert max(decay_particle.px, decay_particle.py, decay_particle.pz) < thres,\
            "not on rest particle {} {} {} {}".format(decay_particle.E, decay_particle.px,decay_particle.py,decay_particle.pz) 
    ...
    def check(self):
        ...
        for particle in self:
            ...
            if particle.mass and (abs(particle.mass) - fourmass)/ abs(particle.mass) > threshold:
                raise Exception( "Do not have correct mass lhe: {} momentum: {} (error at {}".format(particle.mass, fourmass, (abs(particle.mass) - fourmass)/ abs(particle.mass)))
                

        if E/absE > threshold:
            logger.critical(self)
            raise Exception("Do not conserve Energy {}, {}".format(E/absE, E))
        if px/abspx > threshold:
            logger.critical(self)
            raise Exception("Do not conserve Px {}, {}".format(px/abspx, px))         
        if py/abspy > threshold:
            logger.critical(self)
            raise Exception("Do not conserve Py {}, {}".format(py/abspy, py))
        if pz/abspz > threshold:
            logger.critical(self)
            raise Exception("Do not conserve Pz {}, {}".format(pz/abspz, pz))
        ...
        for particle in self:
            ...
            if mcolors != []:
                ...
                if len(canticolors) + len(mcolors) != 3:
                    ...
                    raise Exception("Wrong color flow for {} -> {}".format([m.pid for m in mothers], [c.pid for c in childs]))              
                else:
...
""" 
        ...
        if self.eventflag:
            event_flag = ' %s' % ' '.join('{}="{}"'.format(k,v) for (k,v) in self.eventflag.items())
        else:
            event_flag = ''
        ...
        if self.reweight_data:
            ...
            reweight_str = '<rwgt>\n%s\n</rwgt>' % '\n'.join(
                        '<wgt id=\'{}\'> {:+13.7e} </wgt>'.format(i, float(self.reweight_data[i]))
        ...
        if self.matched_scale_data:
            tmp_scale = ' '.join(['pt_clust_%i=\"%s\"' % (i+1,v)
                                   for i,v in enumerate(self.matched_scale_data)
                                              if v!=-1])
            if tmp_scale:
                tag_str = "<scales {}></scales>{}".format(tmp_scale, self.tag)
...
class FourMomentum:
    ...
    def __repr__(self):
        return 'FourMomentum({},{},{},{})'.format(self.E, self.px, self.py,self.pz)
...
#0.929944817736d-03 0.000000000000d+00 0.000000000000d+00  5 21 3 -11 11 3 2 0.12112732d-02 0.45047393d-01 0.58606724d+03 0.58606724d+03 0.58606724d+03  2 11  3 0.835109616010d+02
        ...
        assert sum(len(c) for c in get_weights_for_momenta.values()) == int(nb_wgt), "{} != {}".format(sum(len(c) for c in get_weights_for_momenta.values()), nb_wgt)
...
if '__main__' == __name__:   
    ...
    if False: 
        ...
        for event in lhe:
            ...
            e_out = Particle(line="   11 1 1 2 0 0  {} {} {} {} {} 0 9 ".format(peout.px, peout.py, peout.pz, peout.E, peout.mass))
            e_in.event = event
            #e_in.event_id = 0
            e_out.event = event
            #e_out.event_id = 2
