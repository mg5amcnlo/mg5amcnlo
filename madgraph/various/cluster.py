...
# check that file exists and is not empty.
        ...
        if time_check == 0:
            logger.debug(f'''Job {job_id}: missing output:{path}''')
                ...
                try:
                    ...
                    # function. for CPU intensive fct this will slow down the computation
                        ...
                        if returncode != 0:
                            ...
                            self.stoprequest.set()
                            self.remove(f"fct {exe} does not return 0:\n {returncode}")
                except Exception as error:
...
class PBSCluster(Cluster):
    ...
    def submit(self, prog, argument=[], cwd=None, stdout=None, stderr=None, log=None,
        ...
        if len(self.submitted_ids) > self.maximum_submited_jobs:
            fct = lambda idle, run, finish: logger.info(f'Waiting for free slot: {idle} {run} {finish}')
