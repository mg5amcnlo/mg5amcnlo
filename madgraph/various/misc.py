...
def get_scan_name(first, last):
    ...
    if end:
        name = "{}[{}-{}]{}".format(base, first[len(base):-len(end)], last[len(base):-len(end)],end)
    else:
        name = "{}[{}-{}]{}".format(base, first[len(base):], last[len(base):],end)
    return name
...
def gunzip(path, keep=False, stdout=None):
    ...
    if os.path.getsize(path) > 1e8:
        if stdout:
            os.system('gunzip -c {} > {}'.format(path, stdout))
        else:
...
class open_file:
    ...
    def find_valid(possibility, program='program'):
        ...
        for p in possibility:
            if which(p):
                logger.info('Using default {} \"{}\". '.format(program, p) + \
    ...
    def open_mac_program(self, program, file_path):
        ...
        if not program:
            ...
            # not shell program
            os.system('open -a {} {}'.format(program, file_path))
...
def sprint(*args, **opt):
    ...
    if not use_print:
        log.log(level, ' '.join([intro]+[str(a) for a in args]) + \
                   ' \033[1;30m[{} at line {}]\033[0m'.format(os.path.basename(filename), lineno))
    else:
            ...
            if os.path.exists(pjoin(plugpath, plug, '__init__.py')):
                try:
                    with stdchannel_redirected(sys.stdout, os.devnull):
                        __import__('{}.{}'.format(plugindirname,plug))
                except Exception as error:
                    if warning:
                        logger.warning("error detected in plugin: %s.", plug)
                        logger.warning("%s", error)
                    continue
                plugin = sys.modules['{}.{}'.format(plugindirname,plug)] 
                if hasattr(plugin, target_type):
                    ...
                    if keyname is None:
                        ...
                        if keyname in getattr(plugin, target_type):
                            if not info:
                                logger.info('Using from plugin {} mode {}'.format(plug, keyname), '$MG:BOLD')
                            else:
