...
def recover_banner(results_object, level, run=None, tag=None):
    ...
    if tag:        
        banner_path = pjoin(path,'Events',run,f'{run}_{tag}_banner.txt')
    else:
...
class ConfigFile(dict):
    ...
    def __setitem__(self, name, value, change_userdefine=False,raiseerror=False):
        ...
        if change_userdefine and lower_name in self.system_only:
            text=f'{name} is a private entry which can not be modify by the user. Keep value at {self[name]}'
        ...
        if lower_name in self.list_parameter:
            ...
            logger.debug(f'Trying to add argument {name} in {self.__class__.__name__}. ' +\
    ...
    def add_param(self, name, value, system=False, comment=False, typelist=None,
        ...
        if __debug__:
            if lower_name in self:
                raise Exception(f"Duplicate case for {name} in {self.__class__}")
    ...
    def do_help(self, name):
        ...
        out = f"## Information on parameter {name} from class {self.__class__.__name__}\n"
        if name.lower() in self:
            out += f"## current value: {self[name]} (parameter should be of type {type(self[name])})\n"
        ...
        if (six.PY2 and not isinstance(value, (str,str)) or (six.PY3 and  not isinstance(value, str))):
            ...
            if targettype == UnknownType:
                ...
                if value.lower() in ['0', '.false.', 'f', 'false', 'off']:
                    ...
                    raise InvalidCmd(f"{repr(value)} can not be mapped to True/False for {name}")
            elif targettype == str:
                if value.startswith('\'') and value.endswith('\''):
                    value = value[1:-1]
                ...
                if value.isdigit():
                    ...
                    try:
                        ...
                        if value == new_value:
                            ...
                            raise InvalidCmd(f"incorect input: {value} need an integer for {name}")
    ...
    def write(self, output_file, template=None):
        ...
        for line in open(template):
            ...
            if len(nline) != 2:
                ...
                text += f'{nline[0]}\t= {self[nline[0].strip()]} {comment}\n'
            else:
    ...
    def write(self, output_file, template=None):
        ...
        for line in open(template):                  
            ...
            if len(nline) != 2:
                ...
                text += f'  {self[nline[1].strip()]}\t= {nline[1]} {comment}'        
            else:
                ...
                if python_template and nline[0].strip().startswith('$'):
                    ...
                    if python_template:
                        ...
                        if not comment or comment[-1]!='\n':
                            ...
                            endline = ''
                        text += f'  {value}\t= {name} {comment}{endline}'
                        written.add(name)                        
            ...
            for key in self.includepath[incname]:                
                ...
                if isinstance(value, list):
                    ...
                    if targettype is bool:
                        ...
                        line = f'{fortran_name}({0}) = {self.f77_formatting(len(value))} \n'
                        fsock.writelines(line)
                    elif targettype is float:
                        line = f'{fortran_name}({0}) = {self.f77_formatting(float(len(value)))} \n'
                        fsock.writelines(line)
                    # output the rest of the list in fortran
                    for i,v in enumerate(value):
                        line = f'{fortran_name}({i+1}) = {self.f77_formatting(v)} \n'
                        fsock.writelines(line)
                elif isinstance(value, dict):
                    for fortran_name, onevalue in value.items():
                        line = f'{fortran_name} = {self.f77_formatting(onevalue)} \n'
                        fsock.writelines(line)                       
                elif isinstance(incname,str) and 'compile' in incname:
                    line = f'{fortran_name} = {value} \n'
                    fsock.write(line)
                else:
                    line = f'{fortran_name} = {self.f77_formatting(value)} \n'
                    fsock.writelines(line)
            if not output_file:
                fsock.close()
        ...
        for i in [1,2]:
            ...
            if self[lpp] == 0: # nopdf
                ...
                if self[pdlabelX] in ['eva', 'iww', 'edff','chff','none']:
                    raise InvalidRunCard(f"{lpp} \'{self[lpp]}\' not compatible with {pdlabelX} \'{self[pdlabelX]}\'")
            elif abs(self[lpp]) in [3,4]: # PDF from PDF library
                if self[pdlabelX] not in ['none','eva', 'iww'] + sum(self.allowed_lep_densities.values(),[]):
                    logger.warning(f"{lpp} \'{self[lpp]}\' not compatible with {pdlabelX} \'{self[pdlabelX]}\'. Change {pdlabelX} to eva")
                    self.set(pdlabelX, 'eva')
                    mod = True
            elif abs(self[lpp]) == 2:
                if self[pdlabelX] not in ['none','chff','edff']:
                    logger.warning(f"{lpp} \'{self[lpp]}\' not compatible with {pdlabelX} \'{self[pdlabelX]}\'. Change {pdlabelX} to edff")
            ...
            for pdg in self['pdg_cut']:
                for var in ['pt','e','eta', 'Mxx']:
                    for minmax in ['min', 'max']:
                        if var in ['Mxx'] and minmax =='max':
                            continue
                        new_var = f'{var}{minmax}4pdg'
                        old_var = f'{var}_{minmax}_pdg'
                ...
                if 'default' in self[old_var]:
                    ...
                    if str(pdg) not in self[old_var]:
                        raise Exception(f"no default value defined for {old_var} and no value defined for pdg {pdg}") 
...
class eMELA_info(ConfigFile): 
    ...
    def log_and_update(self, banner, card, par, v):
        ...
        """
        logger.info(f' Setting {par} = {str(v)} in the {card}')
        if card != 'param_card':
