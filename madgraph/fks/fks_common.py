...
def link_rb_conf(born_amp, real_amp, i, j, ij): #test written, sm and heft
    """finds the real diagrams that match the born ones, i.e.  for
    each born diagrams, the real diagrams that has the ij -> i j
    splitting. It ignores diagrams without all 3-point interactions.
    i, j and ij are integers, and refer to the leg position in the
    real process (i, j) and in the born process (ij).
    """
...
def link_rb_configs(born_amp, real_amp, i, j, ij):
    ...
    for each born configuration, the real configuration that has the
    ij -> i j splitting. This is different from the ling_rb_conf,
    because that one links diagrams, while this one links
    configurations.  i, j and ij are integers, and refer to the leg
