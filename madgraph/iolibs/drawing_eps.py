...
class EpsDiagramDrawer(draw.DiagramDrawer):
    ...
    def line_format(self, x1, y1, x2, y2, name):
        ...
        if self.april_fool:
            ...
            d = random.random()
            e = random.random()
            return f"{c} {d} {e} setrgbcolor\n {x1} {y1} {x2} {y2} {name} \n"
        else:
            return f"{x1} {y1} {x2} {y2} {name} \n"
    def draw_vertex(self, vertex, bypass = ['QED','QCD']  ):
        ...
        if interaction:
            ...
            if order:
                ...
                self.text += f" {x1} {y1} {self.blob_size} 1.0 Fblob \n"
...
#                                          'yielding a tadpole is not supported')            
    ...
    def draw_curved_curly(self, line, cercle, type=''):
        ...
        self.text += self.line_format(line.end.pos_x,
                        line.end.pos_y, line.begin.pos_x,
                        line.begin.pos_y, f'0 {-1*curvature} Fgluonl{type}')
    ...
    def put_diagram_number(self, number=0):
        ...
        #write the text
        self.text += f' {x}  {y} moveto \n'
        
        if hasattr(self, 'diagram_type'):
            self.text += f'({self.diagram_type} diagram {number + 1} )   show\n' # +1 python
                                                            #starts to count at
                                                            #zero.
        else:
...
#                                          'yielding a tadpole is not supported')            
        ...
        #write EPS code
        self.text += f' {x_pos}  {y_pos} moveto \n'
        self.text += '(' + name + ')   show\n'
...
class MultiEpsDiagramDrawer(EpsDiagramDrawer):
    ...
    def draw(self, diagramlist='', opt=None):
        ...
        self.text += f' (page {self.curr_page + 1}/{self.npage}) show\n'
    ...
    def pass_to_next_page(self):
        ...
        self.text += 'showpage\n'
        self.text += '%%' + f'Page: {self.curr_page+1} {self.curr_page+1} \n'
