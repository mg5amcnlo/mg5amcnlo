      SUBROUTINE SMATRIX6(P,ANS_SUMMED)
C     
...
C     look for orders which match the nlo order constraint 
      ...
      IF (FIRSTTIME) THEN
        DO I = 1, NSQAMPSO
          DO J = 1, NSPLITORDERS
            IF(GETORDPOWFROMINDEX6(J, I) .GT. NLO_ORDERS(J)) THEN
              KEEP_ORDER(I) = .FALSE.
              EXIT
            ENDIF
          ENDDO
          IF (KEEP_ORDER(I)) THEN
            WRITE(*,*) 'REAL 6: keeping split order ', I
          ELSE
            WRITE(*,*) 'REAL 6: not keeping split order ', I
          ENDIF
        ENDDO
        FIRSTTIME = .FALSE.
      ENDIF

      CALL SMATRIX6_SPLITORDERS(P,ANS)
      ANS_SUMMED = 0D0
      ANS_MAX = 0D0
...
C          different coupling combinations
          DO J = 1, NSPLITORDERS
            AMP_ORDERS(J) = GETORDPOWFROMINDEX6(J, I)
          ENDDO
...
C     avoid fake non-zeros
      ...
      SUBROUTINE SMATRIX6_SPLITORDERS(P,ANS)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C         add to the sum of helicities
      ...
      SUBROUTINE MATRIX_6(P,NHEL,RES)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     
      ...
      INTEGER FUNCTION SQSOINDEX6(AMPORDERA,AMPORDERB)
C     
...
C     FUNCTION
C     
      INTEGER SQSOINDEX_FROM_ORDERS6
C     
C     BEGIN CODE
C     
      DO I=1,NSPLITORDERS
     ...
     $   +AMPSPLITORDERS(AMPORDERB,I)
      ENDDO
      SQSOINDEX6=SQSOINDEX_FROM_ORDERS6(SQORDERS)
      END



      INTEGER FUNCTION SQSOINDEX_FROM_ORDERS6(ORDERS)
C     
...
C     
 ...
 1009   CONTINUE
     ...
     $ ,NSPLITORDERS)
      ...
      INTEGER FUNCTION GETORDPOWFROMINDEX6(IORDER, INDX)
C     
...
C     
      IF (IORDER.GT.NSPLITORDERS.OR.IORDER.LT.1) THEN
        WRITE(*,*) 'INVALID IORDER 6', IORDER
        WRITE(*,*) 'SHOULD BE BETWEEN 1 AND ', NSPLITORDERS
        STOP
      ENDIF

      IF (INDX.GT.NSQAMPSO.OR.INDX.LT.1) THEN
        WRITE(*,*) 'INVALID INDX 6', INDX
        WRITE(*,*) 'SHOULD BE BETWEEN 1 AND ', NSQAMPSO
        STOP
      ENDIF

      GETORDPOWFROMINDEX6=SQSPLITORDERS(INDX, IORDER)
      END



      SUBROUTINE GET_NSQSO_REAL6(NSQSO)
C     
