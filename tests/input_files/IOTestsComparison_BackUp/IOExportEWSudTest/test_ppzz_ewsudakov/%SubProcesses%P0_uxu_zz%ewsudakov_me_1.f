      SUBROUTINE EWSDK_ME_1(P,NHEL,ANS_SUMMED)
C     
...
C     and
C     Process: u~ u > a z [ LOonly = QCD QED ] QCD^2=2 QED^2=4
C     
C     
C     CONSTANTS
C     
...
C         endif
      ...
      CALL EWSUDAKOV_ME_1_SPLITORDERS(P,NHEL,ANS)
      ANS_SUMMED = (0D0,0D0)
      MAX_VAL = 0D0
...
C     this is to avoid fake non-zero contributions 
      ...
      SUBROUTINE EWSUDAKOV_ME_1_SPLITORDERS(P,NHEL,ANS)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Reshuffle the momenta here
      P_SAVE(:,:) = P(:,:)
      PASS_RESHUFFLE = .TRUE.
      CALL RESHUFFLE_MOMENTA(P,P_RESH,(/3,0/),(/23,0/),(/22,0/)
     $ ,PASS_RESHUFFLE)
      P(:,:)=P_RESH(:,:)
...
C     Helas calls and color basis for the Sudakov amplitude
      ...
      CALL VXXXXX(P(0,4),MDL_MZ,NHEL(4),+1*IC(4),W(1,4))
      CALL FFV1_1(W(1,1),W(1,3),GC_2,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 1
      CALL FFV2_5_0(W(1,2),W(1,5),W(1,4),-GC_68,GC_76,AMP2(1))
      CALL FFV2_5_1(W(1,1),W(1,4),-GC_68,GC_76,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(1,2),W(1,5),W(1,3),GC_2,AMP2(2))
C     JAMPs contributing to orders QCD=0 QED=2
