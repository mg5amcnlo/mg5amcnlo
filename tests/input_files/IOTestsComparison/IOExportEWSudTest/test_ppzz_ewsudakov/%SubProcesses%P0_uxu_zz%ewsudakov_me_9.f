      SUBROUTINE EWSDK_ME_9(P,NHEL,ANS_SUMMED)
C     
...
C     Process: u~ u > z z [ LOonly = QCD QED ] QCD^2=2 QED^2=4
C     and
C     Process: u~ u > w- w+ [ LOonly = QCD QED ] QCD^2=2 QED^2=4
C     
C     
C     CONSTANTS
C     
...
C         write(*,*) 'SDK9: keeping split order ', i
C         else
C         write(*,*) 'SDK9: not keeping split order ', i
C         endif
        ENDDO
      ...
      CALL EWSUDAKOV_ME_9_SPLITORDERS(P,NHEL,ANS)
      ANS_SUMMED = (0D0,0D0)
      MAX_VAL = 0D0
...
C     this is to avoid fake non-zero contributions 
      ...
      SUBROUTINE EWSUDAKOV_ME_9_SPLITORDERS(P,NHEL,ANS)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     
      ...
      PARAMETER ( NGRAPHS1=2, NGRAPHS2=3)
...
C     Helas calls and color basis for the base amplitude
      ...
      CALL FFV2_5_1(W(1,1),W(1,3),-GC_68,GC_76,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 1
      CALL FFV2_5_0(W(1,2),W(1,5),W(1,4),-GC_68,GC_76,AMP1(1))
      CALL FFV2_5_1(W(1,1),W(1,4),-GC_68,GC_76,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL FFV2_5_0(W(1,2),W(1,5),W(1,3),-GC_68,GC_76,AMP1(2))
...
C     Reshuffle the momenta here
      P_SAVE(:,:) = P(:,:)
      PASS_RESHUFFLE = .TRUE.
      CALL RESHUFFLE_MOMENTA(P,P_RESH,(/3,4/),(/23,23/),(/-24,24/)
     $ ,PASS_RESHUFFLE)
      P(:,:)=P_RESH(:,:)
...
C     Helas calls and color basis for the Sudakov amplitude
      ...
      CALL FFV1P0_3(W(1,2),W(1,1),GC_2,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 1
      CALL VVV1_0(W(1,5),W(1,3),W(1,4),-GC_3,AMP2(1))
      CALL FFV2_5P0_3(W(1,2),W(1,1),-GC_68,GC_76,MDL_MZ,MDL_WZ,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL VVV1_0(W(1,3),W(1,4),W(1,5),-GC_70,AMP2(2))
      CALL FFV2_1(W(1,1),W(1,3),GC_124,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 3
      CALL FFV2_0(W(1,2),W(1,5),W(1,4),GC_124,AMP2(3))
C     JAMPs contributing to orders QCD=0 QED=2
      JAMP2(1,1) = (-1.000000000000000D+00)*AMP2(1)+(
     $ -1.000000000000000D+00)*AMP2(2)+(-1.000000000000000D+00)*AMP2(3)
