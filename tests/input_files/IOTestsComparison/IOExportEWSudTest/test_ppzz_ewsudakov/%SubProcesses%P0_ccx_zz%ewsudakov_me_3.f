      SUBROUTINE EWSDK_ME_3(P,NHEL,ANS_SUMMED)
C     
...
C         write(*,*) 'SDK3: keeping split order ', i
C         else
C         write(*,*) 'SDK3: not keeping split order ', i
C         endif
        ENDDO
      ...
      CALL EWSUDAKOV_ME_3_SPLITORDERS(P,NHEL,ANS)
      ANS_SUMMED = (0D0,0D0)
      MAX_VAL = 0D0
...
C     this is to avoid fake non-zero contributions 
      ...
      SUBROUTINE EWSUDAKOV_ME_3_SPLITORDERS(P,NHEL,ANS)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     Reshuffle the momenta here
      P_SAVE(:,:) = P(:,:)
      PASS_RESHUFFLE = .TRUE.
      CALL RESHUFFLE_MOMENTA(P,P_RESH,(/1,2/),(/4,-4/),(/3,-3/)
     $ ,PASS_RESHUFFLE)
      P(:,:)=P_RESH(:,:)
...
C     Amplitude(s) for diagram number 1
      CALL FFV2_3_0(W(5),W(2),W(4),GC_68,GC_76,AMP2(1))
      CALL FFV2_3_2(W(1),W(4),GC_68,GC_76,ZERO,ZERO,W(5))
C     Amplitude(s) for diagram number 2
      CALL FFV2_3_0(W(5),W(2),W(3),GC_68,GC_76,AMP2(2))
C     JAMPs contributing to orders QCD=0 QED=2
