...
C     and
C     Process: t~ t > t t~ [ LOonly = QCD QED ] QCD^2=6 QED^2=0
C     
C     
C     CONSTANTS
C     
...
C     
      ...
      PARAMETER ( NGRAPHS1=1, NGRAPHS2=2)
...
C     Reshuffle the momenta here
      P_SAVE(:,:) = P(:,:)
      PASS_RESHUFFLE = .TRUE.
      CALL RESHUFFLE_MOMENTA(P,P_RESH,(/1,2/),(/-5,5/),(/-6,6/)
     $ ,PASS_RESHUFFLE)
      P(:,:)=P_RESH(:,:)
...
C     Helas calls and color basis for the Sudakov amplitude
      CALL OXXXXX(P(0,1),MDL_MT,NHEL(1),-1*IC(1),W(1,1))
...
C     Amplitude(s) for diagram number 1
      ...
      CALL FFV1P0_3(W(1,4),W(1,1),GC_11,ZERO,ZERO,W(1,5))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(1,2),W(1,3),W(1,5),GC_11,AMP2(2))
C     JAMPs contributing to orders QCD=2 QED=0
      JAMP2(1,1) = (-1.666666666666667D-01)*AMP2(1)+(
     $ -5.000000000000000D-01)*AMP2(2)
      JAMP2(2,1) = (5.000000000000000D-01)*AMP2(1)+(1.666666666666667D
     $ -01)*AMP2(2)
