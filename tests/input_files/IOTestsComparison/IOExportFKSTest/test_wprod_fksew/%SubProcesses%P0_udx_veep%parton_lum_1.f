...
C         
     ...
     $ =EE_COMP_PROD(U1_COMPONENTS,DX2_COMPONENTS)
      IPROC=IPROC+1  ! c s~ > ve e+ a
      PD(IPROC) = C1*SX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(C1_COMPONENTS,SX2_COMPONENTS)

      PD2(0) = 0D0
      IPROS = 0

      IPROS=IPROS+1  ! u d~ > ve e+ a
      PD2(IPROS) = DX2

      IPROS=IPROS+1  ! c s~ > ve e+ a
      PD2(IPROS) = SX2

      PD1(0) = 0D0
      IPROSS = 0

      IPROSS=IPROSS+1  ! u d~ > ve e+ a
      PD1(IPROSS) = U1

      IPROSS=IPROSS+1  ! c s~ > ve e+ a
      PD1(IPROSS) = C1
      DO I=1,IPROC
