...
C              to reset here.
          ...
          ENDIF
        ENDIF
        CALL SETDELTAIR_CLL(0.0D0,(PI**2)/6.0D0)
...
C        the CTMODE 2
C       by using coefficients shifted by COLLIER internal error
C        estimate.
        IF (CTMODE.EQ.1) THEN
     ...
     $         +COEFMAP_THREE(I)
              DO K=1,3
                TIRCOEFS(I,K)=TIR_COEFS_DIRECT_MODE(I,K,ID)*(1.0D0
     $           +TIR_COEFS_ERROR(CURR_RANK,ID))
              ENDDO
            ENDDO
            RETURN
          ENDIF
...
C     Now compute the IR poles if asked for
      IF (COLLIERCOMPUTEIRPOLES) THEN
        IF(COLLIER_CACHE_ACTIVE.EQ.1) THEN
          ...
          ENDIF
        ENDIF
        CALL SETDELTAIR_CLL(1.0D0,(PI**2)/6.0D0)
        IF (NLOOPLINE.NE.1) THEN
     ...
     $     , RANK, TNTENERR_IR1)
        ...
        ENDIF
        CALL SETDELTAIR_CLL(0.0D0,1.0D0+(PI**2)/6.0D0)
        IF (NLOOPLINE.NE.1) THEN
...
C       Finish by caching the coefficients and error just computed
     ...
     $       +COEFMAP_THREE(I)
            DO J=1,3
              TIRCOEFS(I,J) = TIRCOEFS(I,J) * (1.0D0
     $         +TNTENERR(CURR_RANK))
            ENDDO
          ENDDO
        ENDIF
