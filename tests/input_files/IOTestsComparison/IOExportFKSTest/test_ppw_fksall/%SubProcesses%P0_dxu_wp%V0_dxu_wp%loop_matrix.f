...
C      DIFFERENT EVALUATION METHODS IN ORDER TO ASSESS STABILITY.
     ...
     $ /NSQSOXNLG*.FALSE./
      ...
      COMMON/RUNTIME_OPTIONS/AUTOMATIC_TIR_CACHE_CLEARING
...
C       Setup the file paths
 ...
 104    CONTINUE
     ...
     $   //'e ColorNumFactors.dat. File not found'
 105    CONTINUE
        CLOSE(1)
 ...
 106    CONTINUE
        STOP 'Color factors could not be initialized from fil'
     $   //'e ColorDenomFactors.dat. File not found'
 107    CONTINUE
        CLOSE(1)
 ...
 108    CONTINUE
     ...
     $   //'d from file HelConfigs.dat. File not found'
 109    CONTINUE
        CLOSE(1)
...
C       CutTools call for loop numbers 1
        CALL LOOP_3(1,2,3,DCMPLX(ZERO),DCMPLX(ZERO),DCMPLX(ZERO),2
     $   ,I_SO,1)
C       At this point, all reductions needed for (QCD=2), i.e. of
...
C         DOUBLE CHECK THE HELICITY FILTER
          IF (GOODHEL(HELPICKED).EQ.-HELOFFSET) THEN
            IF (.NOT.ISZERO(DABS(HELSAVED(1,HELPICKED))+DABS(HELSAVED(2
     $       ,HELPICKED))+DABS(HELSAVED(2,HELPICKED)),REF/DBLE(NCOMB),
     $       -1,-1)) THEN
 ...
 348          CONTINUE
          ...
          IF (GOODHEL(HELPICKED).LT.-HELOFFSET.AND.NTRY.NE.0) THEN
            IF(ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1,ABS(GOODHEL(HELP
     $       ICKED)+HELOFFSET)),REF,.TRUE.).EQ.0) THEN
              WRITE(*,*) '##W15 Helicity filter could not be successfu'
     $         //'lly double checked.'
              WRITE(*,*) '##One reason for this is that you might hav'
...
C     
 ...
 1009   CONTINUE
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5SOINDEX_FOR_SQUARED_ORD'
     $ //'ERS'
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5GET_SQUARED_ORDERS_FOR_'
     $ //'SOINDEX'
...
C     
      ...
      WRITE(*,*) 'ERROR:: Stopping function ML5GET_ORDERS_FOR_AMPSOIND'
     $ //'EX'
...
C     
      ...
      SUBROUTINE SET_AUTOMATIC_TIR_CACHE_CLEARING(ONOFF)
C     
