      SUBROUTINE DLUM_1(LUM)
C     ****************************************************            
C         
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
C     RETURNS PARTON LUMINOSITIES FOR MADFKS                          
C        
C     
C     Process: d d~ > t t~ a [ real = QCD QED ] QCD^2<=4 QED^2<=2
C     Process: s s~ > t t~ a [ real = QCD QED ] QCD^2<=4 QED^2<=2
C     
C     ****************************************************            
C         
      IMPLICIT NONE
C     
C     CONSTANTS                                                       
C         
C     
      INCLUDE 'genps.inc'
      INCLUDE 'nexternal.inc'
      DOUBLE PRECISION       CONV
      PARAMETER (CONV=389379660D0)  !CONV TO PICOBARNS             
C     
C     ARGUMENTS                                                       
C         
C     
      DOUBLE PRECISION LUM
C     
C     LOCAL VARIABLES                                                 
C         
C     
      INTEGER I, ICROSS,LP
      DOUBLE PRECISION D1,S1
      DOUBLE PRECISION SX2,DX2
C     
C     EXTERNAL FUNCTIONS                                              
C         
C     
      DOUBLE PRECISION PDG2PDF
C     
C     GLOBAL VARIABLES                                                
C         
C     
      INTEGER              IPROC
      DOUBLE PRECISION PD(0:MAXPROC)
      COMMON /SUBPROC/ PD, IPROC

      INTEGER             IPROS, IPROSS
      DOUBLE PRECISION PD1(0:MAXPROC), PD2(0:MAXPROC)
      COMMON /PDFVALUES/ PD1, PD2

      INCLUDE 'coupl.inc'
      INCLUDE 'run.inc'
      INTEGER IMIRROR
      COMMON/CMIRROR/IMIRROR
C     
C     STUFF FOR DRESSED EE COLLISIONS
C     
      INCLUDE 'eepdf.inc'
      DOUBLE PRECISION EE_COMP_PROD
      DOUBLE PRECISION DUMMY_COMPONENTS(N_EE)
      DOUBLE PRECISION D1_COMPONENTS(N_EE),S1_COMPONENTS(N_EE)
      DOUBLE PRECISION SX2_COMPONENTS(N_EE),DX2_COMPONENTS(N_EE)

      INTEGER I_EE
      INCLUDE '../../Source/PDF/pdf.inc'
C     
C     DATA                                                            
C         
C     
      DATA D1,S1/2*1D0/
      DATA SX2,DX2/2*1D0/
      DATA ICROSS/1/
C     ----------                                                      
C         
C     BEGIN CODE                                                      
C         
C     ----------                                                      
C         
      LUM = 0D0
      IF (ABS(LPP(1)) .GE. 1) THEN
        D1=PDG2PDF(LPP(1),1,1,XBK(1),DSQRT(Q2FACT(1)))
        IF ((ABS(LPP(1)).EQ.4.OR.ABS(LPP(1)).EQ.3)
     $   .AND.PDLABEL.NE.'none') D1_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
        S1=PDG2PDF(LPP(1),3,1,XBK(1),DSQRT(Q2FACT(1)))
        IF ((ABS(LPP(1)).EQ.4.OR.ABS(LPP(1)).EQ.3)
     $   .AND.PDLABEL.NE.'none') S1_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      IF (ABS(LPP(2)) .GE. 1) THEN
        SX2=PDG2PDF(LPP(2),-3,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') SX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
        DX2=PDG2PDF(LPP(2),-1,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') DX2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d d~ > t t~ a
      PD(IPROC) = D1*DX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(D1_COMPONENTS,DX2_COMPONENTS)
      IPROC=IPROC+1  ! s s~ > t t~ a
      PD(IPROC) = S1*SX2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(S1_COMPONENTS,SX2_COMPONENTS)

      PD2(0) = 0D0
      IPROS = 0

      IPROS=IPROS+1  ! d d~ > t t~ a
      PD2(IPROS) = DX2

      IPROS=IPROS+1  ! s s~ > t t~ a
      PD2(IPROS) = SX2

      PD1(0) = 0D0
      IPROSS = 0

      IPROSS=IPROSS+1  ! d d~ > t t~ a
      PD1(IPROSS) = D1

      IPROSS=IPROSS+1  ! s s~ > t t~ a
      PD1(IPROSS) = S1
      DO I=1,IPROC
        IF (NINCOMING.EQ.2) THEN
          LUM = LUM + PD(I) * CONV
        ELSE
          LUM = LUM + PD(I)
        ENDIF
      ENDDO
      RETURN
      END

