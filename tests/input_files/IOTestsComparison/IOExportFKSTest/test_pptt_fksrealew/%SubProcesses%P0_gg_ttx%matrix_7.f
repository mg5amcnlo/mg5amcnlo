      SUBROUTINE SMATRIX7(P,ANS_SUMMED)
C     
...
C     
C     
C     Process: g u~ > t t~ u~ [ real = QCD QED ] QCD^2<=4 QED^2<=2
C     Process: g c~ > t t~ c~ [ real = QCD QED ] QCD^2<=4 QED^2<=2
C     
C     
C     CONSTANTS
C     
...
C     look for orders which match the nlo order constraint 
      ...
      IF (FIRSTTIME) THEN
        DO I = 1, NSQAMPSO
          DO J = 1, NSPLITORDERS
            IF(GETORDPOWFROMINDEX7(J, I) .GT. NLO_ORDERS(J)) THEN
              KEEP_ORDER(I) = .FALSE.
              EXIT
            ENDIF
          ENDDO
          IF (KEEP_ORDER(I)) THEN
            WRITE(*,*) 'REAL 7: keeping split order ', I
          ELSE
            WRITE(*,*) 'REAL 7: not keeping split order ', I
          ENDIF
        ENDDO
        FIRSTTIME = .FALSE.
      ENDIF

      CALL SMATRIX7_SPLITORDERS(P,ANS)
      ANS_SUMMED = 0D0
      ANS_MAX = 0D0
...
C          different coupling combinations
          DO J = 1, NSPLITORDERS
            AMP_ORDERS(J) = GETORDPOWFROMINDEX7(J, I)
          ENDDO
...
C     avoid fake non-zeros
      ...
      SUBROUTINE SMATRIX7_SPLITORDERS(P,ANS)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C         add to the sum of helicities
      ...
      SUBROUTINE MATRIX_7(P,NHEL,RES)
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     ----------
      ...
      CALL FFV1_1(W(2),W(1),GC_11,ZERO,ZERO,W(6))
      CALL FFV1P0_3(W(4),W(3),GC_2,ZERO,ZERO,W(7))
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(5),W(6),W(7),GC_2,AMP(1))
      CALL FFV1P0_3(W(4),W(3),GC_11,ZERO,ZERO,W(8))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(5),W(6),W(8),GC_11,AMP(2))
      CALL FFV2_5_3(W(4),W(3),GC_51,GC_58,MDL_MZ,MDL_WZ,W(9))
C     Amplitude(s) for diagram number 3
      CALL FFV2_5_0(W(5),W(6),W(9),GC_51,GC_58,AMP(3))
      CALL FFV1_1(W(3),W(1),GC_11,MDL_MT,MDL_WT,W(6))
      CALL FFV1P0_3(W(5),W(2),GC_2,ZERO,ZERO,W(10))
C     Amplitude(s) for diagram number 4
      CALL FFV1_0(W(4),W(6),W(10),GC_2,AMP(4))
      CALL FFV1P0_3(W(5),W(2),GC_11,ZERO,ZERO,W(11))
C     Amplitude(s) for diagram number 5
      CALL FFV1_0(W(4),W(6),W(11),GC_11,AMP(5))
      CALL FFV2_5_3(W(5),W(2),GC_51,GC_58,MDL_MZ,MDL_WZ,W(12))
C     Amplitude(s) for diagram number 6
      CALL FFV2_5_0(W(4),W(6),W(12),GC_51,GC_58,AMP(6))
      CALL FFV1_2(W(4),W(1),GC_11,MDL_MT,MDL_WT,W(6))
C     Amplitude(s) for diagram number 7
      CALL FFV1_0(W(6),W(3),W(10),GC_2,AMP(7))
C     Amplitude(s) for diagram number 8
      CALL FFV1_0(W(6),W(3),W(11),GC_11,AMP(8))
C     Amplitude(s) for diagram number 9
      CALL FFV2_5_0(W(6),W(3),W(12),GC_51,GC_58,AMP(9))
      CALL FFV1_2(W(5),W(1),GC_11,ZERO,ZERO,W(6))
C     Amplitude(s) for diagram number 10
      CALL FFV1_0(W(6),W(2),W(7),GC_2,AMP(10))
C     Amplitude(s) for diagram number 11
      CALL FFV1_0(W(6),W(2),W(8),GC_11,AMP(11))
C     Amplitude(s) for diagram number 12
      CALL FFV2_5_0(W(6),W(2),W(9),GC_51,GC_58,AMP(12))
C     Amplitude(s) for diagram number 13
      CALL VVV1_0(W(1),W(11),W(8),GC_10,AMP(13))
C     JAMPs contributing to orders QCD=3 QED=0
     ...
     $ -01)*AMP(11)
      JAMP(3,1) = (-1.666666666666667D-01)*AMP(5)+(-1.666666666666667D
     $ -01)*AMP(8)
      JAMP(4,1) = (5.000000000000000D-01)*AMP(5)+(5.000000000000000D
     $ -01)*AMP(11)+((0.000000000000000D+00,-5.000000000000000D-01))
     $ *AMP(13)
C     JAMPs contributing to orders QCD=1 QED=2
      ...
      DO M = 1, NAMPSO
        ...
        DO I = 1, NCOLOR
          ...
          ENDDO
          DO N = 1, NAMPSO
            RES(SQSOINDEX7(M,N)) = RES(SQSOINDEX7(M,N)) + ZTEMP
     $       *DCONJG(JAMP(I,N))
          ENDDO
        ENDDO
...
C     
      ...
      INTEGER FUNCTION SQSOINDEX7(AMPORDERA,AMPORDERB)
C     
...
C     FUNCTION
C     
      INTEGER SQSOINDEX_FROM_ORDERS7
C     
C     BEGIN CODE
C     
      DO I=1,NSPLITORDERS
     ...
     $   +AMPSPLITORDERS(AMPORDERB,I)
      ENDDO
      SQSOINDEX7=SQSOINDEX_FROM_ORDERS7(SQORDERS)
      END



      INTEGER FUNCTION SQSOINDEX_FROM_ORDERS7(ORDERS)
C     
...
C     
 ...
 1009   CONTINUE
     ...
     $ ,NSPLITORDERS)
      ...
      INTEGER FUNCTION GETORDPOWFROMINDEX7(IORDER, INDX)
C     
...
C     
      IF (IORDER.GT.NSPLITORDERS.OR.IORDER.LT.1) THEN
        WRITE(*,*) 'INVALID IORDER 7', IORDER
        WRITE(*,*) 'SHOULD BE BETWEEN 1 AND ', NSPLITORDERS
        STOP
      ENDIF

      IF (INDX.GT.NSQAMPSO.OR.INDX.LT.1) THEN
        WRITE(*,*) 'INVALID INDX 7', INDX
        WRITE(*,*) 'SHOULD BE BETWEEN 1 AND ', NSQAMPSO
        STOP
      ENDIF

      GETORDPOWFROMINDEX7=SQSPLITORDERS(INDX, IORDER)
      END



      SUBROUTINE GET_NSQSO_REAL7(NSQSO)
C     
