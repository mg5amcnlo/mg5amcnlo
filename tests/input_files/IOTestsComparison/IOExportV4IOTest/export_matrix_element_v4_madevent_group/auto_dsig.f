      DOUBLE PRECISION FUNCTION DSIG1(PP,IFLAV,WGT,IMODE)
C     ****************************************************
C     
...
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG1=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL-IPSEL_SHIFT))
      ELSE
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG1(ALL_PP(0,1,1),IFLAV_VEC(1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C         Select a flavor combination (need to do here for right sign)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX1_MULTI(P_MULTI, IFLAV_VEC, HEL_RAND, COL_RAND,
...
C           Call UNWGT to unweight and store events
     ...
     $       , SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
      ...
      SUBROUTINE SMATRIX1_MULTI(P_MULTI, IFLAV_VEC, HEL_RAND, COL_RAND
