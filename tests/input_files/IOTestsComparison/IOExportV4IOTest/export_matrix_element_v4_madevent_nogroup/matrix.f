      SUBROUTINE SMATRIX(P, IFLAV, RHEL, RCOL, DUM, IVEC, ANS,
...
C     Per-row FLAVOR lookup used by BROKEN_SYM. The IFLAV-indexed
C      FLAVOR
C     table above can collapse distinct same-flavor / different-flavor
C     leshouche rows that share the same coupling group; BROKEN_SYM
C      needs
C     row-level information to apply the identical-particle correction.
      INTEGER FLAVOR_ROW(NEXTERNAL,1)
      DATA (FLAVOR_ROW(I,  1),I=  1, NEXTERNAL) /1, 1, 1, 1/
...
C           WRITE(HEL_BUFF,'(20i5)')(NHEL(II,I),II=1,NEXTERNAL)
 ...
 10     CONTINUE
     ...
     $     //' zero but not the total matrix-element'
        ...
        ENDIF
      ENDIF
      IF (IPSEL.GE.1.AND.IPSEL.LE.1) THEN
        FLAVOR_FOR_SYM(:) = FLAVOR_ROW(:, IPSEL)
      ELSE
        FLAVOR_FOR_SYM(:) = FLAVOR(:)
      ENDIF
      ANS=ANS/DBLE(IDEN)*BROKEN_SYM(FLAVOR_FOR_SYM)
      ...
      SUBROUTINE GET_FLAVOR(IFLAV, FLAVOR_OUT)
C     Returns the flavor array for a given flavor index IFLAV
      ...
      DATA (FLAVOR(I,  1),I=  1, NEXTERNAL) /1, 1, 1, 1/
...
C      index.
      ...
      CALL FFV1_3(W(1),W(2),GQQ,ZERO, FK_ZERO,W(5))
C     Amplitude(s) for diagram number 1
      CALL VVV1_0(W(3),W(4),W(5),G,AMP(1))
      CALL FFV1_2(W(1),W(3),GQQ,ZERO, FK_ZERO,W(5))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(5),W(2),W(4),GQQ,AMP(2))
      CALL FFV1_2(W(1),W(4),GQQ,ZERO, FK_ZERO,W(5))
