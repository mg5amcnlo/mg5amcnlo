...
C     ===========================================
      ...
      SUBROUTINE ML5_0_SAMURAI_LOOP(NLOOPLINE,PL,M2L,RANK,RES,STABLE)
C     
C     Module used
C     
      USE MSAMURAI
...
C     This tells Samurai to perform the reduction all the way to
C      tadpoles.
...
C     
      ...
      REAL*8 P_SAM(0:NLOOPLINE-1,4)
      COMPLEX*16 M2L(NLOOPLINE)
      COMPLEX*16 M2L_SAM(0:NLOOPLINE-1)
      COMPLEX*16 RES(3), SAM_RES(-2:0)
      LOGICAL STABLE
C     
C     LOCAL VARIABLES 
C     
...
C     Hook for numerator function for Samurai
      COMPLEX*16 ML5_0_SAMURAI_LOOPNUM
      EXTERNAL ML5_0_SAMURAI_LOOPNUM
C     
C     GLOBAL VARIABLES
C     
      INCLUDE 'coupl.inc'
...
C     CONVERT THE MOMENTA FLOWING IN THE LOOP LINES TO SAMURAI
C      CONVENTIONS
      DO I=0,3
        DO J=0,(NLOOPLINE-1)
          P_TMP(I,J)=0.D0
...
C     In samurai, the first index is the momentum one (with energy
C      last) and the second one is the leg number
      DO I=0,NLOOPLINE-1
        P_SAM(I,1) = P_TMP(1,I)
        P_SAM(I,2) = P_TMP(2,I)
        P_SAM(I,3) = P_TMP(3,I)
...
C     Optionally one can set the kinematic matrix fo samurai
      CALL ML5_0_SET_SAMURAI_S_MAT(NLOOPLINE,P_TMP,M2L)

      CALL SAMURAI(ML5_0_SAMURAI_LOOPNUM,SAM_RES,R1,P_SAM,M2L_SAM
     $ ,NLOOPLINE,RANK,SAM_REDUCTION_STEPS,MU_R**2,STABLE)
...
C     =====================================================
      FUNCTION ML5_0_SAMURAI_LOOPNUM(ICUT_DUMMY,Q_IN,MU2_DUMMY)
      IMPLICIT NONE
C     
C     ARGUMENTS 
C     
      ...
      SUBROUTINE ML5_0_SET_SAMURAI_S_MAT(NLOOPLINE,P_LOOP_IN,M2L)
C     
C     Module used
C     
...
C     
      INTEGER NLOOPLINE
      COMPLEX*16 M2L(NLOOPLINE)
      REAL*8 P_LOOP_IN(0:3,0:NLOOPLINE-1)
C     
C     LOCAL VARIABLES 
C     
      INTEGER I,J,K
      REAL*8 P_LOOP(NLOOPLINE,0:3)
      COMPLEX*16 S_MAT_OUT(NLOOPLINE,NLOOPLINE)
C     
C     BEGIN CODE
C     
      DO I=1,NLOOPLINE
        ...
        ENDDO
      ENDDO
      CALL ML5_0_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP,M2L,S_MAT_OUT)
...
C     3 -> QCDLOOP
      IF (CTLOOPLIBRARY.EQ.1) THEN
        WRITE(*,*) 'Warning in Samurai initialization. LoopTools i'
     $   //'s not supported by the Samurai interface. It will us'
     $   //'e OneLOop instead.'
        LOOPLIB = 2
      ...
      ELSEIF (CTLOOPLIBRARY.EQ.2) THEN
        LOOPLIB = 2
      ELSE
        WRITE(*,*) 'Error in Samurai initialization. Loop library ID='
...
C     -------------------------------
      CALL INITSAMURAI(NUMERATORFORM,LOOPLIB,SAM_VERBOSITY,STABILITYTES
     $ T)
...
C         Samurai is used
          CALL ML5_0_SAMURAI_LOOP(NLOOPLINE,PL,M2L,RANK,LOOPRES(1
