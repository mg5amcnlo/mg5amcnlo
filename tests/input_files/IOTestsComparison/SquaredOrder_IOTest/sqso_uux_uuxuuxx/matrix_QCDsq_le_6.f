      SUBROUTINE SMATRIX(P, FLAVOR, ANS_SUMMED)
C     
...
C     BEGIN CODE
C     
      CALL SMATRIX_SPLITORDERS(P, FLAVOR, ANS)
      ANS_SUMMED=ANS(0)
...
C     ----------
      ...
      SUBROUTINE SMATRIXHEL_SPLITORDERS(P,HEL, FLAVOR, ANS)
      IMPLICIT NONE
C     
C     CONSTANT
C     
...
C     ----------
      ...
      SUBROUTINE SMATRIX_SPLITORDERS(P, FLAVOR, ANS)
C     
...
C      only three external particles.
     ...
     $       -1.AND.(.NOT.IS_BORN_HEL_SELECTED(IHEL))) THEN
              CYCLE
            ENDIF
            CALL MATRIX(P ,NHEL(1,IHEL),JC(1),FLAVOR(1), T)
            BUFF=0D0
            DO I=1,NSQAMPSO
     ...
     $         ) THEN
      ...
      SUBROUTINE MATRIX(P,NHEL,IC,FLAVOR,RES)
      USE MODEL_OBJECT
C     
C     Generated by MadGraph5_aMC@NLO v. %(version)s, %(date)s
C     By the MadGraph5_aMC@NLO Development Team
...
C     BEGIN CODE
C     ----------
      CALL GET_AMP(P,NHEL,IC,FLAVOR,AMP)
C     WRITE (*,*) '  -> AMP = ', AMP
...
CF2PY INTENT(OUT) :: IDEN_STAR
      ...
      SUBROUTINE GET_AMP(P,NHEL,IC,FLAVOR,AMP)
      USE MODEL_OBJECT
C     
C     Process: u u~ > u u~ u u~ QED^2<=6
C     
