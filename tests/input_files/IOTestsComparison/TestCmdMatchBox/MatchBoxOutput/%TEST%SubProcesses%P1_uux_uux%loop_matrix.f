      SUBROUTINE P1_SLOOPMATRIXHEL(P,HEL,ANS)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     
      ...
      REAL*8 ANS(3)
      INTEGER HEL, USERHEL
      COMMON/P1_USERCHOICE/USERHEL
C     ----------
C     BEGIN CODE
C     ----------
      USERHEL=HEL
      CALL P1_SLOOPMATRIX(P,ANS)
      END

      SUBROUTINE P1_SLOOPMATRIX(P_USER,ANS)
C     
...
C     This parameter is designed for the check timing command of MG5
      ...
      COMMON/P1_STABILITY_TESTS/MAXSTABILITYLENGTH
C     
C     ARGUMENTS 
C     
...
C      stability check to take place
      ...
      COMMON/P1_BYPASS_CHECK/BYPASS_CHECK, ALWAYS_TEST_STABILITY
C     
C     FUNCTIONS
C     
      INTEGER P1_ISSAME
...
C     Typically, the hel_offset is 10000
     ...
     $ UNCS)
      ...
      COMMON/P1_HELCONFIGS/HELC

      REAL*8 PREC,USER_STAB_PREC
      DATA USER_STAB_PREC/-1.0D0/
      COMMON/P1_USER_STAB_PREC/USER_STAB_PREC
...
C     RETURNCODE=100*RET_CODE_H+10*RET_CODE_T+RET_CODE_U
      ...
      COMMON/P1_ACC/ACCURACY,RET_CODE_H,RET_CODE_T,RET_CODE_U

      LOGICAL MP_DONE_ONCE
      DATA MP_DONE_ONCE/.FALSE./
      COMMON/MP_P1_DONE_ONCE/MP_DONE_ONCE
...
C     ----------
      ...
      IF(NTRY.EQ.0) THEN
        ...
        CALL P1_SET_N_EVALS(N_DP_EVAL,N_QP_EVAL)
...
C       um conservation)
        CALL P1_IMPROVE_PS_POINT_PRECISION(PS)
      ENDIF
...
C     Check if we directly go to multiple precision
      IF (CTMODE.GE.4) THEN
        IF (.NOT.MP_DONE) THEN
          CALL MP_P1_COMPUTE_LOOP_COEFS(MP_P,BUFFR_BIS)
...
C         Coefficient construction for loop diagram with ID 30
     ...
     $     ,82,H)
      ...
      CALL P1_ADD_COEFS(LOOPCOEFS(0,1),2,LOOPCOEFS(0,3),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,1),2,LOOPCOEFS(0,17),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,1),2,LOOPCOEFS(0,19),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,1),2,LOOPCOEFS(0,25),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,1),2,LOOPCOEFS(0,27),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,2),2,LOOPCOEFS(0,6),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,2),2,LOOPCOEFS(0,18),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,2),2,LOOPCOEFS(0,20),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,2),2,LOOPCOEFS(0,26),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,2),2,LOOPCOEFS(0,28),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,4),2,LOOPCOEFS(0,5),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,7),2,LOOPCOEFS(0,8),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,9),2,LOOPCOEFS(0,10),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,11),2,LOOPCOEFS(0,15),2)
      CALL P1_ADD_COEFS(LOOPCOEFS(0,13),2,LOOPCOEFS(0,14),2)
...
C        METHODS
     ...
     $   .1).OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.1)) ) THEN
          EVAL_DONE(3)=.TRUE.
          CALL P1_ROTATE_PS(PS,P,1)
          IF (DOING_QP_EVALS) CALL MP_P1_ROTATE_PS(MP_PS,MP_P,1)
          GOTO 200
        ENDIF
     ...
     $   .2).OR.((.NOT.DOING_QP_EVALS).AND.NROTATIONS_DP.GE.2)) ) THEN
          EVAL_DONE(4)=.TRUE.
          CALL P1_ROTATE_PS(PS,P,2)
          IF (DOING_QP_EVALS) CALL MP_P1_ROTATE_PS(MP_PS,MP_P,2)
          GOTO 200
        ENDIF

        CALL P1_ROTATE_PS(PS,P,0)
        IF (DOING_QP_EVALS) CALL MP_P1_ROTATE_PS(MP_PS,MP_P,0)

C       END OF THE DEFINITIONS OF THE DIFFERENT EVALUATION METHODS

        IF(DOING_QP_EVALS) THEN
          CALL P1_COMPUTE_ACCURACY(QP_RES,N_QP_EVAL,ACC,ANS)
          ACCURACY=ACC
          RET_CODE_H=3
          IF(ACC.GE.MLSTABTHRES) THEN
            RET_CODE_H=4
            NEPS=NEPS+1
            CALL P1_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,TEMP1,TEMP)
     ...
     $          unstable PS points will now be suppressed.'
          ...
          ENDIF
        ELSE
          CALL P1_COMPUTE_ACCURACY(DP_RES,N_DP_EVAL,ACC,ANS)
          IF(ACC.GE.MLSTABTHRES) THEN
            DOING_QP_EVALS=.TRUE.
            EVAL_DONE(1)=.TRUE.
            DO I=2,MAXSTABILITYLENGTH
...
C           Be loser for helicity comparison, so bring a factor 100
 ...
 1231   CONTINUE
      ...
      SUBROUTINE P1_COMPUTE_ACCURACY(FULLLIST, LENGTH, ACC, ESTIMATE)
      IMPLICIT NONE
C     
C     PARAMETERS 
C     
...
C     The following is used instead
      ...
      SUBROUTINE P1_SET_N_EVALS(N_DP_EVALS,N_QP_EVALS)
...
C      VARIABLES FROM A GIVEN VARIABLE IN DOUBLE PRECISION
      ...
      DO I=1,NEXTERNAL
        ...
        ENDDO
      ENDDO
      CALL MP_P1_IMPROVE_PS_POINT_PRECISION(MP_PS)
      DO I=1,NEXTERNAL
        DO J=0,3
          MP_P(J,I)=MP_PS(J,I)
        ENDDO
      ...
      SUBROUTINE P1_FORCE_STABILITY_CHECK(ONOFF)
C     
...
C     
      ...
      SUBROUTINE P1_SLOOPMATRIXHEL_THRES(P,HEL,ANS,PREC_ASKED
     $ ,PREC_FOUND,RET_CODE)
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     ----------
      ...
      CALL P1_SLOOPMATRIXHEL(P,HEL,ANS)
...
C     Reset it to default value not to affect next runs
      ...
      SUBROUTINE P1_SLOOPMATRIX_THRES(P,ANS,PREC_ASKED,PREC_FOUND
     $ ,RET_CODE)
C     
C     Inputs are:
