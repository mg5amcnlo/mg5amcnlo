...
c
     ...
     .dsqrt(dabs(DOT(p(0,i),p(0,i))))
      ...
      write (*,*) "-----------------------------------------------------------------------------"
              ...
              FLAVOR(:,:) =1
        PDG_FOR_FLAVOR(1,1) = 24
        PDG_FOR_FLAVOR(2,1) = -24
        PDG_FOR_FLAVOR(3,1) = 24
        PDG_FOR_FLAVOR(4,1) = -24
c     
...
c     (last zero avoid to update as, otherwise new value for as can be  provided
       call GET_DENSITY(P,  POS, N_CHANGING, ALLOW_HEL, N_COMB, FLAVOR, 0d0, INTER)
