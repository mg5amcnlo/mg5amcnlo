...
C     ----------
      ...
      CALL VVV1_1(W(1,1),W(1,2),GC_10,ZERO,ZERO,W(1,6))
      CALL FFV1_3(W(1,4),W(1,3),GC_11,ZERO,ZERO,W(1,7))
...
C     Amplitude(s) for diagram number 4
      ...
      CALL VVV1_1(W(1,2),W(1,5),GC_10,ZERO,ZERO,W(1,11))
...
C     Amplitude(s) for diagram number 9
      ...
      CALL VVV1_1(W(1,1),W(1,5),GC_10,ZERO,ZERO,W(1,6))
...
C     Amplitude(s) for diagram number 15
      ...
      CALL VVVV1_1(W(1,1),W(1,2),W(1,5),GC_12,ZERO,ZERO,W(1,11))
      CALL VVVV3_1(W(1,1),W(1,2),W(1,5),GC_12,ZERO,ZERO,W(1,7))
      CALL VVVV4_1(W(1,1),W(1,2),W(1,5),GC_12,ZERO,ZERO,W(1,10))
...
C     
C     Generated by MadGraph 5 v. 2.0.0.beta3_iso, 2013-06-24
C     By the MadGraph Development Team
C     Please visit us at https://launchpad.net/madgraph5
C     
C     RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C     ----------
     ...
     $    called only with calculatedborn = true'
      ...
      DO IHEL=1,NCOMB
        WGT_HEL(IHEL)=0D0
        IF (GOODHEL(IHEL,1,NFKSPROCESS) .OR. NTRY(NFKSPROCESS) .LT. 2
     $   ) THEN
          WGT_HEL(IHEL)=BORN_HEL(P1,NHEL(1,IHEL),IHEL,JC(1))/DBLE(IDEN(
     $     1))
          ANS=ANS+WGT_HEL(IHEL)
          IF ( T.NE.0D0 .AND. .NOT.GOODHEL(IHEL,1,NFKSPROCESS)) THEN
            GOODHEL(IHEL,1,NFKSPROCESS)=.TRUE.
            NGOOD(NFKSPROCESS) = NGOOD(NFKSPROCESS) +1
            IGOOD(NGOOD(NFKSPROCESS),NFKSPROCESS) = IHEL
          ENDIF
        ENDIF
      ...
      REAL*8 FUNCTION BORN_HEL(P,NHEL,HELL,IC)
C     
C     Generated by MadGraph 5 v. 2.0.0.beta3_iso, 2013-06-24
C     By the MadGraph Development Team
C     Please visit us at https://launchpad.net/madgraph5
