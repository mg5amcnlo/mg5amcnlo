...
class TestFKSProcessFromReals(unittest.TestCase):
    ...
    def setUp(self):
        if not hasattr(self, 'mymodel') or \
           not hasattr(self, 'myleglist') or \
           not hasattr(self, 'myproc') or \
           not hasattr(self, 'fks1') or \
           not hasattr(self, 'fks1_rem') or \
           not hasattr(self, 'fks2'):
            # PROCESS: u u~ > d d~ g g 
            myleglist = MG.LegList([ \
                MG.Leg({'id': 2, 'number': 1, 'state': False}), 
                MG.Leg({'id': -2, 'number': 2, 'state': False}),
                MG.Leg({'id': 1, 'number': 3, 'state': True}),
                MG.Leg({'id': -1, 'number': 4, 'state': True}),
                MG.Leg({'id': 21, 'number': 5, 'state': True}),
                MG.Leg({'id': 21, 'number': 6, 'state': True})])

            # PROCESS: u u~ > u u~ g  
            myleglist2 = MG.LegList([ \
                MG.Leg({'id': 2, 'number': 1, 'state': False}),
                MG.Leg({'id': -2, 'number': 2, 'state': False}),
                MG.Leg({'id': 2, 'number': 3, 'state': True}),
                MG.Leg({'id': -2, 'number': 4, 'state': True}),
                MG.Leg({'id': 21, 'number': 5, 'state': True,})])
            ...
            TestFKSProcessFromReals.mymodel = mymodel 
            TestFKSProcessFromReals.myleglist = myleglist
            TestFKSProcessFromReals.myproc = myproc 
            TestFKSProcessFromReals.fks1 = fks.FKSProcessFromReals(myproc, False)
            TestFKSProcessFromReals.fks1_rem = fks.FKSProcessFromReals(myproc)
            TestFKSProcessFromReals.fks2 = fks.FKSProcessFromReals(MG.Process(dict2), False)
