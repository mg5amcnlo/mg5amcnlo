...
c
      real  x      !  OUT - quasi-random value returned
                   !        x is in the range (a,b)
      real  a      !  IN  - Lower bound of interval for x
      real  b      !  IN  - Upper bound of interval for x
      integer iseed    !  IN  - Sequence number, restricted to 1 <= i <= 25
c
c--Local Variable Declarations
c
      integer ndim,kdim,mdim,i
      parameter (ndim = 25, kdim = 181, mdim = 1060)
      integer base_minus1(ndim),mix(mdim),k(kdim)
      real accum(kdim)
...
c
      j = 1
      offset = koffset(iseed)
      do while (k(offset+j) .eq. base_minus1(iseed))
         k(offset+j)=0
         j=j+1
         if (j.gt.maxj(iseed)) then
            do j=1,maxj(iseed)
               k(offset+j)=0
               accum(offset+j)=0.
            end do
      ...
      k(offset+j)=k(offset+j)+1
      accum(offset+j) = accum(offset+j+1) +
     .     mix(mix_offset(iseed)+k(offset+j))/pbase(offset+j)
      do  jj=2,j-1
         accum(offset+jj) = accum(offset+j)
      end do
      x = a + (b-a) * accum(offset+j)
