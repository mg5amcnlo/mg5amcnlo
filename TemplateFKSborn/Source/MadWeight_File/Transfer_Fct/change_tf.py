...
def create_TF_main(name,make):
    ...
    print "transfer_functions_fct.f created"
    ...
    os.chdir('../../../') #go to main
    P_dir,MW_dir=MW_param.detect_SubProcess(P_mode=0)
 
    for directory in MW_dir:
        obj=TF_in_SubProcesses(TF_file,directory)
        obj.write_transfer_function_file() 
    print 'transfer_function.f created in for all Subprocesses'
...
class TF_input(XML_input):
    ...
    def create_ordering_file(self):
        ...
        #define the rule of how modif file Tranfer_FCT/input/ordering_file.inc        
        modif_rule={}
    ...
    def create_transfer_functions(self):
        ...
        ff=open('./transfer_functions_fct.f','w')
        ff.writelines(text)
        return list_var
...
class TF_in_SubProcesses:
    ...
    def write_transfer_function_file(self):
        ...
        template = mod_file.Mod_file(rule_file='./Source/MadWeight_File/Transfer_Fct/input/mod_generic')
        text=mod_file.mod_text(text,template.dico)
        text=put_in_fortran_format(text)
        
        ff=open(self.dir+'/transfer_functions.f','w')
        ff.writelines(text)
        ff.close()
    ...
    def text_tf_E_for_one_part(self,i):
        ...
        text+='\n     purpose: returns the value of the transfert function (in energy)\n'
...
def extract_tf_name(filepos='./Cards/proc_card.dat'):
    ...
    for line in file(filepos):
        if found:
            ...
            return name
        if line.startswith('# Begin Transfer Fct'):
            found=1
...
if(__name__=="__main__"):
    ...
    opt=sys.argv
    if len(opt)<2:
        listdir=os.listdir('./Source/MadWeight_File/Transfer_Fct/data')
