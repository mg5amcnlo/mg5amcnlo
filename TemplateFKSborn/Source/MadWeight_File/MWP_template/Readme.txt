...
**          $> ./bin/madweight.py A+ 
**                  launch all steps following (and including) step A
**          $> ./bin/madweight.py A- 
**                  launch all steps preceeding (and including) step A
**
**    Where A/B/C are one of the following names:
**
**    1) param: Creates all the param_card.dat if required in MadWeight_card
...
**   relaunch: Relaunch failed (and zero results) run. 
**   clean=NAME: Supress the event by event log/result/... for the run NAME
**        example: $> ./bin/madweight.py clean=my_run
**   clean: Supress the event by event log/result/... for the current run_name 
