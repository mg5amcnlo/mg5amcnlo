...
c Choose between process and mirror
      ...
      rescale_mir=selproc(imirror)/sumprob
c If mirror process rotate (all) the momenta around the x-axis
      if (imirror.eq.2) then
        do k=1,nexternal
          ...
          pp(3,i)=-pp(3,i)
          do i=-2,2
            p1_cnt(2,i,k)=-p1_cnt(2,i,k)
            p1_cnt(3,i,k)=-p1_cnt(3,i,k)
          enddo
        enddo
        do k=1,nexternal-1
          p_born(2,i)=-p_born(2,i)
          p_born(3,i)=-p_born(3,i)
        enddo
      endif
