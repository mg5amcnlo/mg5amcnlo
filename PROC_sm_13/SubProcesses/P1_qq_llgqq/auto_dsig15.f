...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d d~ > e+ e- g u u~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > e+ e- g c c~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > mu+ mu- g u u~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > mu+ mu- g c c~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > e+ e- g u u~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > e+ e- g c c~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > mu+ mu- g u u~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > mu+ mu- g c c~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG15 = PD(0)
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX15(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,15,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG15=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG15*WGT,15,SELECTED_HEL(1), SELECTED_COL(1),
     $    1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG15_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG15(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(15,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(15) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(15)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX15_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C          1.0)
          DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,15, IVEC)
...
C           Call UNWGT to unweight and store events
     ...
     $       ,15, SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
          ENDIF
        ENDDO
      ENDDO
      ...
      SUBROUTINE SMATRIX15_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX15(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
...
!$OMP END PARALLEL
      ...
      INTEGER FUNCTION GET_NHEL15(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL15 = NHEL(IPART, IABS(HEL))
      RETURN
      END
