...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX20(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,20,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG20=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG20*WGT,20,SELECTED_HEL(1), SELECTED_COL(1),
     $    1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG20_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG20(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C           LP=SIGN(1,LPP(IB(2)))
     ...
     $       ,DSQRT(ALL_Q2FACT(2, IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=DX1(IVEC)*SX2(IVEC)
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(20,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(20) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(20)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX20_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C          1.0)
          DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,20, IVEC)
...
C           Call UNWGT to unweight and store events
     ...
     $       ,20, SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
          ENDIF
        ENDDO
      ENDDO
      ...
      SUBROUTINE SMATRIX20_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX20(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
...
!$OMP END PARALLEL
      ...
      INTEGER FUNCTION GET_NHEL20(HEL, IPART)
...
C      particle 
      ...
      GET_NHEL20 = NHEL(IPART, IABS(HEL))
      RETURN
      END
