...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g d > e+ e- d s s~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > mu+ mu- d s s~
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > e+ e- s d d~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > mu+ mu- s d d~
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG8 = PD(0)
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX8(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,8,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG8=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG8*WGT,8,SELECTED_HEL(1), SELECTED_COL(1), 1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG8_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT,
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG8(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C           LP=SIGN(1,LPP(IB(2)))
     ...
     $       ,DSQRT(ALL_Q2FACT(2, IVEC)))
      ...
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=G1(IVEC)*D2(IVEC)
      ...
      IPROC=IPROC+1  ! g s > e+ e- s d d~
      DO IVEC=1, VECSIZE_USED
        ALL_PD(IPROC,IVEC)=G1(IVEC)*S2(IVEC)
        ALL_PD(0,IVEC)=ALL_PD(0,IVEC)+DABS(ALL_PD(IPROC,IVEC))
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(8,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(8) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(8)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX8_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C          1.0)
          DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,8, IVEC)
...
C           Call UNWGT to unweight and store events
            ...
            CALL UNWGT(ALL_PP(0,1,IVEC), ALL_OUT(IVEC)*ALL_WGT(IVEC),8
     $       , SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
      ...
      SUBROUTINE SMATRIX8_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX8(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
