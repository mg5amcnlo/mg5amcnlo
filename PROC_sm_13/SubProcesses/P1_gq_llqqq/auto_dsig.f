...
C       Output weights and number of events
        ...
        IF (SUMPROB.NE.0D0)THEN
          DO J=1,SYMCONF(0)
            WRITE(*,'(24E12.4)')((SUMWGT(K,I,J)/SUMPROB,K=1,2),I=1
     $       ,MAXSPROC)
          ENDDO
        ENDIF
        SUMPROB=0D0
        ...
        IF (SUMPROB.NE.0D0)THEN
          DO J=1,SYMCONF(0)
            WRITE(*,'(24E12.4)')((NUMEVTS(K,I,J)/SUMPROB,K=1,2),I=1
     $       ,MAXSPROC)
          ENDDO
        ENDIF
        WRITE(*,*)'Events:'
        DO J=1,SYMCONF(0)
          WRITE(*,'(24I12)')((NUMEVTS(K,I,J),K=1,2),I=1,MAXSPROC)
        ENDDO
...
C      discrete sampler(imode=5) and 
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! g u > e+ e- u u u~
      IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! g d > e+ e- d d d~
      IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! g u~ > e+ e- u u~ u~
      IF(IPROC.EQ.4) DSIGPROC=DSIG4(P1,WGT,IMODE)  ! g d~ > e+ e- d d~ d~
      IF(IPROC.EQ.5) DSIGPROC=DSIG5(P1,WGT,IMODE)  ! g u > e+ e- u c c~
      IF(IPROC.EQ.6) DSIGPROC=DSIG6(P1,WGT,IMODE)  ! g u > e+ e- u d d~
      IF(IPROC.EQ.7) DSIGPROC=DSIG7(P1,WGT,IMODE)  ! g d > e+ e- u d u~
      IF(IPROC.EQ.8) DSIGPROC=DSIG8(P1,WGT,IMODE)  ! g d > e+ e- d s s~
      IF(IPROC.EQ.9) DSIGPROC=DSIG9(P1,WGT,IMODE)  ! g u~ > e+ e- c u~ c~
      IF(IPROC.EQ.10) DSIGPROC=DSIG10(P1,WGT,IMODE)  ! g u~ > e+ e- d u~ d~
      IF(IPROC.EQ.11) DSIGPROC=DSIG11(P1,WGT,IMODE)  ! g d~ > e+ e- u u~ d~
      IF(IPROC.EQ.12) DSIGPROC=DSIG12(P1,WGT,IMODE)  ! g d~ > e+ e- s d~ s~
C     ENDIF
...
C            the warp)
     ...
     $ ,IMIRROR_VEC,VECSIZE_USED)  ! g u > e+ e- u u u~
      IF(IPROC.EQ.2) CALL DSIG2_VEC(ALL_P1,ALL_XBK,ALL_Q2FACT
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,SYMCONF, CONFSUB,ICONF_VEC
     $ ,IMIRROR_VEC,VECSIZE_USED)  ! g d > e+ e- d d d~
