...
C       LP=SIGN(1,LPP(IB(2)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u~ > e+ e- c u~ c~
      PD(IPROC)=G1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g u~ > mu+ mu- c u~ c~
      PD(IPROC)=G1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c~ > e+ e- u c~ u~
      PD(IPROC)=G1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c~ > mu+ mu- u c~ u~
      PD(IPROC)=G1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG9 = PD(0)
      ...
      CALL RANMAR(RHEL)
      CALL RANMAR(RCOL)
      CALL SMATRIX9(P1,RHEL, RCOL,CHANNEL,1, DSIGUU, SELECTED_HEL(1),
     $  SELECTED_COL(1))
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,9,1)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG9=DSIGN(CONV*PD(0)*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG9*WGT,9,SELECTED_HEL(1), SELECTED_COL(1), 1)
      ENDIF
...
C     
      ...
      DOUBLE PRECISION FUNCTION DSIG9_VEC(ALL_PP, ALL_XBK, ALL_Q2FACT,
...
C     ----------
      ...
      IF(IMODE.EQ.1)THEN
        NFACT = DSIG9(ALL_PP(0,1,1), ALL_WGT(1), IMODE)
        RETURN
      ENDIF
...
C         Select a flavor combination (need to do here for right sign)
          ...
          CHANNELS(IVEC) = CONFSUB(9,SYMCONF(ICONF_VEC(CURR_WARP)))
          SUBDIAG(9) = CHANNELS(IVEC)  ! only valid if a single process
          CHANNEL = SUBDIAG(9)
     ...
     $       ,1,IVEC))
      ...
      CALL SMATRIX9_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
C          1.0)
          DSIGUU=DSIGUU*CUSTOM_BIAS(P1,DSIGUU,9, IVEC)
...
C           Call UNWGT to unweight and store events
            ...
            CALL UNWGT(ALL_PP(0,1,IVEC), ALL_OUT(IVEC)*ALL_WGT(IVEC),9
     $       , SELECTED_HEL(IVEC), SELECTED_COL(IVEC), IVEC)
      ...
      SUBROUTINE SMATRIX9_MULTI(P_MULTI, HEL_RAND, COL_RAND, CHANNELS,
...
!$OMP DO
      DO IVEC=1, VECSIZE_USED
        CALL SMATRIX9(P_MULTI(0,1,IVEC),
     &	                         hel_rand(IVEC),
     &                           col_rand(IVEC),
     &				 channels(IVEC),
     &                           IVEC,
...
!$OMP END PARALLEL
      ...
      INTEGER FUNCTION GET_NHEL9(HEL, IPART)
