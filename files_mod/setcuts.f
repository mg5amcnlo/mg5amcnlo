...
CMZ                        if (j_fks.gt.nincoming)
CMZ     &                       taumin(iFKS,ichan) = taumin(iFKS,ichan)-dsqrt(ptl**2+emass(i)**2) +
CMZ     &                              max(mll/2d0,mll_sf/2d0,dsqrt(ptl**2+emass(i)**2))
                        taumin(iFKS,ichan) = taumin(iFKS,ichan)-dsqrt(ptl**2+emass(i)**2) +
...
c                    assume smin apply always on the same set of particle
            ...
            !write(*,*) 'TAU LB1', tau_born_lower_bound, tau_lower_bound
c         
...
c
     ...
     $              ,'     --      ',taumin_s(iFKS,ichan)
      ...
      !write(*,*) 'TAU LB2', tau_born_lower_bound, tau_lower_bound
