...
c Common block with granny information
     ...
     &     ,granny_chain_real_final
      ...
      integer skip
      double precision rand_save(3)
      common /to_randsave/rand_save
      !logical skip_evpr_initial, skip_evpr 
      !parameter (skip_evpr_initial=.true.)

      !skip_evpr = skip_evpr_initial.and.j_fks.le.nincoming
...
c Start by generating all the invariant masses of the s-channels
     ...
     &        ,cBW,cBW_mass,cBW_width,s,x,xjac,pass)
      else
         call generate_inv_mass_sch(ns_channel,itree,m,sqrtshat_born
     $        ,totmass,qwidth,qmass,cBW,cBW_mass,cBW_width,s,x,xjac
     $        ,pass)
      endif
      if (.not.pass) goto 222
...
C   born momenta in the partonic com frame 
      !if (skip_evpr) call boost_born_momenta_noevpr(pb,xp,xi_i_fks,
      call boost_born_momenta_noevpr(pb,xp,xi_i_fks,
     &                      i_fks,shat,srec)

      !write(*,*) 'XI', icountevts, xi_i_fks, shat, xi_i_fks*sqrt(shat)/2d0
      !do i = 1, nexternal
      !    write(*,*) 'XP', i, xp(:,i)
      !enddo
c
...
c must stay so for the computation of enhancement factors.
     ...
     &            (jac_cnt(2).le.0.d0)
      !! MZ: this should be commented out, since otherwise fills the
      !! output with warnings about the counterterms kinematics
C      call xmom_compare(i_fks,j_fks,jac,jac_cnt,p,p1_cnt,
C     &                  p_i_fks_ev,p_i_fks_cnt,
C     &                  xi_i_fks_ev,y_ij_fks_ev,check_cnt)
c check_cnt=.false. is an exceedingly rare situation -- just dump the event
C      if(.not.check_cnt)goto 222
c
...
c
     ...
     &     ( 2-xi_i_fks*(1-xp(0,j_fks)/veckn*y_ij_fks) )
      ...
      subroutine boost_born_momenta_noevpr(pb,xp,xi_i_fks,i_fks,shat,srec)
      implicit none
      include 'nexternal.inc'
      include 'genps.inc'
      double precision pb(0:3,-max_branch:nexternal-1),xp(0:3,nexternal)
      ...
      ! pb are in the recoil center of frame. Must be boosted in the
      ...
      pb(0,1) = sqrt(shat)/2d0
      pb(1,1) = 0d0
      pb(2,1) = 0d0
      pb(3,1) = sqrt(shat)/2d0

      pb(0,2) = sqrt(shat)/2d0
      pb(1,2) = 0d0
      pb(2,2) = 0d0
      pb(3,2) =-sqrt(shat)/2d0
