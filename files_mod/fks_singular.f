...
C or the FKS partner is initial (noevpr)
c use the Born computed with the corresponding momentum mapping.
      enhance_real=1.d0
      !!if ((granny_is_res .or. j_fks.le.nincoming).and. imode.eq.2) then
      if ((granny_is_res).and. imode.eq.2) then
         if (granny_is_res) p_born_used(:,:) = p_born_ev(:,:) 
         !!if (j_fks.le.nincoming) p_born_used(:,:) = p_born_norad(:,:) 
...
c call the analysis/histogramming routines
            ...
            !write(*,*) 'BEFORE OUTFUN',i, plot_id(i)
            !do kkk = 1, nexternal
            !  write(*,*) momenta(:,kkk,i)
            !enddo
