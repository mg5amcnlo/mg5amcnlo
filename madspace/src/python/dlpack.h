...
typedef enum {
     ...
     * Setting bits != 6 is currently unspecified, and the producer must ensure it is
     * set while the consumer must stop importing if the value is unexpected.
     */
    kDLFloat6_e2m3fn = 15U,
    kDLFloat6_e3m2fn = 16U,
    /*! \brief FP4 data types
     * Setting bits != 4 is currently unspecified, and the producer must ensure it is
     * set while the consumer must stop importing if the value is unexpected.
     */
 ...
 * i.e., for a packed data set D ((D >> (i * bits)) && bit_mask) stores the i-th
 * element.
 */
typedef struct {
    /*!
...
struct DLManagedTensorVersioned {
     ...
     * DLManagedTensorVersioned. It can be NULL if there is no way for the caller to
     * provide a reasonable destructor. The destructor deletes the argument self as
     * well.
     */
