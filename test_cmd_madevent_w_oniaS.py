...
C
    ...
    def test_onia(self):
        """check that p p  > Jpsi + etac gives the correct result"""
        
        mg_cmd = MGCmd.MasterCmd()
        mg_cmd.no_notification()
        mg_cmd.exec_cmd(' import model sm_onia-c_mass')
        mg_cmd.exec_cmd(' generate p p  > Jpsi etac')
        mg_cmd.exec_cmd('output %s/' % self.run_dir)
        self.cmd_line = MECmd.MadEventCmdShell(me_dir=  self.run_dir)
        self.cmd_line.no_notification()
    ...
    def test_onia_standalone1(self):
        """Standalone test for p p > Jpsi etac, chekcs a given Fock state combination with gg initial state"""
        ...
        mg_cmd.exec_cmd('import model sm_onia-c_mass')
        mg_cmd.exec_cmd('generate g g > Jpsi(1|3S18) etac(1|1S08)')
        
        # --------------------------------------------------
        # IMPORTANT:
        # Output DIRECTLY to self.run_dir
        # Do NOT create subdirectories manually
        # --------------------------------------------------
        mg_cmd.exec_cmd(f'output standalone {self.run_dir} -f')
        ...
        # --------------------------------------------------
        proc_dirs = glob.glob(os.path.join(self.run_dir,
                                           "SubProcesses",
                                           "P*"))
        ...
        print("Standalone Matrix Element =", val1)
        
        target =  1.552716e-12  # value to comapre to
        
        self.assertAlmostEqual(val1, target, places=8)
