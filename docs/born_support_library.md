...
For each labelled history, the driver projects the fixed real momenta onto the
native Born momenta and recovers only the three radiation coordinates. It does
not invert Born invariant masses, angles, Bjorken sampling coordinates or
integration channels. The radiation generator then constructs the real and
counterevents with a unit Born sampling measure, retaining their radiation and
flux factors. Native clustering uses configuration 1; no Born chart is sampled.
Auxiliary maps use physical thresholds, without integration sampling cutoffs
that would exclude a valid point generated by another history. Physical cuts
are still applied to the native counterevent and real event.
...
The cancellation and the COMMON/SAVE state audit are documented in
[native_fks_projection.md](native_fks_projection.md).
...
radiation projection/reconstruction roundoff does not create artificial differences between
...
`mcatnlo_delta=False` and no shower. Unsupported splitting modes remain
unsupported. Subsequent optimization, fixed-order and Delta
