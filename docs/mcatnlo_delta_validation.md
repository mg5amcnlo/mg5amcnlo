...
### Merge with the history optimisations

Merging `edc385495` into the optimised branch at `33fd22a5d` passed all
81 focused Born, helicity, Delta, native-history and related regression
tests. The new Delta suite is included in the shared Born/native CI job.

A fresh `p p > t t~ [QCD]` export with `loop_sm-no_b_mass` compiled on
three workers and generated 20 unshowered Delta events with the Pythia
path unset. It used 6.5 TeV beams, fixed 173 GeV scales, `nn23nlo`, seed
92325 and requested accuracy 0.1. All three subprocesses passed the
matrix-element and MC checks and 20/20 pole checks at `1e-5`. The complete
integration result in `res_1.txt` was `756.196674 +/- 4.4623 pb`.
The abbreviated post-generation summary omits the channel receiving no
events in this small sample; it is not used as the complete integral.

All 20 events passed colour conservation and directed dipole-coverage
checks, with finite scales and 27 finite scale weights each. Seventeen
events have unequal dipole scales; the maximum relative momentum residual
is `5.98e-16`. The MC counterterm object references the native Fortran
scale module and no removed Pythia runtime symbol. This is an integrated
merge smoke check, not a new precision or shower comparison. Its cards,
logs, LHE file and `delta_lhe_audit.json` are retained under
`/export/tmp/rikkert/mg5_delta_merge_c7kxvrbj`.
