...
class ParamCardWriter:
    ...
    def write_block(self, name, scale=None):
        ...
        if name!='DECAY':
            if scale:
                self.fsock.write(f"""Block {name.lower()} Q= {scale} \n""")
            else:
    ...
    def write_param(self, param, lhablock):
        ...
        if lhablock != 'DECAY':
            text = f"""  {lhacode} {param.value.real:e} # {info} \n""" 
        elif param.value == 'auto':
            text = f'''DECAY {lhacode} auto # {info} \n'''
        else:
            text = f'''DECAY {lhacode} {param.value.real:e} # {info} \n'''
