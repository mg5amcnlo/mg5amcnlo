...
def repr(obj):
    ...
    if text.startswith('_'):
        text =  f'{str(obj.__class__.__name__)[0].upper()}{text}'
    return text
...
class UFOModel:
            ...
            for p in restrict_list: 
                ...
                for parameter in self.new_external:
                    ...
                    try:    
                        ...
                        logger.warning(f"{p} will not acting for {block} {lhaid}")
    ...
    def format_param(self, param):
        ...
        if isinstance(param, str): 
            ...
            return '{%s}' % ','.join([f'{self.format_param(key)}: {self.format_param(value)}' for key, value in param.items()])
    ...
    def create_data_text(self, obj):
        ...
        if hasattr(obj, 'require_args_all'):
            ...
            args = []
        if args:
            text = f"""{repr(obj)} = {obj.__class__.__name__}("""
        else:
            text = f"""{obj.name} = {obj.__class__.__name__}("""
...
"""
    ...
    def add_particle(self, particle, identify=None):
        ...
        if old_part:
            ...
            if old_part.pdg_code == particle.pdg_code:
                ...
                logger.warning(f'The particle coming from the plug-in model will be rename to \'{name}{self.addon}\'')
                particle.name = f'{name}{self.addon}'
                particle.antiname = f'{particle.antiname}{self.addon}'
        ...
        if p_base.mass.name != p_plugin.mass.name:
            ...
            if p_plugin.mass.name in self.old_new:
                if self.old_new[p_plugin.mass.name] != p_base.mass.name:
                    raise USRMODERROR(f'Some inconsistency in the mass assignment in the model: equivalent of {p_plugin.mass.name} is {self.old_new[p_plugin.mass.name]} != {p_base.mass.name} ')
             ...
             '     conflict name %s\n' % self.old_new + \
             f'     pdg_code: {p_base.pdg_code} {p_plugin.pdg_code}')
        # Check the width
        ...
        if old_param:
            if old_param.lhablock == parameter.lhablock and \
                ...
                logger.info('The two model defines the parameter \'%s\'\n' % parameter.name +
                  ...
                  f'      We will rename the one from the plugin to {parameter.name}{self.addon}')
    ...
    ''')
                ...
                self.old_new[parameter.name] = f'{parameter.name}{self.addon}'
                parameter.name = f'{parameter.name}{self.addon}'
                #
    ...
    def add_coupling(self, coupling):
        ...
        if same_name:
            coupling.name = f'{coupling.name}{self.addon}'
    ...
    def add_lorentz(self, lorentz):
        ...
        if same_name:
            lorentz.name = f'{lorentz.name}{self.addon}'
    ...
    def add_interaction(self, interaction , model):
        ...
        if same_name:
            interaction.name = f'{interaction.name}{self.addon}'
        ...
        if identify_particles:
            for new, old in dict(identify_particles).items():
                ...
                if new_part.antiname not in identify_particles:
                    ...
                    logger.info(f"adding identification for anti-particle: {new_anti}={old_anti}")
