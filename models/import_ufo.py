...
def import_model(model_name, decay=False, restrict=True, prefix='mdl_',
    ...
    if restrict_file:
        try:
            logger.info(f'Restrict model {model_name} with file {os.path.relpath(restrict_file)} .')
        except OSError:
            # sometimes has trouble with relative path
            logger.info(f'Restrict model {model_name} with file {restrict_file} .')
...
#                                 delattr(newCoupling,"CTparam_dependence")
    ...
    def shorten_expo(self, matchobj):
        ...
        output = f'{expr}__exp__{new_exponent}'
        old_expr = f'{expr}**{exponent}'
...
class RestrictModel(model_reader.ModelReader):
    ...
    def remove_interactions(self, zero_couplings):
        ...
        for vertex in mod_vertex:
            ...
            orders = [f'{order}={value}' for order,value in vertex['orders'].items()]
