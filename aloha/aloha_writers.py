...
class ALOHAWriterForFortran(WriteALOHA): 
    ...
    def write_combined(self, lor_names, mode='self'):
        ...
        name = combine_name(self.abstractname, lor_names, self.offshell)
        ...
        if not self.offshell:
            header += ' double complex TMP\n'
        else:
            spin = self.particles[self.offshell -1] 
            header += ' double complex TMP(%s)\n integer i' % self.type_to_size[spin]         
        ...
        for i,name in enumerate(lor_names):
            writer.writelines(line % (name, addon, i+2, 'TMP'))
            if not self.offshell:
                writer.writelines(' vertex = vertex + tmp\n')
            else:
                text=""" do i=1,%(id)d
                %(main)s(i) = %(main)s(i) + tmp(i)
                enddo\n"""
                size = self.type_to_size[spin] -2
                text = text % {'id': size, 'main':main}
