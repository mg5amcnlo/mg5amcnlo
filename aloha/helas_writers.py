...
class HelasWriter(object): 
    ...
    # Prototype parameters for make_call_lists
    declare_list_init = []
    declare_dict = {}
    declare_string = ''
    @staticmethod
    def call_string(d, s, i, n):
        """Prototype call_string function for use in make_call_lists"""
        pass
    ...
    def __init__(self, abstracthelas, dirpath):
        ...
        # First look for the different variables
        self.collect_variables()
        # Compute the expression for the call ordering, definition of
        # objects etc.
        self.make_call_lists()
    ...
    def make_call_lists(self):
        ...
        ScalarList = []
        TensorList = [] 
        DeclareList = copy.copy(self.declare_list_init)
        call_string = type(self).call_string
        for index, elem in enumerate(self.particles):
            
            # First define the size of the associate Object 
            DeclareList.append(self.declare_string % \
                               (self.declare_dict[elem], index + 1,
                                self.type_to_size[elem]))
            if elem == 'S':
                ScalarList.append(call_string(self.declare_dict[elem], elem,
                            index + 1, self.type_to_size[elem]))
                ScalarNumber += 1 
            elif elem == 'T':
                TensorList.append(call_string(self.declare_dict[elem], elem,
                            index + 1, self.type_to_size[elem]))
                TensorNumber += 1
            elif elem == 'V':
                VectorList.append(call_string(self.declare_dict[elem], elem,
                            index + 1, self.type_to_size[elem]))
                VectorNumber += 1 
            elif elem == 'F':
                FermiList.append(call_string(self.declare_dict[elem], elem,
                            index + 1, self.type_to_size[elem]))  
                FermionNumber += 1 

            # Define Momentum Conservation
            if elem in ['V', 'S', 'T']:
                MomentumConserve.append('-%s%d' % (elem, index + 1))
...
class HelasWriterForFortran(HelasWriter): 
    """routines for writing out Fortran"""

    extension = '.f'

    # Parameters for make_call_lists
    declare_list_init = ['double complex C']
    declare_dict = {'F':'double complex f', 'V':'double complex V', \
                    'S':'double complex s', 'T':'double complex T'}
    declare_string = '%s%d(%d)'
    @staticmethod
    def call_string(d, s, i, n): return '%s%d' % (s, i)
    ...
    def define_expression(self):
        ...
        if not self.offshell:
            ...
            OffShellParticle = '%s%d' % (self.particles[self.offshell-1], self.offshell)
    ...
    def define_symmetry(self):
        ...
        Outstring = 'call '+self.namestring+'('+','.join(calls)+',C,M%s,W%s,%s%s)'%(number,number, self.particles[self.offshell-1],number)
        return Outstring
...
class HelasWriterForCPP(HelasWriter): 
    """Routines for writing out C++ Helas routines"""
    extension = '.cc'

    # Parameters for make_call_lists
    declare_list_init = ['complex<double> C']
    declare_dict = {'F':'complex<double> F', 'V':'complex<double> V',
                    'S':'complex<double> S', 'T':'complex<double> T'}
    declare_string = '%s%d[%d]'
    @staticmethod
    def call_string(d, s, i, n): return '%s%d[%d]' % (d, i, n)

    def __init__(self, abstracthelas, dirpath):
        """Initialize the HelasWriterForCPP"""

        HelasWriter.__init__(self, abstracthelas, dirpath)
        
        self.out_head = os.path.join(dirpath, self.namestring + '.h')
    ...
    def define_header(self):
        ...
        headerstring += '#define ' + self.namestring + '_guard\n'
        string = '#include <complex>\n using namespace std;\n'
        headerstring += '#include <complex>\n'
        headerstring += 'using namespace std;\n'
        # define the type of function and argument
        if not OffShell:
            callstring = 'void ' + self.namestring + '('+ \
                         ','.join(CallList + ['complex<double> C'] + Mass + \
                                  Width + ['complex<double> &vertex']) + ')'
            headerstring += callstring + ';\n'
            string += callstring + '{\n'
        else: 
            AddedDeclare.append('complex<double> denom')
            temp = self.particles[self.offshell-1]+'%s' %(self.offshell)
            OutputParticle = 'complex<double> ' + temp + \
                             '[%s]' % self.type_to_size[temp[0]]
            callstring = 'void ' + self.namestring + '('+ \
                         ','.join(CallList + ['complex<double> C'] + Mass + \
                                  Width + [OutputParticle]) + ')'
            headerstring += callstring +';\n' 
            string += callstring + '{\n' 
        headerstring += '#endif'
        # Declare all the variable
    ...
    def write(self):        
        hWriter = writers.CPPWriter(self.out_head)
        ccWriter = writers.CPPWriter(self.out_path)
