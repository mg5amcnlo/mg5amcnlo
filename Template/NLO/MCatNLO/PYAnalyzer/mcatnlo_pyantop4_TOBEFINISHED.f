...
C----------------------------------------------------------------------
      REAL*8 XNORM
      INTEGER I,J,K,IVLEP1,IVLEP2,IDEC,MAXEV
      COMMON/VVLIN/IVLEP1,IVLEP2
      COMMON/CMAXEV/MAXEV
      OPEN(UNIT=99,NAME='PYTHIA_QQ.top',STATUS='UNKNOWN')
      XNORM=MAXEV*1.D0/IEVT
      DO I=1,100              
 	CALL MFINAL3(I)             
        CALL MCOPY(I,I+100)
...
C FIND FINAL STATE HADRONS, TO BE CLUSTERED INTO JETS
        ELSEIF(IST.LE.10.AND.ABS(ID1).GT.100)THEN
          NN=NN+1
          IF (NN.GT.NMAX)THEN
...
C FIND THE DECAY PRODUCTS IF SPIN CORRELATIONS ARE INCLUDED
        ...
        IF(K(IT1,5)-K(IT1,4).EQ.1)THEN
          DO IJ=K(K((IT1,5),4),4),K(K(K(IT1,5),4),5)
     ...
     #          K(K(K(IT1,5)-1,4),5)
        ...
        IF(K(IT2,5)-K(IT2,4).EQ.1)THEN
          DO IJ=K(K((IT2,5),4),4),K(K(K(IT2,5),4),5)
...
C FIND THE DECAY PRODUCTS IF SPIN CORRELATIONS ARE NOT INCLUDED
        IWP=K(IT1,4)
        IBQ=K(IT1,5)
        IF(K(IT1,5)-K(IT1,4).EQ.2)THEN
          IF(K(K(IT1,4)+1,2).EQ.5)IBQ=K(IT1,4)+1
        ENDIF
        IWM=K(IT2,4)
        IBB=K(IT2,5)
        IF(K(IT2,5)-K(IT2,4).EQ.2)THEN
          IF(K(K(IT2,4)+1,2).EQ.-5)IBB=K(IT2,4)+1
        ENDIF
C CHECK THAT THE DECAY PRODUCTS ARE WHAT THEY ARE SUPPOSED TO BE
        IF( ( K(IWP,2).NE.24 .OR. K(IWP,1).GT.10 ) .OR.
     #      ( K(IBQ,2).NE.5  .OR. K(IBQ.1).GT.10 ) .OR.
     #      ( K(IWM,2).NE.-24.OR. K(IWM,1).GT.10 ) .OR.
     #      ( K(IBB,2).NE.-5 .OR. K(IBB,1).GT.10 ) )THEN
C CHECK!! THE ORIGINAL WERE ISTHEP=123 AND 124
...
C EXCHANGED IN THE EVENT RECORDS, IT DEPENDS ON THE CALL TO HWMODK
        ...
        ILM=K(K(IWM,4),5)
        INB=K(K(IWM,4),4)
        IF( K(ILP,2).NE.-11 .OR. K(INU,2).NE. 12 .OR.
     #      K(ILM,2).NE. 11 .OR. K(INB,2).NE.-12 )THEN
          WRITE(*,*)IWP,ILP,INU
          WRITE(*,*)IWM,ILM,INB
          WRITE(*,*)'ERROR 507 IN PYANAL'
          STOP
        ENDIF
