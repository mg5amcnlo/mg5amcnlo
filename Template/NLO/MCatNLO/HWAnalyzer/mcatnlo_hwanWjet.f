...
c$$$        IDENT=-24
c$$$      ELSEIF(JPR.EQ.210.OR.JPR.EQ.211.OR.JPR.EQ.212) THEN
c$$$        IDENT=24
c$$$      ELSE
c$$$c        CALL HWWARN('HWANAL',502,*999)
c$$$      ENDIF
      IDENT=24
...
C CHECK MOMENTUM AND CHARGE CONSERVATION
      ...
      ENDIF
C FIND E and NU_E
      IF( ABS(IDHEP(JDAHEP(1,IV))).EQ.11 .AND.
     #    ABS(IDHEP(JDAHEP(2,IV))).EQ.12 )THEN
        IE=JDAHEP(1,IV)
        INU=JDAHEP(2,IV)
      ELSEIF( ABS(IDHEP(JDAHEP(2,IV))).EQ.11 .AND.
     #        ABS(IDHEP(JDAHEP(1,IV))).EQ.12 )THEN
        IE=JDAHEP(2,IV)
        INU=JDAHEP(1,IV)
      ELSE
c        CALL HWUEPR
c        CALL HWWARN('HWANAL',504,*999)
      ENDIF
      DO IJ=1,5
        PPE(IJ)=PHEP(IJ,IE)
        PPNU(IJ)=PHEP(IJ,INU)
