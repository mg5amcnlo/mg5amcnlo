...
c end lhef
      real * 8 xgammax,xmlow,xmupp,xmbin
...
c
      xgammax=30.d0
      xmlow=max(0.d0,rmass(198)-xgammax*gamw)
      xmupp=rmass(198)+xgammax*gamw
      xmbin=(xmupp-xmlow)/100.d0
      xmls=sngl(rmass(198)-(49*xmbin+xmbin/2.d0))
      xmus=sngl(rmass(198)+(50*xmbin+xmbin/2.d0))
      xmbs=sngl(xmbin)
c
      call inihist
      do j=1,2
...
c
      ...
      call mbook(k+ 6,'mV'//cc(j),xmbs,xmls,xmus)
c
...
c
      ...
      call mbook(k+18,'llb inv m'//cc(j),xmbs,xmls,xmus)
...
c begin lhef
     ...
     #   ABS(IDWTUP).EQ.4) THEN
      ...
      ENDIF
C
      OPEN(UNIT=lhefile,NAME='llp.lhe',STATUS='UNKNOWN')
...
c end lhef
      REAL*8 XNORM
      INTEGER I,J,K
      OPEN(UNIT=99,NAME='HERLL.TOP',STATUS='UNKNOWN')
...
c$$$      IF(JPR.EQ.135.OR.JPR.EQ.136.OR.JPR.EQ.137) THEN
c$$$        IDENT=23
c$$$      ELSEIF(JPR.EQ.145) THEN
c$$$        IDENT=24
c$$$      ELSEIF(JPR.EQ.146) THEN
c$$$        IDENT=24
c$$$      ELSEIF(JPR.EQ.147) THEN
c$$$        IDENT=-24
c$$$      ELSE
c$$$        CALL HWWARN('HWANAL',502)
c$$$      ENDIF
      IDENT=24
...
C CHECK MOMENTUM AND CHARGE CONSERVATION
      ...
      ENDIF
C FIND THE LEPTONS
      ILL0=MAX(JDAHEP(1,IV),JDAHEP(2,IV))
      ILLB0=MIN(JDAHEP(1,IV),JDAHEP(2,IV))
      IF(IV.EQ.IHRD)THEN
        IF(ILL0-ILLB0.EQ.1)THEN
          CONTINUE
        ELSEIF(ILL0-ILLB0.EQ.2)THEN
          IF(ABS(IDHEP(ILLB0)).LT.11 .OR. 
     #       ABS(IDHEP(ILLB0)).GT.16)THEN
            ILLB0=ILLB0+1
          ELSEIF(ABS(IDHEP(ILLB0+1)).LT.11 .OR. 
     #           ABS(IDHEP(ILLB0+1)).GT.16)THEN
            CONTINUE
          ELSEIF(ABS(IDHEP(ILLB0+2)).LT.11 .OR. 
     #           ABS(IDHEP(ILLB0+2)).GT.16)THEN
            ILL0=ILL0-1
          ELSE
            CALL HWUEPR
            WRITE(*,*)IV,ILL0,ILLB0
            CALL HWWARN('HWANAL',550)
          ENDIF
        ELSE
          CALL HWUEPR
          WRITE(*,*)IV,ILL0,ILLB0
          CALL HWWARN('HWANAL',540)
        ENDIF
      ENDIF
      IF( IDHEP(JDAHEP(1,ILL0)).GT.0 .AND.
     #    IDHEP(JDAHEP(1,ILLB0)).LT.0 )THEN
        ILL=JDAHEP(1,ILL0)
        ILLB=JDAHEP(1,ILLB0)
      ELSEIF( IDHEP(JDAHEP(1,ILLB0)).GT.0 .AND.
     #        IDHEP(JDAHEP(1,ILL0)).LT.0 )THEN
        ILL=JDAHEP(1,ILLB0)
        ILLB=JDAHEP(1,ILL0)
      ELSE
        CALL HWUEPR
        WRITE(*,*)IV,ILL,ILLB,ILL0,ILLB0
