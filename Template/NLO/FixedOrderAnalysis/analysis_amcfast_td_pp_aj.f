************************************************************************
*
*     Analysis routines for gamma+j production
*
************************************************************************
...
*
      do j=1,2
         do kk=1,nwgt_analysis
            l = ( kk - 1 ) * 4 + ( j - 1 ) * 2
            call bookup(l+1,"y_gm  "//weights_info(kk)//cc(j),
     1                  0.5d0,-3d0,3d0)
            call bookup(l+2,"pT_gm "//weights_info(kk)//cc(j),
     1                  5d0,80d0,180d0)
         enddo
      enddo
*
...
*     Rapidity of the photon
      ygm = getrapidity(p(0,3),p(3,3))
*     Transverse momentum of the photon
      pTgm = dsqrt(p(1,3)**2d0 + p(2,3)**2d0)
...
*
            call mfill(l+1,ygm,www)
            call mfill(l+2,pTgm,www)
         enddo
      enddo
*
      return
...
*
      ...
      do i=1,2
         do kk=1,nwgt_analysis
            l = ( kk - 1 ) * 4 + ( i - 1 ) * 2
            call multitop(l+1,3,2,"y_gm  "," ","LIN")
            call multitop(l+2,3,2,"pT_gm "," ","LIN")
         enddo
      enddo
*
