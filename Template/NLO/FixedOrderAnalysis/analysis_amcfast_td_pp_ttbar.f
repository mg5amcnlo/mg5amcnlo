************************************************************************
*
*     Analysis routines for top-pair production
*
************************************************************************
...
*
      do j=1,2
         do kk=1,nwgt_analysis
            l = ( kk - 1 ) * 4 + ( j - 1 ) * 2
            call bookup(l+1,"y_tt  "//weights_info(kk)//cc(j),
     1                  0.25d0,-3d0,3d0)
            call bookup(l+2,"m_tt  "//weights_info(kk)//cc(j),
     1                  12d0,320d0,800d0)
         enddo
      enddo
*
...
*     
     ...
     &               "only for ttbar production"
         stop 1
      endif
*
*     Check that the third and the forth particle are tops
*
      if (abs(ipdg(3)).ne.6)then
         write(*,*) "error in analysis_fill: "//
     1              "The first outcoming particle must be a top quark",
     2              " ID =",ipdg(3)
         call exit(-10)
      endif
      if (abs(ipdg(4)).ne.6)then
         write(*,*) "error in analysis_fill: "//
     1              "The second outcoming particle must be a top quark",
     2              " ID =",ipdg(4)
         call exit(-10)
      endif
*
...
*     Rapidity of the pair
      ytt = getrapidity(p(0,3)+p(0,4),p(3,3)+p(3,4))
*     Invariant mass of the pair
      mtt = dsqrt(( p(0,3) + p(0,4) )**2d0 - ( p(1,3) + p(1,4) )**2d0 
...
*
      ...
      do i=1,2
         do kk=1,nwgt_analysis
            l = ( kk - 1 ) * 4 + ( i - 1 ) * 2
            call multitop(l+1,3,2,"y_tt  "," ","LIN")
            call multitop(l+2,3,2,"m_tt  "," ","LIN")
         enddo
      enddo
*
