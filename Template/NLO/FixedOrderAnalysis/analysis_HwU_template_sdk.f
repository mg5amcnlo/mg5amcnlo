...
c declare (i.e. book) the histograms
      ...
      call HwU_book(3,'mvv |T@ SM2-LO ', 50,0.d0,1000d0)
      call HwU_book(4,'mvv |T@ INT-LO ', 50,0.d0,1000d0)
      call HwU_book(5,'mvv |T@ NP2-LO ', 50,0.d0,1000d0)
      call HwU_book(6,'mvv |T@ SM2-SDK ', 50,0.d0,1000d0)
      call HwU_book(7,'mvv |T@ INT-SDK ', 50,0.d0,1000d0)
      call HwU_book(8,'mvv |T@ NP2-SDK ', 50,0.d0,1000d0)

      call HwU_book(9,'ptv |T@ SM2-LO ', 50,0.d0,1000d0)
      call HwU_book(10,'ptv |T@ INT-LO ', 50,0.d0,1000d0)
      call HwU_book(11,'ptv |T@ NP2-LO ', 50,0.d0,1000d0)
      call HwU_book(12,'ptv |T@ SM2-SDK ', 50,0.d0,1000d0)
      call HwU_book(13,'ptv |T@ INT-SDK ', 50,0.d0,1000d0)
      call HwU_book(14,'ptv |T@ NP2-SDK ', 50,0.d0,1000d0)
      return
      end
...
c fluctuations).
      integer ibody
c local variable
      double precision var, ptv, mvv, pvv(0:3)
...
c only fill the total rate for the Born when ibody=3
      ...
      pvv(0:3) = p(0:3,3)+p(0:3,4)
      mvv = dsqrt(pvv(0)**2-pvv(1)**2-pvv(2)**2-pvv(3)**2 )
      ptv = dsqrt(p(1,3)**2+p(2,3)**2)

      ! fill the born contributions
      if (ibody.eq.3) then
          if (orders_tag_plot.eq.40000) call HwU_fill(3,mvv,wgts)
          if (orders_tag_plot.eq.40002) call HwU_fill(4,mvv,wgts)
          if (orders_tag_plot.eq.40004) call HwU_fill(5,mvv,wgts)

          if (orders_tag_plot.eq.40000) call HwU_fill(9,ptv,wgts)
          if (orders_tag_plot.eq.40002) call HwU_fill(10,ptv,wgts)
          if (orders_tag_plot.eq.40004) call HwU_fill(11,ptv,wgts)
      else if (ibody.eq.11) then
          if (orders_tag_plot.eq.40000) call HwU_fill(6,mvv,wgts)
          if (orders_tag_plot.eq.40002) call HwU_fill(7,mvv,wgts)
          if (orders_tag_plot.eq.40004) call HwU_fill(8,mvv,wgts)

          if (orders_tag_plot.eq.40000) call HwU_fill(12,ptv,wgts)
          if (orders_tag_plot.eq.40002) call HwU_fill(13,ptv,wgts)
          if (orders_tag_plot.eq.40004) call HwU_fill(14,ptv,wgts)
      else
          write(*,*) 'WRONG IBODY', ibody
          stop 1
      endif
      ! a check to be removed
      if (orders_tag_plot.eq.60004) then
          write(*,*) 'WRONG TAG', orders_tag_plot
          stop 1
      endif
