...
c Jacobian due to delta() of tau_born
      ...
      subroutine generate_tau_BW(stot,x,mass,width,tau,jac)
      implicit none
      real*8 pi
      parameter (pi=3.1415926535897932d0)
      include 'run.inc'
      include 'genps.inc'
      double precision stot,x,tau,jac,mass,width
      double precision smax,smin,xm02,stemp,bwmdpl,bwmdmn,bwfmpl,bwfmmn
      double precision bwdelf,xbwmass3,bwfunc
     ...
     &     ,tau_lower_bound_resonance,tau_lower_bound
      ...
      bwmdpl=smax-xm02
      bwmdmn=xm02-smin
      bwfmpl=atan(bwmdpl/(mass*width))
      bwfmmn=atan(bwmdmn/(mass*width))
      bwdelf=(bwfmpl+bwfmmn)/pi
      stemp=xbwmass3(x,xm02,width,bwdelf,bwfmmn)
      jac=jac*bwdelf/bwfunc(stemp,xm02,width)
      tau=stemp/stot
      jac=jac/stot
      return
      end
...
c Breit Wigner
            ...
            bwmdpl=smax-xm02
            bwmdmn=xm02-smin
            bwfmpl=atan(bwmdpl/(qmass(ibranch)*qwidth(ibranch)))
            bwfmmn=atan(bwmdmn/(qmass(ibranch)*qwidth(ibranch)))
            bwdelf=(bwfmpl+bwfmmn)/pi
            s(ibranch)=xbwmass3(x(-ibranch),xm02,qwidth(ibranch),bwdelf
     &           ,bwfmmn)
            xjac0=xjac0*bwdelf/bwfunc(s(ibranch),xm02,qwidth(ibranch))
         else
c not a Breit Wigner
