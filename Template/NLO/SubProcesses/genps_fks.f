...
c Generate tau according to a Breit-Wiger function
     ...
     $              -ns_channel-1,1),cBW_width(-ns_channel-1,1)
     $              ,tau_born,xjac0)
            else
...
c     one to go on-shell
         a=1d0-width/(width+BWwidth) ! TOO NAIVE: SHOULD USE THE FULL BW INFORMATION HERE...
c     use the mass separation to determine which region of phase-space
c     should be generated by which BW
         if (BWmass.lt.mass) then
            write (*,*) 'Error in generate_tau_BW #1',mass,BWmass
            stop
         endif
         b=(BWmass-mass)/(width+BWwidth)
         b=mass+b*width ! WRONG: THIS GIVES NO PHASE SPACE FOR MOST LIKELY TO GO ON-SHELL
         if (x.lt.a) then
c the resonace BW
            m=mass
            w=width
