...
c Skip father (it cannot be its own colour partner)
                  if(ICOLUP(1,j,i).eq.0.and.ICOLUP(2,j,i).eq.0) nsngl
     $                 =nsngl+1
                  if(ICOLUP(1,j,i).ne.0.and.ICOLUP(2,j,i).ne.0) nglu
     $                 =nglu+1
                  if ( (j.le.nincoming.and.fksfather.gt.nincoming) .or.
     $                 (j.gt.nincoming.and.fksfather.le.nincoming) )
     $                 then
...
c
      ...
      ! these tests only apply for QCD-type splittings
         do i=1,ipartners(0)
            ipart=ipartners(i)
            if( ipart.eq.fksfather .or. ipart.le.0 .or.
     $           ipart.gt.nexternal-1 .or. (
...
c
            if( colorflow(i,0).le.0 .or. colorflow(i,0).gt.max_bcol
     $           )then
               write(*,*)'Error #4 in check_mc_matrices',i,colorflow(i
     $              ,0)
               stop
            endif
c
...
c
     ...
     $        (.not.notagluon).and. ( (.not.isspecial(1)).and.ntot.ne.(2
     $        *num_leading_cflows) .or.
     $        (isspecial(1).and.ntot.ne.num_leading_cflows) ) ) )then
            write(*,*)'Error #6 in check_mc_matrices', notagluon,ntot
...
C stuff to keep track of the individual orders
      double precision amp_split_bornbars(amp_split_size,max_bcol
     $     ,nsplitorders),amp_split_bornbarstilde(amp_split_size
     $     ,max_bcol,nsplitorders)
...
c Initialise
      ...
      xkern(1:2) = 0d0
      xkernazi(1:2) = 0d0
      born_red(1:nsplitorders) = 0d0
      born_red_tilde(1:nsplitorders) = 0d0
...
c Compute MC subtraction terms
         if(lzone(npartner))then
            ...
            if( (ileg.ge.3 .and.
     $           (m_type.eq.8.or.(m_type.eq.1.and.dabs(ch_m).lt.tiny)))
     $           .or.(ileg.le.2 .and.
     $           (j_type.eq.8.or.(j_type.eq.1.and.dabs(ch_j).lt.tiny)))
     $           )then
               if(i_type.eq.8)then
c g->gg, go->gog (icode=1)
                  if(ileg.le.2)then
                     ...
                     elseif(non_limit)then
                        xfact=(1-yi)*(1-x)/x
                        prefact=4/(s*N_p)
                        call AP_reduced(m_type,i_type,ch_m,ch_i,one
     $                       ,z(npartner),ap)
                        do i=1,2
                          ap(i)=ap(i)/(1-z(npartner))
                          xkern(i)=prefact*xfact*xjac(npartner)*ap(i)
     $                         /xi(npartner)
                        enddo
                        call Qterms_reduced_spacelike(m_type,i_type,ch_m
     $                       ,ch_i,one,z(npartner),Q)
                        do i=1,2
                          Q(i)=Q(i)/(1-z(npartner))
                          xkernazi(i)=prefact*xfact*xjac(npartner)*Q(i)
     $                         /xi(npartner)
                        enddo
...
c
                  elseif(ileg.eq.3)then
                     ...
                     if(non_limit)then
                        ...
                        prefact=2/(s*N_p)
                        call AP_reduced_SUSY(j_type,i_type,ch_j,ch_i,one
     $                       ,z(npartner),ap)
                        do i=1,2
                           ap(i)=ap(i)/(1-z(npartner))
                           xkern(i)=prefact*xfact*xjac(npartner)*ap(i)
...
c
                  elseif(ileg.eq.4)then
                     N_p=2
                     if(limit)then
                        xkern(1)=(g**2/N_p)*( 8*vca* (s**2*(1-(1-x)*x)-s
     $                       *(1+x)*xm12+xm12**2)**2 )/ ( s*(s-xm12)**2
     $                       *(s*x-xm12)**2 )
                        xkernazi(1)=-(g**2/N_p)*(16*vca*s*(1-x)**2)/((s
     $                       -xm12)**2)
                        xkern(2)=0d0
                        xkernazi(2)=0d0
                     elseif(non_limit)then
                        xfact=(2-(1-x)*(1-yj))/xij*(1-xm12/s)*(1-x)*(1
     $                       -yj)
                        prefact=2/(s*N_p)
                        call AP_reduced(j_type,i_type,ch_j,ch_i,one
     $                       ,z(npartner),ap)
                        do i=1,2
                           ap(i)=ap(i)/(1-z(npartner))
                           xkern(i)=prefact*xfact*xjac(npartner)*ap(i)
     $                          /xi(npartner)
                        enddo
                        call Qterms_reduced_timelike(j_type,i_type,ch_j
     $                       ,ch_i,one,z(npartner),Q)
                        do i=1,2
                           Q(i)=Q(i)/(1-z(npartner))
                           xkernazi(i)=prefact*xfact*xjac(npartner)*Q(i)
     ...
     $                          xkernazi
               ...
               elseif(abs(i_type).eq.3 .or.
     $                 (i_type.eq.1.and.dabs(ch_i).gt.tiny))then
c g->qq, a->qq, a->ee (icode=2)
                  if(ileg.le.2)then
                     N_p=1
                     if(limit)then
                        xkern(1)=(g**2/N_p)*4*vtf*(1-x)*((1-x)**2+x**2)
     $                       /(s*x)
...
c
     ...
     &                        ( (s-xm12)**2*(s*x-xm12) )
                        xkern(2)=xkern(1) * dble(gal(1))**2 / g**2 *
     $                       ch_i**2 * abs(i_type) / vtf
                        xkernazi(1)=(g**2/N_p)*(16*vtf*s*(1-x)**2)/((s
     $                       -xm12)**2)
     ...
     $              (m_type.eq.1.and.dabs(ch_m).gt.tiny)))
     $              .or.(ileg.le.2 .and. (abs(j_type).eq.3 .or.
     $              (j_type.eq.1.and.dabs(ch_j).gt.tiny))) )then
               if(abs(i_type).eq.3 .or.
     $              (i_type.eq.1.and.dabs(ch_i).gt.tiny))then
c q->gq, q->aq, e->ae (icode=3)
                  if(ileg.le.2)then
                     N_p=2
                     if(limit)then
                        xkern(1)=(g**2/N_p)*4*vcf*(1-x)*((1-x)**2+1)/(s
     $                       *x**2)
                        xkern(2)=xkern(1) * (dble(gal(1))**2 / g**2) *
     $                       (ch_i**2 / vcf)
                        xkernazi(1)=-(g**2/N_p)*16*vcf*(1-x)**2/(s*x**2)
                        xkernazi(2)=xkernazi(1) * (dble(gal(1))**2 / g
     $                       **2) *(ch_i**2 / vcf)
                     elseif(non_limit)then
                        xfact=(1-yi)*(1-x)/x
                        prefact=4/(s*N_p)
...
c
                  elseif(ileg.eq.3)then
                     N_p=1
                     if(non_limit)then
                        xfact=(2-(1-x)*(1-(kn0/kn)*yj))/kn*knbar*(1-x)
     $                       *(1-yj)
                        prefact=2/(s*N_p)
                        call AP_reduced(j_type,i_type,ch_j,ch_i,one
     $                       ,z(npartner),ap)
...
c
                  elseif(ileg.eq.4)then
                     N_p=1
                     if(limit)then
                        xkern(1)=(g**2/N_p)* ( 4*vcf*(1-x)*(s**2*(1-x)
     $                       **2+(s-xm12)**2) )/ ( (s-xm12)*(s*x-xm12)
     $                       **2 )
                        xkern(2)=xkern(1) * (dble(gal(1))**2 / g**2) * 
     &                                      (ch_i**2 / vcf)
                     elseif(non_limit)then
     ...
     $                         /xi(npartner)
               ...
               elseif(i_type.eq.8 .or.
     $                 (i_type.eq.1.and.dabs(ch_i).lt.tiny))then
...
c
     ...
     $                       *(1-yj)
                        ...
                        if(abs(PDG_type(j_fks)).le.6)then
                           if(shower_mc.ne.'HERWIGPP') call
     $                          AP_reduced(j_type,i_type,ch_j,ch_i,one
     $                          ,z(npartner),ap)
                           if(shower_mc.eq.'HERWIGPP') call
     $                          AP_reduced_massive(j_type,i_type,ch_j
     $                          ,ch_i,one,z(npartner), xi(npartner),xm12
     $                          ,ap)
                        else
                           call AP_reduced_SUSY(j_type,i_type,ch_j,ch_i
     $                          ,one,z(npartner),ap)
                        endif
                        do i=1,2
                          ap(i)=ap(i)/(1-z(npartner))
...
c
                  elseif(ileg.eq.4)then
                     N_p=1
                     if(limit)then
                        xkern(1)=(g**2/N_p)*4*vcf* ( s**2*(1+x**2)-2
     $                       *xm12*(s*(1+x)-xm12) )/ ( s*(s-xm12)*(s*x
     $                       -xm12) )
                        xkern(2)=xkern(1) * (dble(gal(1))**2 / g**2) *
     $                       (ch_j**2 / vcf)
                     elseif(non_limit)then
...
c Dead zone
        ...
        !
        amp_split_bornred(1:amp_split_size,1:nsplitorders)=0d0
        amp_split_bornredtilde(1:amp_split_size,1:nsplitorders)=0d0
        born_red(1:nsplitorders)=0d0
        born_red_tilde(1:nsplitorders)=0d0
        do i=1,2
        ...
        do cflows=1,colorflow(npartner,0)
           ...
           do iord = 1, nsplitorders
              born_red(iord)=born_red(iord)+N_p
     $             *bornbars(colorflow(npartner,cflows),iord)
              born_red_tilde(iord)=born_red_tilde(iord)+N_p
     $             *bornbarstilde(colorflow(npartner,cflows),iord)
              do iamp=1,amp_split_size
                 amp_split_bornred(iamp,iord)=amp_split_bornred(iamp
     $                ,iord)+amp_split_bornbars(iamp,colorflow(npartner
     $                ,cflows),iord)
                 amp_split_bornredtilde(iamp,iord)
     $                =amp_split_bornredtilde(iamp,iord)
     $                +amp_split_bornbarstilde(iamp,colorflow(npartner
     $                ,cflows),iord)
              enddo
           enddo
        enddo
c check the lines below
     ...
     $          (iord.ne.qed_pos.and.iord.ne.qcd_pos)) cycle
           if (iord.eq.qcd_pos) then
              xmcxsec(npartner) = xkern(1)*born_red(iord)+xkernazi(1)
     $             *born_red_tilde(iord)
              do iamp=1,amp_split_size
                 amp_split_xmcxsec(iamp,npartner) = xkern(1)
     $                *amp_split_bornred(iamp,iord) + xkernazi(1)
     $                *amp_split_bornredtilde(iamp,iord)
              enddo
           else if(iord.eq.qed_pos) then
              xmcxsec(npartner) = xkern(2)*born_red(iord)+xkernazi(2)
     $             *born_red_tilde(iord)
              do iamp=1,amp_split_size
                 amp_split_xmcxsec(iamp,npartner) = xkern(2)
     $                *amp_split_bornred(iamp,iord) + xkernazi(2)
     $                *amp_split_bornredtilde(iamp,iord)
              enddo
           endif
        enddo
        if (dampMCsubt) then 
           ...
           do iamp=1,amp_split_size
              amp_split_xmcxsec(iamp,npartner) = amp_split_xmcxsec(iamp
     $             ,npartner)*emscwgt(npartner)
           enddo
        endif
        ...
        do iamp=1,amp_split_size
           amp_split_mc(iamp)=amp_split_mc(iamp)+amp_split_xmcxsec(iamp
     $          ,npartner)
        enddo
