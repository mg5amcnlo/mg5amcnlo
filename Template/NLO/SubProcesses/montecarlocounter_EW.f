...
c g->gg, go->gog (icode=1)
                  if(ileg.le.2)then
                     ...
                     elseif(non_limit)then
                        ...
                        enddo
                        xkern(1)=prefact*xfact*xjac(npartner)*ap(1)/xi(npartner)
                        call Qterms_reduced_spacelike(m_type,i_type,ch_m,ch_i,one,z(npartner),Q)
                        do i=1,2
                          Q(i)=Q(i)/(1-z(npartner))
                        enddo
                        xkernazi(1)=prefact*xfact*xjac(npartner)*Q(1)/xi(npartner)
                     endif
c
...
c g->qq, a->qq, a->ee (icode=2)
                  if(ileg.le.2)then
                     ...
                     if(limit)then
                        ...
                        xkern(2)=xkern(1)*(dble(gal(1))**2/g**2)*(ch_i**2*vca/vtf)
...
c
     ...
     &                        ( (s-xm12)**2*(s*x-xm12) )
                        ...
                        xkernazi(2)=xkernazi(1)*(dble(gal(1))**2/g**2)*(ch_i**2*vca/vtf)
                        if(abs(i_type).eq.1)xkernazi(2)=xkernazi(2)/vca
                     elseif(non_limit)then
                        xfact=(2-(1-x)*(1-yj))/xij*(1-xm12/s)*(1-x)*(1-yj)
                        prefact=2/(s*N_p)
                        call AP_reduced(j_type,i_type,ch_j,ch_i,one,z(npartner),ap)
...
c q->gq, q->aq, e->ae (icode=3)
                  if(ileg.le.2)then
                     ...
                     if(limit)then
                        ...
                        xkern(2)=xkern(1)*(dble(gal(1))**2/g**2)*(ch_i**2/vcf)
                        xkernazi(1)=-(g**2/N_p)*16*vcf*(1-x)**2/(s*x**2)
                        xkernazi(2)=xkernazi(1)*(dble(gal(1))**2/g**2)*(ch_i**2/vcf)
                     elseif(non_limit)then
                        xfact=(1-yi)*(1-x)/x
                        prefact=4/(s*N_p)
                        call AP_reduced(m_type,i_type,ch_m,ch_i,one,z(npartner),ap)
...
c q->qg, q->qa, sq->sqg, sq->sqa, e->ea (icode=4)
                  if(ileg.le.2)then
                     ...
                     if(limit)then
                        ...
                        xkern(2)=xkern(1)*(dble(gal(1))**2/g**2)*(ch_m**2/vcf)
                     elseif(non_limit)then
                        xfact=(1-yi)*(1-x)/x
                        prefact=4/(s*N_p)
                        call AP_reduced(m_type,i_type,ch_m,ch_i,one,z(npartner),ap)
...
c
     ...
     &                        ( s*(s-xm12)*(s*x-xm12) )
                        xkern(2)=xkern(1)*(dble(gal(1))**2/g**2)*(ch_j**2/vcf)
                     elseif(non_limit)then
                        xfact=(2-(1-x)*(1-yj))/xij*(1-xm12/s)*(1-x)*(1-yj)
                        prefact=2/(s*N_p)
                        call AP_reduced(j_type,i_type,ch_j,ch_i,one,z(npartner),ap)
...
c check the lines below
        do iord = 1, nsplitorders
           ...
           if(iord.eq.qcd_pos)xmcxsec(npartner)=
     &     xmcxsec(npartner)+xkern(1)*born_red+xkernazi(1)*born_red_tilde
           if(iord.eq.qed_pos)xmcxsec(npartner)=
     &     xmcxsec(npartner)+xkern(2)*born_red+xkernazi(2)*born_red_tilde
        enddo
