...
C FOUND A TOP; KEEP ONLY THE FIRST ON RECORD
        ...
        ELSEIF(IST.EQ.2.AND.ID1.EQ.-5)THEN
C FOUND AN ANTIBOTTOM; KEEP ONLY THE FIRST ON RECORD
          IQB=IQB+1
          IF(IQB.EQ.1)IB1=IHEP
        ELSEIF(IST.GE.123.AND.IST.LE.125.AND.ABS(ID1).NE.6)THEN
          NPRIMARY=NPRIMARY+1
          IF(NPRIMARY.GT.10)THEN
            CALL HWUEPR
            WRITE(*,*)'NPRIMARY=',NPRIMARY
            CALL HWWARN('HWANAL',511)
          ENDIF
          JPOS(NPRIMARY)=IHEP
        ENDIF
  100 CONTINUE
      IF((IQT.EQ.0.or.IQB.EQ.0).AND.IERROR.EQ.0)
...
c IBQ is the position in the event record of the b quark before hadronization
     ...
     #          JDAHEP(2,JDAHEP(1,JDAHEP(2,IT1)-1))
        ...
        ENDIF
        IBH=JDAHEP(1,JDAHEP(1,JDAHEP(1,IBQ)))
        IBH1=JDAHEP(2,JDAHEP(1,JDAHEP(1,IBQ)))
        ID1=IHADR(IDHEP(IBH1))
        IF(ID1.EQ.5)IBH=IBH1
        IF(ISTHEP(IBH).NE.1.AND.ISTHEP(IBH).NE.196.AND.
     #     ISTHEP(IBH).NE.197.AND.ISTHEP(IBH).NE.199)THEN
          CALL HWUEPR
          CALL HWWARN('HWANAL',503)
        ENDIF
c IBH is the position in the event record of the b hadron emerging from top
c decay. If such hadron is unstable (status 196 or 197), find the stable
c b hadron emerging from its decay. Status 199 seems to correspond to an
c hadronically-decaying hadrons (such as eta_b).
        IF(ISTHEP(IBH).EQ.196)THEN
          IBH1=JDAHEP(1,IBH)
        ELSEIF(ISTHEP(IBH).EQ.197)THEN
          IBH1=JDAHEP(1,IBH)
          ID1=IHADR(IDHEP(IBH1))
          IF(ID1.NE.5)IBH1=JDAHEP(2,IBH)
c The following fails if there are more than two unstable b-hadrons that decay
          IF(ISTHEP(IBH1).EQ.198)THEN
            IBH2=JDAHEP(1,IBH1)
            ID1=IHADR(IDHEP(IBH2))
            IF(ID1.NE.5)IBH2=JDAHEP(2,IBH1)
            IBH1=IBH2
          ENDIF
        ELSEIF(ISTHEP(IBH).EQ.199)THEN
c Appears to be rare, so throw the event away and continue, in order 
c not to complicate the analysis too much (for findind the bjet)
          CALL HWWARN('HWANAL',114)
          goto 999
        ELSE
          IBH1=IBH
        ENDIF
        IF(ISTHEP(IBH1).NE.1)THEN
          CALL HWUEPR
