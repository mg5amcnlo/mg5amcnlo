...
c RF
      SUBROUTINE MTOP(N,M,BTIT,LTIT,SCALE,norm)
c      SUBROUTINE MTOP(N,M,BTIT,LTIT,SCALE,nevent,sum,norm,gnu)
cend
...
c$$$      if (gnu) then
c$$$         call mtopgnuplot(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
c$$$      else
c$$$         call MTOPtdrawer(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
         call MTOPtdrawer(N,M,BTIT,LTIT,SCALE,norm)
c$$$      endif


      END

c$$$      SUBROUTINE MTOPtdrawer(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
      SUBROUTINE MTOPtdrawer(N,M,BTIT,LTIT,SCALE,norm)
...
c$$$         IF(abs(HIST(N,J  )).le.maxbin*10d-7.and.
c$$$     &      abs(HIST(N,J-1)).le.maxbin*10d-7.and.
c$$$     &      abs(HIST(N,J+1)).le.maxbin*10d-7     ) GO TO 1
         WRITE(99,'(3X,F10.4,2(2X,E15.4))')  
     &        XHIS(N,J),HIST(N,J)*norm,HIST(M,J)*norm
c$$$     &                            XHIS(N,J),HIST(N,J),HIST(M,J)
    1 CONTINUE
      WRITE(99,200)
  200 FORMAT('   HISTO')
c$$$      WRITE(99,300) sum,HAVG(N),HSIG(N),nevent,ient(n),IUSCORE(N)
c$$$     &   ,IOSCORE(N)
  300 FORMAT( /1x,
     &' (INFO-BOX'/,1X,                               
