...
# Pierre: no ME jobs ...
    ...
    class create_file:
        ...
        def all(self):
            ...
            # Pierre: no normalization by Xsection
            #if self.MWparam.norm_with_cross:
            #    self.all_MadEvent()
        ...
        def all_MadWeight(self,main=0):
            ...
            nb_create_file=len(self.MWparam.MW_listdir)*len(self.MWparam.actif_param)*(self.MWparam.nb_event-self.MWparam.startevent)
            if hasattr(self.MWparam,'n_join'):
                Npacket=(self.MWparam.nb_event-self.MWparam.startevent)/self.MWparam.n_join
                if ((self.MWparam.nb_event-self.MWparam.startevent) != self.MWparam.n_join*Npacket):
                    Npacket+=1
            self.pbar=progressbar.progbar('MW Creation/Submission',Npacket*len(self.MWparam.MW_listdir)*len(self.MWparam.actif_param))
        ...
        def Event(self,directory,nb_event,dir_type):
            ...
            if dir_type=='M':
            	...
            	self._packet([directory]*n_job,               #directory
            				...
            				dir_type=['P']*n_job,prog='madevent.py')					 # type of job
        ...
        def Card(self,directory,nb_card,dir_type):
            ...
            n_job=self.MWparam.nb_event_MW[directory]	
            if n_job == 0:
                return
            self._packet([directory]*n_job,               #directory
            			cardnb=[nb_card]*n_job,                 #first number  
            			eventnb=range(0,n_job),  #second number
        ...
        def _packet(self,directory,cardnb,eventnb=[],dir_type=[],prog=''):
            ...
            if not eventnb and dir_type[0]=='P':
                eventnb=[-1]*len(cardnb)
    ...
    class launch:
        ...
        def directory(self,directory,dir_type='auto'):
            ...
            for event in list_nbevent:
                if not os.path.exists(os.path.join('SubProcesses',directory,self.MWparam.name,'card_%s' % list_nbcard[0],'event_%s' % event)):
                    break
    ...
    class control:
        ...
        def init_block_level(self):
            ...
            if self.MWparam.norm_with_cross:
# Pierre: currently no normalization 
                return 
        ...
        def block_control(self,block,dir_type,level=0):
            ...
            if dir_type == 'M':
                list_nbcard=self.MWparam.actif_param
                list_nbevent=range(self.MWparam.startevent,self.MWparam.nb_event_MW[block])
                list_dir=[block]
            elif(dir_type == 'P'):
                list_nbevent=[-1]
                list_nbcard=[block]
