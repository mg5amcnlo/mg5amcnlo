module dual_variables
   ...
   type :: DualVariable
      ...
      ! complex(kind(1d0)) :: comp(0:(der_order+1)**npwave-1) = (0d0,0d0) ! npwave and der_order taken from onia.inc
   ...
   integer,parameter::bnarray(nmax)=(/1,2,5,15,52,203,877,4140,21147,115975/)
   
   contains

!   subroutine get_lenght(DV)
!     class(DualVariable),intent(in)::DV
...
!       class(DualVariable), intent(in) :: self
!       class(DualVariable), intent(in) :: other   ! accepts any type
!       res = .false.
!       
...
!  Addition rules
   function add_DualVariable(a, b) result(res)
      type(DualVariable), intent(in) :: a, b
      type(DualVariable) :: res
      integer::i,nn
      if(size(a).ne.size(b))then
         WRITE(*,*)"Error: cannot add two different type of dual variables"
         stop
      endif
      nn=size(a)
      do i=0,2**nn-1
   ...
   function add_CN_DualVariable(cn,a) result(res)
      ...
      if(size(res).ne.nn)then
         WRITE(*,*)"Error: the output dual variable does not match in add"
         stop
      endif
...
!  Subtraction rules
   function minus_DualVariable(a, b) result(res)
      ...
      integer::i,nn
      if(size(a).ne.size(b))then
         WRITE(*,*)"Error: cannot minus two different type of dual variables"
         stop
      endif
      nn=size(a)
      if(size(res).ne.nn)then
         WRITE(*,*)"Error: the output dual variable does not match in minus"
         stop
      endif
      do i=0,2**nn-1
         res%comp(i) = a%comp(i) - b%comp(i)
...
!  Multiplication rules
   function multiply_DualVariable(a, b) result(res)
      ...
      if(size(a).ne.size(b))then
         WRITE(*,*)"Error: cannot multiply two different type of dual variables"
         stop
      endif
      nn=size(a)
      if(size(res).ne.nn)then
         WRITE(*,*)"Error: the output dual variable does not match in multiply"
         stop
      endif      
      do i=0,2**nn-1
      enddo
      do i=0,2**nn-1
         ...
         do j=1,2**n_ones
            res%comp(i)=res%comp(i)+a%comp(c12(j,1))*b%comp(c12(j,2))
         enddo
      enddo
...
!  Power rules
   function power_DualVariable_int(a,np) result(res)
      ...
      if(nn.gt.nmax)then
         write(*,*)"Error: please increase nmax and bnarray in power_DualVariable_int"
         stop
      endif
      ...
      do i=0,2**nn-1
         ...
         call count_binary_ones(i,n_ones)
         bn=bell(n_ones)
         call generate_binary_partitions(i,n_ones,bn,c_split(1:bn,0:n_ones))
         res%comp(i) = dcmplx(0d0,0d0)
         do j=1,bn
            ng=c_split(j,0)
   ...
   function power_DualVariable_real(a,np) result(res)
      ...
      if(nn.gt.nmax)then
         write(*,*)"Error: please increase nmax and bnarray in power_DualVariable_real"
         stop
      endif
...
!  Division rules
   function division_DualVariable(a, b) result(res)
      type(DualVariable), intent(in) :: a, b
      type(DualVariable)::oneoverb
      type(DualVariable) :: res
      integer::nn
      if(size(a).ne.size(b))then
         WRITE(*,*)"Error: cannot division two different type of dual variables"
         stop
      endif
      nn=size(a)
      if(size(res).ne.nn)then
         WRITE(*,*)"Error: the output dual variable does not match in division"
         stop
      endif
...
!  Square-root rule
   function sqrt_DualVariable(a) result(res)
      ...
      nn=size(a)
      if(size(res).ne.nn)then
         WRITE(*,*)"Error: the output dual variable does not match in sqrt"
         stop
      endif
...
!  Natural-logarithmic rule
   function log_DualVariable(a) result(res)
      ...
      nn=size(a)
      if(size(res).ne.nn)then
         WRITE(*,*)"Error: the output dual variable does not match in log"
         stop
      endif

      if(nn.gt.nmax)then
         write(*,*)"Error: please increase nmax and bnarray in log_DualVariable"
         stop
      endif
...
! Conjugation rules
   ...
   function Dual_Conjugation(a, j) result(res)
      ...
      if (min_idx.LE.0) then
         write(*,*) "Error: index of dual conjugation cannot be less or equal than 0"
         stop
      end if

      if (max_idx.GT.nn) then
      write(*,*) "Error: index of dual conjugation (", j, ") exceed dual variable dimension (", nn, ")"
      stop
      end if
...
! Additional functions
   ...
   subroutine generate_binary_partitions(c,n,bn,c_split)
      ...
      ENDDO
      if(n_ones.ne.n)then
         WRITE(*,*)"ERROR: input does not match in generate_binary_partitions"
         stop
      ENDIF
      ngroups = 0
      ic = 0
      call genrate_binary_partitions_rec(1,n,group,ngroups,one_positions,bn,ic,c_split)
      return
   end subroutine generate_binary_partitions

   recursive subroutine genrate_binary_partitions_rec(i,m,group,ngroups,bits,bn,ic,c_split)
      IMPLICIT NONE
      ...
      do g = 1, ngroups
         group(i) = g
         call genrate_binary_partitions_rec(i+1,m,group,ngroups,bits,bn,ic,c_split)
      end do
