C**************************************************************************
c     dummy_fct_fiducialCuts.f
c     Imposes the fiducial cuts on final-state objects
C**************************************************************************
...
C     
      ...
      integer kk,jj
      double precision minPtX,maxEta,minDR2
      double precision rap,pt,r2
      external rap,pt,r2
...
c     for each incoming:
      do kk=nincoming+1,nexternal
c     check eta            
         if(abs(rap(p(0,kk))).gt.maxEta) then
            dummy_cuts=.false.
            return
         endif
c     check pT            
         if(pt(p(0,kk)).lt.minPtX) then
            dummy_cuts=.false.
            return
         endif
      enddo
c     check separation
      do kk=nincoming+1,nexternal-1
         do jj=kk+1,nexternal
            if(r2(p(0,kk),p(0,jj)).lt.minDR2) then
               dummy_cuts=.false.
               return
            endif
         enddo
