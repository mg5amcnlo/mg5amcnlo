...
extern "C" {
    ...
    void get_momenta_(double*, double*, double*);
...
// momenta
void GetMomenta(std::vector<double>* momenta, double* x1, double* x2) {
    get_momenta_(momenta->data(), x1, x2);
}
...
extern "C" void call_madgraph(
    ...
    for (int ibatch = 0; ibatch < nbatch; ibatch++) {
        ...
        GetMomenta(&momenta, &x1, &x2);
        for(size_t imom = 0; imom < 4*npart; ++imom){
            mom_out[ibatch*4*npart + imom] = momenta[imom];
        }
