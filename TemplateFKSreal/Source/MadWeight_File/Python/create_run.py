...
sys.path.append('../Transfer_Fct/')
sys.path.append('Source/MadWeight_File/Transfer_Fct/')
sys.path.append('../Source/MadWeight_File/Transfer_Fct/')
import change_tf
...
def update_cuts_status(MW_param):
    ...
    if (MW_param.info['mw_run']['91']+cut_is_active('cuts.f'))%2: #pass if not coherent between asked and actual status
        ...
        rule=['./Source/MadWeight_File/mod_file/suppress_cuts_MG']
        #modify file
        mod_file.mod_file(file_to_mod,rule,opt={'nowarning':"""['PASSCUTS','MW_NEW_DEF','DESACTIVATE_BW_CUT']"""})
        #check update
        check_Subprocesses_update('cuts.f',MW_param.P_listdir,'./Source/MadWeight_File/mod_file/suppress_cuts_MG')
    ...
    if (MW_param.info['mw_run']['9']+cut_is_active('cuts_MW.f'))%2: #pass if not coherent between asked and actual status
        ...
        rule=['./Source/MadWeight_File/mod_file/suppress_cuts_MW']
        #modify file
        mod_file.mod_file(file_to_mod,rule,opt={'nowarning':"""['PASSCUTS','TO_SPECISA','DESACTIVATE_BW_CUT']"""})
        #check update
        check_Subprocesses_update('cuts_MW.f',MW_param.MW_listdir,'./Source/MadWeight_File/mod_file/suppress_cuts_MW')
    ...
    if (MW_param['mw_run']['me_bw_cut']+bw_cut_is_active('cuts.f'))%2: #pass if not coherent between asked and actual status
        file_to_mod=['./SubProcesses/cuts.f']
        rule=['./Source/MadWeight_File/mod_file/suppress_BW_cuts']
        #modify file
        mod_file.mod_file(file_to_mod,rule,opt={'nowarning':"""['PASSCUTS','MW_NEW_DEF']"""})
        #check update 
        check_Subprocesses_update('cuts.f',MW_param.P_listdir,'./Source/MadWeight_File/mod_file/suppress_BW_cuts')
    ...
    if (MW_param.info['mw_run']['mw_bw_cut']+bw_cut_is_active('cuts_MW.f'))%2: #pass if not coherent between asked and actual status
        ...
        #check update
        check_Subprocesses_update('cuts_MW.f',MW_param.MW_listdir,'./Source/MadWeight_File/mod_file/suppress_BW_cuts')
...
def activate_acceptance_run():
    ...
    mod_file.mod_file('./Source/combine_events.f','./Source/MadWeight_File/mod_file/mod_combine_events')

    #4. modify pythia_card.dat
    mod_file.mod_file('./Cards/pythia_card.dat','./Source/MadWeight_File/mod_file/mod_combine_events')
